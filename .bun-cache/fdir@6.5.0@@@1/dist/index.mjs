...
const resolveSymlinksAsync = (path, state, callback$1) => {
  const {
    queue,
    fs,
...
const resolveSymlinks = (path, state, callback$1) => {
  const {
    queue,
    fs,
    options: { suppressErrors },
  ...
  walk = (entries, directoryPath, depth) => {
    ...
    for (let i = 0; i < entries.length; ++i) {
      ...
      if (
        ...
        const path = joinPathWithBasePath(entry.name, directoryPath);
