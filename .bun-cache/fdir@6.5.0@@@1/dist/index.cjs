...
function pushDirectoryWithRelativePath(root) {
  return (directoryPath, paths) => {
    paths.push(directoryPath.substring(root.length) || ".");
  };
}
function pushDirectoryFilterWithRelativePath(root) {
  return (directoryPath, paths, filters) => {
...
const resolveSymlinksAsync = (path$1, state, callback$1) => {
  const {
    queue,
    fs: fs$1,
...
const resolveSymlinks = (path$1, state, callback$1) => {
  const {
    queue,
    fs: fs$1,
    options: { suppressErrors },
  ...
  walk = (entries, directoryPath, depth) => {
    ...
    for (let i = 0; i < entries.length; ++i) {
      ...
      if (
        ...
        const path$1 = joinDirectoryPath(
          entry.name,
          directoryPath,
        ...
        const path$1 = joinPathWithBasePath(entry.name, directoryPath);
...
function callback(root, options, callback$1) {
  const walker = new Walker(root, options, callback$1);
  walker.start();
}
