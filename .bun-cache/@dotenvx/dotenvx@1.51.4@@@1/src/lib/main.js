...
const config = (options = {}) => {
  ...
  try {
    ...
    if (opsOn) {
      try {
        new Radar().observe({ beforeEnv, processedEnvs, afterEnv });
      } catch {}
      try {
        new Ops().observe({ beforeEnv, processedEnvs, afterEnv });
      } catch {}
    }
    ...
    for (const processedEnv of processedEnvs) {
      if (processedEnv.type === "envVaultFile") {
        logger.verbose(
          `loading env from encrypted ${processedEnv.filepath} (${path.resolve(processedEnv.filepath)})`,
        );
        logger.debug(
          `decrypting encrypted env from ${processedEnv.filepath} (${path.resolve(processedEnv.filepath)})`,
        );
      }

      if (processedEnv.type === "envFile") {
        logger.verbose(
          `loading env from ${processedEnv.filepath} (${path.resolve(processedEnv.filepath)})`,
        );
      }
      ...
      for (const error of processedEnv.errors || []) {
        ...
        if (error.code === "MISSING_ENV_FILE") {
          if (!options.convention) {
            // do not output error for conventions (too noisy)
...
const set = (key, value, options = {}) => {
  ...
  const { processedEnvs, changedFilepaths, unchangedFilepaths } = new Sets(
    key,
    value,
    envs,
    encrypt,
  ...
  for (const processedEnv of processedEnvs) {
    ...
    if (processedEnv.error) {
      if (processedEnv.error.code === "MISSING_ENV_FILE") {
        ...
        logger.help(
          `? add one with [echo "HELLO=World" > ${processedEnv.envFilepath}] and re-run [dotenvx set]`,
        );
      } else {
        logger.warn(processedEnv.error.message);
      ...
      logger.verbose(
        `${processedEnv.key} set${withEncryption} (${processedEnv.envFilepath})`,
      );
      logger.debug(
        `${processedEnv.key} set${withEncryption} to ${processedEnv.value} (${processedEnv.envFilepath})`,
      );
    }
  }
  ...
  if (changedFilepaths.length > 0) {
    logger.success(
      `✔ set ${key}${withEncryption} (${changedFilepaths.join(",")})`,
    );
  ...
  for (const processedEnv of processedEnvs) {
    if (processedEnv.privateKeyAdded) {
      logger.success(
        `✔ key added to ${processedEnv.envKeysFilepath} (${processedEnv.privateKeyName})`,
      );
      logger.help(
      ...
      logger.help(
        `⮕  next run: [${processedEnv.privateKeyName}='${processedEnv.privateKey}' dotenvx get ${key}] to test decryption locally`,
      );
    }
  }
...
/** @type {import('./main').ls} */
const ls = (directory, envFile, excludeEnvFile) =>
  new Ls(directory, envFile, excludeEnvFile).run();

/** @type {import('./main').genexample} */
const genexample = (directory, envFile) =>
  new Genexample(directory, envFile).run();

/** @type {import('./main').keypair} */
const keypair = (envFile, key, envKeysFile = null) => {
