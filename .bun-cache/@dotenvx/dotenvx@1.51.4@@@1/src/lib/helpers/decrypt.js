...
function decrypt(ciphertext, dotenvKey) {
  ...
  try {
    ...
    if (e.code === "ERR_CRYPTO_INVALID_AUTH_TAG") {
      const error = new Error(
        "[INVALID_CIPHERTEXT] Unable to decrypt what appears to be invalid ciphertext.",
      );
      error.code = "INVALID_CIPHERTEXT";
      error.help =
        "[INVALID_CIPHERTEXT] Run with debug flag [dotenvx run --debug -- yourcommand] or manually check .env.vault.";
