...
function executeDynamic(program, command, rawArgs) {
  ...
  const result = childProcess.spawnSync(`dotenvx-${command}`, forwardedArgs, {
    stdio: "inherit",
    env,
  });
  if (result.error) {
    if (command === "radar") {
      logger.warn(
        `[INSTALLATION_NEEDED] install dotenvx-${command} to use [dotenvx ${command}] 📡`,
      );
      logger.warn(
        "[DEPRECATION NOTICE] dotenvx-radar to be sunsetted soon (2026) and its featureset to be rolled into dotenvx-ops",
      );
      logger.help(
        "? see installation instructions [https://dotenvx.com/radar]",
      );
    } else if (command === "ops") {
