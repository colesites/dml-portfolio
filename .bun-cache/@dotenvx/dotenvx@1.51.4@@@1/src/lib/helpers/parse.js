...
class Parse {
  ...
  run() {
    ...
    while ((match = Parse.LINE.exec(lines)) !== null) {
      ...
      if (
        quote !== "'" &&
        (!this.inProcessEnv(key) || this.processEnv[key] === this.parsed[key])
      ) {
      ...
      if (
        !evaled &&
        quote !== "'" &&
        (!this.processEnv[key] || this.overload)
      ) {
        this.parsed[key] = resolveEscapeSequences(
          this.expand(this.parsed[key]),
        );
      }
      ...
      if (Object.hasOwn(this.processEnv, key) && !this.overload) {
  ...
  expand(value) {
    ...
    while ((match = regex.exec(result)) !== null) {
      ...
      if (
        this.literals[key] &&
        /\$\{[^}]+\}|\$[A-Za-z_][A-Za-z0-9_]*/.test(this.literals[key])
      ) {
        break;
      }
  ...
  inProcessEnv(key) {
    return Object.hasOwn(this.processEnv, key);
  }
