...
function executeExtension(ext, command, rawArgs) {
  ...
  if (result.error) {
    ...
    if (["vault", "hub"].includes(command)) {
      logger.warn(
        `[INSTALLATION_NEEDED] install dotenvx-ext-${command} to use [dotenvx ext ${command}] commands`,
      );
      logger.help(
        "? see installation instructions [https://github.com/dotenvx/dotenvx-ext-vault]",
      );
    } else {
