...
class Radar {
  ...
  _radarNpm() {
    const fallbackBin = path.resolve(
      process.cwd(),
      "node_modules/.bin/dotenvx-radar",
    );
    const status = childProcess.execSync(`${fallbackBin} status`, {
  ...
  _radarCli() {
    const status = childProcess.execSync("dotenvx-radar status", {
      stdio: ["pipe", "pipe", "ignore"],
    });
