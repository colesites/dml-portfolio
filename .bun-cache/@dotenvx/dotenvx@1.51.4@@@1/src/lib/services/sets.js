...
class Sets {
      ...
      if (this.encrypt) {
        ...
        if (existingPrivateKey) {
          ...
          if (row.originalValue) {
            row.originalValue = decryptKeyValue(
              row.key,
              row.originalValue,
              privateKeyName,
              privateKey,
          ...
          if (existingPublicKey && existingPublicKey !== publicKey) {
            const error = new Error(
              `derived public key (${truncate(publicKey)}) does not match the existing public key (${truncate(existingPublicKey)})`,
            );
          ...
          if (!existingPublicKey) {
            ...
            const prependPublicKey = this._prependPublicKey(
              publicKeyName,
              publicKey,
              filename,
              relativeFilepath,
  ...
  _prependPublicKey(
    ...
    publicKey,
    filename,
    relativeFilepath = ".env.keys",
  ) {
    const comment =
      relativeFilepath === ".env.keys" ? "" : ` # -fk ${relativeFilepath}`;
