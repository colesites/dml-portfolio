...
class Rotate {
  ...
  _rotateEnvFile(envFilepath) {
    ...
    try {
      ...
      this.envKeysSources[envKeysFilepath] ||= fsx.readFileX(envKeysFilepath, {
        encoding: keysEncoding,
      });
      ...
      for (const [key, value] of Object.entries(envParsed)) {
        // re-encrypt each individual key
        ...
        if (isEncrypted(value)) {
          // only re-encrypt those already encrypted
          row.keys.push(key); // track key(s)

          const decryptedValue = decryptKeyValue(
            key,
            value,
            privateKeyName,
            existingPrivateKey,
