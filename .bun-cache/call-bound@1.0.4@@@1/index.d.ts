...
type IntrinsicPath =
  | IntrinsicName
  | `${StripPercents<IntrinsicName>}.${string}`
  | `%${StripPercents<IntrinsicName>}.${string}%`;
...
type BindMethodPrecise<F> = F extends (
  this: infer This,
  ...args: infer Args
) => infer R
...
type GetPrototypeMethod<
  T extends keyof typeof globalThis,
  M extends string,
...
interface SpecialCases {
  ...
  "%Array.prototype.map%": <T, U>(
    array: readonly T[],
    callbackfn: (value: T, index: number, array: readonly T[]) => U,
    thisArg?: any,
  ) => U[];
  "%Array.prototype.filter%": <T>(
    array: readonly T[],
  ...
  "%Function.prototype.bind%": <T, A extends any[], R>(
    fn: (...args: A) => R,
    thisArg: any,
    ...args: A
  ) => (...remainingArgs: A) => R;
