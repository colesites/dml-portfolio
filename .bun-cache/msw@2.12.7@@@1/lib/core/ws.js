...
function createWebSocketLinkHandler(url) {
  (0, import_outvariant.invariant)(
    url,
    "Expected a WebSocket server URL but got undefined",
  );
  (0, import_outvariant.invariant)(
  ...
  const clientManager =
    new import_WebSocketClientManager.WebSocketClientManager(webSocketChannel);
  return {
    get clients() {
    ...
    addEventListener(event, listener) {
      ...
      handler[import_WebSocketHandler.kEmitter].on(
        "connection",
        async ({ client }) => {
