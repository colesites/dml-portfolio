...
const buildCtx = (opts) => {
  ...
  assert(
    typeof ctx.spacerNoNeighbour === "string",
    'Option "spacerNoNeighbour" has invalid format',
  );
  assert(
    typeof ctx.spacerNeighbour === "string",
    'Option "spacerNeighbour" has invalid format',
  );
  assert(
    typeof ctx.keyNoNeighbour === "string",
    'Option "keyNoNeighbour" has invalid format',
  );
  assert(
    typeof ctx.keyNeighbour === "string",
    'Option "keyNeighbour" has invalid format',
  );
  assert(
    typeof ctx.sortFn === "function" || ctx.sortFn === null,
    'Option "sortFn" has invalid format',
  );
  return ctx;
};
  ...
  while (keys.length !== 0) {
    ...
    result.push(
      [
        ...
        key[key.length - 1],
        ["boolean", "string", "number"].includes(typeof node)
          ? `: ${node}`
          : "",
      ].join(""),
    );
