...
import { AGENTS, detect as detect$1 } from "package-manager-detector";
...
function requireSupportsHyperlinks() {
  ...
  function supportsHyperlink(stream) {
    ...
    if (FORCE_HYPERLINK) {
      return !(
        FORCE_HYPERLINK.length > 0 && parseInt(FORCE_HYPERLINK, 10) === 0
      );
    }
...
const supportsHyperlinks = /*@__PURE__*/ getDefaultExportFromCjs(
  supportsHyperlinksExports,
);
...
function formatPackageWithUrl(pkg, url, limits = 80) {
  return url
    ? terminalLink(pkg, url, {
        fallback: (_, url2) =>
          pkg.length + url2.length > limits ? pkg : pkg + c.dim` - ${url2}`,
      })
    : pkg;
}
...
async function run(fn, args, options = {}) {
  ...
  if (
    ...
    const getV = (a) => {
      ...
      return x(command2, args2, xVersionOptions)
        .then((e) => e.stdout)
...
export {
  CLI_TEMP_DIR as C,
  UnsupportedCommand as U,
  getDefaultAgent as a,
  getGlobalAgent as b,
  getCommand as c,
  detect as d,
  parseNr as e,
  ...
  limitText as q,
  runCli as r,
  serializeCommand as s,
  formatPackageWithUrl as t,
  prompts as u,
  writeFileSafe as w,
};
