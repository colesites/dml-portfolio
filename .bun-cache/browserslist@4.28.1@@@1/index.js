...
function isEolReleased(name) {
  ...
  return browserslist.nodeVersions.some((i) => isVersionsMatch(i, version));
}

function normalize(versions) {
  return versions.filter((version) => typeof version === "string");
}
...
function generateFilter(sign, version) {
  ...
  if (sign === ">") {
    return (v) => parseLatestFloat(v) > version;
  } else if (sign === ">=") {
    return (v) => parseLatestFloat(v) >= version;
  } else if (sign === "<") {
    return (v) => parseFloat(v) < version;
  } else {
    return (v) => parseFloat(v) <= version;
  }
...
function filterByYear(since, context) {
  since = since / 1000;
  return Object.keys(agents).reduce((selected, name) => {
    var data = byName(name, context);
    if (!data) return selected;
    var versions = Object.keys(data.releaseDate).filter((v) => {
...
function normalizeAndroidVersions(androidVersions, chromeVersions) {
  ...
  return androidVersions
    .filter((version) => /^(?:[2-4]\.|[34]$)/.test(version))
...
function resolve(queries, context) {
  return parseQueries(queries).reduce((result, node, index) => {
    if (node.not && index === 0) {
    ...
    var array = type.select.call(browserslist, context, node).map((j) => {
      var parts = j.split(" ");
      if (parts[1] === "0") {
    ...
    if (node.compose === "and") {
      if (node.not) {
        return result.filter((j) => array.indexOf(j) === -1);
      } else {
        return result.filter((j) => array.indexOf(j) !== -1);
      }
    } else {
      if (node.not) {
        ...
        return result.filter((j) => !filter[j]);
      }
...
function browserslist(queries, opts) {
  ...
  var needsPath = parseQueries(queries).some(
    (node) => QUERIES[node.type].needsPath,
  );
  var context = {
  ...
  var result = uniq(resolve(queries, context)).sort((name1, name2) => {
    name1 = name1.split(" ");
    name2 = name2.split(" ");
    if (name1[0] === name2[0]) {
...
browserslist.parse = (queries, opts) => {
  opts = prepareOpts(opts);
  queries = prepareQueries(queries, opts);
  checkQueries(queries);
...
browserslist.coverage = (browsers, stats) => {
  var data;
...
function nodeQuery(context, node) {
  var matched = browserslist.nodeVersions.filter((i) =>
    isVersionsMatch(i, node.version),
  );
  if (matched.length === 0) {
...
function bbmTransform(bbmVersions) {
  ...
  return bbmVersions
    .filter((version) => Object.keys(browsers).indexOf(version.browser) !== -1)
...
function coverQuery(context, node) {
  ...
  var versions = Object.keys(usage).sort((a, b) => usage[b] - usage[a]);
  var covered = 0;
  var result = [];
  var version;
...
var QUERIES = {
  last_major_versions: {
    ...
    select: (context, node) =>
      Object.keys(agents).reduce((selected, name) => {
        var data = byName(name, context);
        if (!data) return selected;
  ...
  last_electron_major_versions: {
    ...
    select: (context, node) => {
      ...
      return validVersions.map((i) => "chrome " + e2c[i]);
    },
  },
  last_node_major_versions: {
    ...
    select: (context, node) =>
      getMajorVersions(browserslist.nodeVersions, node.versions).map(
  ...
  unreleased_versions: {
    ...
    select: (context) =>
      Object.keys(agents).reduce((selected, name) => {
        var data = byName(name, context);
        if (!data) return selected;
        var list = data.versions.filter((v) => data.released.indexOf(v) === -1);
        list = list.map(nameMapper(data.name));
        return selected.concat(list);
      }, []),
  },
  ...
  unreleased_browser_versions: {
    ...
    select: (context, node) => {
      ...
      return data.versions
        .filter((v) => data.released.indexOf(v) === -1)
  ...
  last_years: {
    ...
    select: (context, node) =>
      filterByYear(Date.now() - YEAR * node.years, context),
  },
  since_y: {
    matches: ["year"],
    regexp: /^since (\d+)$/i,
  ...
  popularity: {
    ...
    select: (context, node) => {
      ...
      var usage = browserslist.usage.global;
      return Object.keys(usage).reduce((result, version) => {
        if (node.sign === ">") {
  ...
  firefox_esr: {
    ...
    select: () => ["firefox " + FIREFOX_ESR_VERSION],
  },
  opera_mini_all: {
    matches: [],
  ...
  current_node: {
    ...
    select: (context) => [env.currentNode(resolve, context)],
  },
  maintained_node: {
    matches: [],
  ...
  browserslist_config: {
    ...
    needsPath: true,
    select: (context) => browserslist(undefined, context),
  },
  extends: {
    matches: ["config"],
    regexp: /^extends (.+)$/i,
    needsPath: true,
    select: (context, node) =>
      resolve(env.loadQueries(context, node.config), context),
  },
  defaults: {
    matches: [],
    regexp: /^defaults$/i,
    select: (context) => resolve(browserslist.defaults, context),
  },
  dead: {
    matches: [],
    regexp: /^dead$/i,
...
(() => {
  ...
  browserslist.nodeVersions = jsReleases.map((release) => release.version);
})();
