...
function createResponse(request, body) {
  const responseBodyOrNull = _chunkDOWWQYXZjs.FetchResponse.isResponseWithBody(
    request.status,
  )
    ? body
    : null;
...
var XMLHttpRequestController = class {
      ...
      methodCall: ([methodName, args], invoke) => {
        switch (methodName) {
          ...
          case "send": {
            ...
            const requestBody =
              typeof body === "string"
                ? _chunkLK6DILFKjs.encodeBuffer.call(void 0, body)
                : body;
  ...
  get response() {
    ...
    switch (this.request.responseType) {
      ...
      case "arraybuffer": {
        const arrayBuffer = _chunkLK6DILFKjs.toArrayBuffer.call(
          void 0,
          this.responseBuffer,
        );
  ...
  const XMLHttpRequestProxy = new Proxy(globalThis.XMLHttpRequest, {
    construct(target, args, newTarget) {
      ...
      xhrRequestController.onRequest = async function ({ request, requestId }) {
        const controller = new (0, _chunkDOWWQYXZjs.RequestController)(
          request,
          {
            passthrough: () => {
              this.logger.info(
...
var _XMLHttpRequestInterceptor = class extends _chunkDOWWQYXZjs.Interceptor {
  ...
  checkEnvironment() {
    return _chunkPFGO5BSMjs.hasConfigurableGlobal.call(
      void 0,
      "XMLHttpRequest",
    );
  }
