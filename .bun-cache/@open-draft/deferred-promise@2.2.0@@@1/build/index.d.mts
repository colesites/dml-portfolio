...
declare function createDeferredExecutor<
  Input = never,
  Output = Input,
...
declare class DeferredPromise<Input, Output = Input> extends Promise<Input> {
  ...
  then<ThenResult = Input, CatchResult = never>(
    onFulfilled?: (value: Input) => ThenResult | PromiseLike<ThenResult>,
    onRejected?: (reason: any) => CatchResult | PromiseLike<CatchResult>,
  ): DeferredPromise<ThenResult | CatchResult, Output>;
  catch<CatchResult = never>(
    onRejected?: (reason: any) => CatchResult | PromiseLike<CatchResult>,
  ): DeferredPromise<Input | CatchResult, Output>;
  finally(
    onfinally?: () => void | Promise<any>,
  ): DeferredPromise<Input, Output>;
}
