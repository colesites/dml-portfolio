...
class CodeBlockWriter {
  ...
  quote(text) {
    ...
    this._writeIndentingNewLines(
      text == null
        ? this._quoteChar
        : this._quoteChar +
            escapeForWithinString(text, this._quoteChar) +
            this._quoteChar,
    );
    return this;
  }
  ...
  endsWith(text) {
    ...
    return (
      this.iterateLastCharCodes((charCode, index) => {
    ...
    function writeIndividual(writer, s) {
      if (!writer.isInString()) {
        const isAtStartOfLine =
          writer.isLastNewLine() || writer.getLastChar() == null;
        if (isAtStartOfLine) {
  ...
  _updateInternalState(str) {
    for (let i = 0; i < str.length; i++) {
      ...
      const pastChar =
        i === 0 ? this._getLastCharCodeWithOffset(0) : str.charCodeAt(i - 1);
      const pastPastChar =
        i === 0
      ...
      if (this._isInRegEx) {
        if (
          (pastChar === CHARS.FORWARD_SLASH &&
            pastPastChar !== CHARS.BACK_SLASH) ||
          pastChar === CHARS.NEW_LINE
        ) {
      ...
      if (
        ...
        (currentChar === CHARS.DOUBLE_QUOTE ||
          currentChar === CHARS.SINGLE_QUOTE ||
          currentChar === CHARS.BACK_TICK)
      ) {
