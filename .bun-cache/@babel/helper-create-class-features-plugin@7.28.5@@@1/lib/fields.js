    ...
    if (prop.isPrivate()) {
      ...
      if (prop.isClassPrivateMethod()) {
        if (prop.node.kind === "get") {
          ...
          if (
            body.length === 1 &&
            _core.types.isReturnStatement(($ = body[0])) &&
            _core.types.isCallExpression(($ = $.argument)) &&
            $.arguments.length === 1 &&
            _core.types.isThisExpression($.arguments[0]) &&
          ...
          if (
            body.length === 1 &&
            _core.types.isExpressionStatement(($ = body[0])) &&
            _core.types.isCallExpression(($ = $.expression)) &&
            $.arguments.length === 2 &&
            _core.types.isThisExpression($.arguments[0]) &&
            _core.types.isIdentifier($.arguments[1], {
              name: params[0].name,
            }) &&
            _core.types.isIdentifier(($ = $.callee))
          ) {
    ...
    if (privateFieldsAsProperties) {
      ...
      init = _core.types.newExpression(
        _core.types.identifier(
          isMethod && (!isGetterOrSetter || newHelpers(state))
            ? "WeakSet"
            : "WeakMap",
        ),
        [],
...
function privateNameVisitorFactory(visitor) {
  const nestedVisitor = _traverse.visitors.environmentVisitor(
...
function buildCheckInRHS(rhs, file, inRHSIsObject) {
  if (
    inRHSIsObject ||
    !(file.availableHelper != null && file.availableHelper("checkInRHS"))
  )
    return rhs;
...
const privateNameHandlerSpec = {
  ...
  get: autoInherits(function (member) {
    ...
    const isGetterOrSetter = getId || setId;
    const cloneId = (id) =>
      _core.types.inherits(_core.types.cloneNode(id), privateName);
    if (isStatic) {
      ...
      const skipCheck =
        _core.types.isIdentifier(receiver) && receiver.name === classRef.name;
      if (!isMethod) {
        if (skipCheck) {
      ...
      if (getId) {
        ...
        return _core.types.callExpression(
          file.addHelper("classPrivateGetter"),
          [_core.types.cloneNode(classRef), receiver, cloneId(getId)],
        );
      }
      if (setId) {
      ...
      if (isGetterOrSetter) {
        ...
        if (!newHelpers(file)) {
          return _core.types.callExpression(
            ...
            [this.receiver(member), cloneId(id)],
          );
        }
        return _core.types.callExpression(
          file.addHelper("classPrivateGetter"),
          [_core.types.cloneNode(id), this.receiver(member), cloneId(getId)],
        );
      }
      if (!newHelpers(file)) {
        return _core.types.callExpression(
          file.addHelper("classPrivateMethodGet"),
          [this.receiver(member), _core.types.cloneNode(id), cloneId(methodId)],
        );
      }
      return _core.types.callExpression(file.addHelper("assertClassBrand"), [
    ...
    if (newHelpers(file)) {
      return _core.types.callExpression(
        ...
        [cloneId(id), this.receiver(member)],
      );
    }
  ...
  set: autoInherits(function (member, value) {
    ...
    if (isStatic) {
      ...
      if (setId) {
        ...
        return _core.types.callExpression(
          file.addHelper("classPrivateSetter"),
          [_core.types.cloneNode(classRef), cloneId(setId), receiver, value],
        );
      }
      ...
      if (setId) {
        if (!newHelpers(file)) {
          return _core.types.callExpression(
            ...
            [this.receiver(member), cloneId(id), value],
          );
        }
        return _core.types.callExpression(
          file.addHelper("classPrivateSetter"),
    ...
    if (newHelpers(file)) {
      return _core.types.callExpression(
        ...
        [cloneId(id), this.receiver(member), value],
      );
    }
  ...
  destructureSet(member) {
    ...
    if (isStatic && !isMethod) {
      ...
      if (
        !noUninitializedPrivateFieldAccess ||
        !_core.types.isCallExpression(getCall)
      ) {
        return getCall;
      }
      ...
      return _core.types.memberExpression(
        _core.types.callExpression(file.addHelper("toSetter"), [getCall]),
        _core.types.identifier("_"),
      );
    }
    const setCall = this.set(member, _core.types.identifier("_"));
    if (
      !_core.types.isCallExpression(setCall) ||
      !_core.types.isIdentifier(
        setCall.arguments[setCall.arguments.length - 1],
        {
          name: "_",
        },
      )
    ...
    let args;
    if (
      _core.types.isMemberExpression(setCall.callee, {
        computed: false,
      }) &&
      _core.types.isIdentifier(setCall.callee.property) &&
    ...
    return _core.types.memberExpression(
      _core.types.callExpression(file.addHelper("toSetter"), args),
      _core.types.identifier("_"),
    );
  },
  call(member, args) {
  ...
  return inheritPropComments(
    _core.types.variableDeclaration("var", [
      _core.types.variableDeclarator(
        _core.types.cloneNode(privateName.id),
        value,
      ),
    ]),
    prop,
...
var buildPrivateStaticFieldInitSpecOld = (prop, privateNamesMap) => {
  const privateName = privateNamesMap.get(prop.node.key.id.name);
...
function buildPrivateMethodDeclaration(
  ...
  return inheritPropComments(
    _core.types.functionDeclaration(
...
function replaceThisContext(path, ref, innerBindingRef) {
  ...
  if (
    ...
    (_state$thisRef = state.thisRef) != null &&
    _state$thisRef.name &&
...
function buildFieldsInitNodes(
  ...
  const classRefForInnerBinding =
    ref != null
      ? ref
      : props[0].scope.generateUidIdentifier(
          (innerBindingRef == null ? void 0 : innerBindingRef.name) || "Class",
        );
  ...
  for (const prop of props) {
    ...
    switch (true) {
      case isStaticBlock: {
        ...
        if (
          blockBody.length === 1 &&
          _core.types.isExpressionStatement(blockBody[0])
        ) {
      ...
      case isStatic && isPublic && isField && !setPublicClassFields:
        staticNodes.push(
          buildPublicFieldInitSpec(_core.types.cloneNode(ref), prop, file),
        );
        break;
      case isInstance &&
        isPrivate &&
      ...
      case isInstance && isPublic && isField && !setPublicClassFields:
        ...
        instanceNodes.push(
          buildPublicFieldInitSpec(_core.types.thisExpression(), prop, file),
        );
        break;
      default:
  ...
  if (classRefFlags & 2 && innerBindingRef != null) {
    classBindingNode = _core.types.expressionStatement(
      _core.types.assignmentExpression(
        "=",
        _core.types.cloneNode(classRefForInnerBinding),
        _core.types.cloneNode(innerBindingRef),
      ),
    );
  }
  return {
