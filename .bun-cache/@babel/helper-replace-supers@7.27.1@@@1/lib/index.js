...
const specHandlers = {
  ...
  _call(superMember, args, optional) {
    ...
    if (
      args.length === 1 &&
      _core.types.isSpreadElement(args[0]) &&
      (_core.types.isIdentifier(args[0].argument) ||
        _core.types.isArrayExpression(args[0].argument))
    ) {
    ...
    const call = _core.types.callExpression(
      this.file.addHelper("superPropGet"),
      [
        this.isDerivedConstructor
          ? sequenceExpression([thisExpression(), objectRef])
          : objectRef,
        ...
        _core.types.numericLiteral(
          2 | (this.isStatic || this.isPrivateMethod ? 0 : 1),
        ),
      ],
    );
    if (optional) {
