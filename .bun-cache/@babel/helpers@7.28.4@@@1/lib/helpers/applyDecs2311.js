...
function applyDecs2311(
  ...
  function applyDec(
    ...
    for (var i = decs.length - 1; i >= 0; i -= decoratorsHaveThis ? 2 : 1) {
      ...
      var ctx = {
        ...
        name: name,
        metadata: metadata,
        addInitializer: ((decoratorFinishedRef, initializer) => {
      ...
      if (isClass) {
        ...
        _ = ctx.access = {
          has: isPrivate ? hasPrivateBrand.bind() : (target) => name in target,
        };
        if (!isSetter) {
          _.get = isPrivate
            ? isMethod
  ...
  function applyMemberDecs() {
    ...
    var pushInitializers = (initializers) => {
      if (initializers) {
        ret.push(createRunInitializers(initializers));
      }
    };
    var applyMemberDecsOfKind = (isStatic, isField) => {
      for (var i = 0; i < memberDecs.length; i++) {
        ...
        if ((kind & 8) == isStatic && !kindOnly == isField) {
          ...
          applyDec(
            ...
            isField,
            isStatic && isPrivate
              ? (_) => (0, _checkInRHS.default)(_) === targetClass
              : instanceBrand,
          );
        }
      }
