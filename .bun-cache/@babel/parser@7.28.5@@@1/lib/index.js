...
const TSErrors = ParseErrorEnum`typescript`({
  ...
  InvalidModifierOnAwaitUsingDeclaration: (modifier) =>
  ...
  InvalidModifierOnUsingDeclaration: (modifier) =>
  ...
  UsingDeclarationInAmbientContext: (kind) =>
    ...
    fillOptionalPropertiesForTSESLint(node) {
      ...
      switch (node.type) {
        ...
        case "TSInterfaceDeclaration":
          ...
          return;
        case "TSMappedType":
          (_node$optional5 = node.optional) != null
            ? _node$optional5
            : (node.optional = false);
          (_node$readonly4 = node.readonly) != null
            ? _node$readonly4
...
var placeholders = (superClass) =>
  class PlaceholdersParserMixin extends superClass {
    ...
    chStartsBindingIdentifier(ch, pos) {
      ...
      const next = this.nextTokenStart();
      if (
        this.input.charCodeAt(next) === 37 &&
        this.input.charCodeAt(next + 1) === 37
      ) {
        return true;
      }
      return false;
...
class ExpressionParser extends LValParser {
  ...
  parseObjectProperty(prop, startLoc, isPattern, refExpressionErrors) {
    ...
    if (this.eat(14)) {
      prop.value = isPattern
        ...
        : this.parseMaybeAssignAllowInOrVoidPattern(8, refExpressionErrors);
  ...
  parseMethod(
    ...
    this.scope.enter(
      514 | 16 | (inClassScope ? 576 : 0) | (allowDirectSuper ? 32 : 0),
    );
  ...
  parseMaybeAssignAllowInOrVoidPattern(
    ...
    if (refExpressionErrors != null && this.match(88)) {
      ...
      if (
        nextCode === 44 ||
        nextCode === (close === 3 ? 93 : close === 8 ? 125 : 41) ||
        nextCode === 61
      ) {
