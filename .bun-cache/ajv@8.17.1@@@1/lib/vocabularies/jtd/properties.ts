import { _, and, type Code, type Name, not } from "../../compile/codegen";
...
import { type _JTDTypeError, typeErrorMessage, typeErrorParams } from "./error";
...
export type JTDPropertiesError =
  ...
  | ErrorObject<
      PropKeyword,
      { error: PropError.Additional; additionalProperty: string },
      PropSchema
    >
  | ErrorObject<
      PropKeyword,
      { error: PropError.Missing; missingProperty: string },
      PropSchema
    >;
...
export function validateProperties(cxt: KeywordCxt): void {
  ...
  function commonProperties(): boolean {
    ...
    const optProps = parentSchema.optionalProperties as
      | Record<string, any>
      | undefined;
    if (!(props && optProps)) return false;
    for (const p in props) {
      if (Object.hasOwn(optProps, p)) return true;
    }
    return false;
  }
  ...
  function isAdditional(
    ...
    if (props.length > 8) {
      ...
      const ps =
        jtdDiscriminator === undefined
          ? props
          : [jtdDiscriminator].concat(props);
