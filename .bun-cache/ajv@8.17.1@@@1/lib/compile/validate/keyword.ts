...
type KeywordCompilationResult =
  | AnySchema
  | SchemaValidateFunction
  | AnyValidateFunction;

export function macroKeywordCode(
  cxt: KeywordCxt,
  def: MacroKeywordDefinition,
): void {
...
export function funcKeywordCode(
  cxt: KeywordCxt,
  ...
  gen.if(
    ...
    () => {
      gen
        .assign(
          N.vErrors,
          _`${N.vErrors} === null ? ${errs} : ${N.vErrors}.concat(${errs})`,
        )
...
function useKeyword(
  gen: CodeGen,
  keyword: string,
  result?: KeywordCompilationResult,
): Name {
  if (result === undefined)
...
export function validateKeywordUsage(
  ...
  if (deps?.some((kwd) => !Object.hasOwn(schema, kwd))) {
    throw new Error(
