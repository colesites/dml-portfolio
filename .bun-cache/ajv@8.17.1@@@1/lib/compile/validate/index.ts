...
function validateFunction(
  ...
  if (opts.code.es5) {
    ...
    gen.func(
      validateName,
      _`${N.data}, ${destructureValCxt(opts)}`,
      schemaEnv.$async,
...
function destructureValCxtES5(gen: CodeGen, opts: InstanceOptions): void {
  gen.if(
    ...
    () => {
      ...
      if (opts.dynamicRef)
        gen.var(N.dynamicAnchors, _`${N.valCxt}.${N.dynamicAnchors}`);
    },
    () => {
...
function commentKeyword({
  ...
  if (opts.$comment === true) {
    ...
    gen.code(
      _`${N.self}.opts.$comment(${msg}, ${schemaPath}, ${rootName}.schema)`,
    );
  }
}
...
function schemaKeywords(
  ...
  if (
    ...
    gen.block(() =>
      keywordCode(it, "$ref", (RULES.all.$ref as Rule).definition),
    ); // TODO typecast
    return;
  }
