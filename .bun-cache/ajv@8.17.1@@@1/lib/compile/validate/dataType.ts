...
import { _, and, type Code, type Name, nil, not, operators } from "../codegen";
import { reportError } from "../errors";
import { isJSONType, type JSONType } from "../rules";
...
export function coerceAndCheckDataType(
  it: SchemaObjCxt,
  types: JSONType[],
): boolean {
...
function assignParentData(
  { gen, parentData, parentDataProperty }: SchemaObjCxt,
  expr: Name,
): void {
...
export function checkDataTypes(
  ...
  for (const t in types)
    cond = and(cond, checkDataType(t as JSONType, data, strictNums, correct));
  return cond;
}
