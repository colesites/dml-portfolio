...
export default function compileSerializer(
  ...
  try {
    ...
    const serialize: (data: unknown) => string = makeSerialize(
      this.scope.get(),
    );
...
function serializeNullable(
  cxt: SerializeCxt,
  serializeForm: (_cxt: SerializeCxt) => void,
): void {
...
function serializeSchemaProperties(
  ...
  for (const key of optProps) {
    ...
    gen.if(
      and(_`${value} !== undefined`, isOwnProperty(gen, data, key)),
      () => {
