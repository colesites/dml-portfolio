...
import { type IntType, intRange } from "../../vocabularies/jtd/type";
...
import { type JTDForm, jtdForms, type SchemaObjectMap } from "./types";
...
function tryParseItems(
  ...
  const { gen } = cxt;
  gen.for(
    _`;${N.jsonPos}<${N.jsonLen} && ${jsonSlice(1)}!==${endToken};`,
    () => {
      block();
  ...
  if (properties) {
    ...
    const allProps: Code = and(
      ...Object.keys(properties).map(
        (p): Code => _`${hasProp}.call(${data}, ${p})`,
      ),
    );
    gen.if(not(allProps), () =>
      parsingError(cxt, str`missing required properties`),
    );
  }
}

function parseDefinedProperty(
  cxt: ParseCxt,
  key: Name,
  ...
  switch (schema.type) {
    ...
    case "timestamp": {
      ...
      const fail: Code = parseDate
        ? or(
            notValid,
            _`(${data} = new Date(${data}), false)`,
            _`isNaN(${data}.valueOf())`,
          )
        : notValid;
...
function parseRef(cxt: ParseCxt): void {
  ...
  const sch = compileParser.call(
    self,
    new SchemaEnv({ schema: refSchema, root }),
    definitions,
  );
...
function partialParse(cxt: ParseCxt, parseFunc: Name, args?: SafeExpr): void {
  ...
  gen.if(_`${data} === undefined`, () =>
    parsingError(cxt, _`${parseFunc}.message`),
  );
}
