...
export {
  ...
  JTDSchemaType,
  SomeJTDSchemaType,
} from "./types/jtd-schema";
export { DefinedError } from "./vocabularies/errors";

import { compileSchema, resolveSchema, SchemaEnv } from "./compile";
import { type Code, ValueScope } from "./compile/codegen";
import MissingRefError from "./compile/ref_error";
import { getSchemaRefs, normalizeId } from "./compile/resolve";
import {
  getRules,
  type JSONType,
  type Rule,
...
export default class Ajv {
  ...
  compile<T = unknown>(
    ...
    return (sch.validate ||
      this._compileSchemaEnv(sch)) as AnyValidateFunction<T>;
  }
  ...
  compileAsync<T = unknown>(
    ...
    function checkLoaded(
      this: Ajv,
      { missingSchema: ref, missingRef }: MissingRefError,
    ): void {
      if (this.refs[ref]) {
        throw new Error(
  ...
  getSchema<T = unknown>(keyRef: string): AnyValidateFunction<T> | undefined {
    ...
    return (sch.validate || this._compileSchemaEnv(sch)) as
      | AnyValidateFunction<T>
      | undefined;
  }
  ...
  private _removeAllSchemas(
    schemas: { [Ref in string]?: SchemaEnv | string },
    regex?: RegExp,
  ): void {
