...
import type { SchemaCxt, SchemaEnv, SchemaObjCxt } from "../compile";
import type {
  Code,
  CodeGen,
  Name,
...
export interface AsyncValidateFunction<T = unknown>
  extends ValidateFunction<T> {
  (...args: Parameters<ValidateFunction<T>>): Promise<T>;
  $async: true;
}

export type AnyValidateFunction<T = any> =
  | ValidateFunction<T>
...
export type ErrorNoParams<K extends string, S = unknown> = ErrorObject<
  K,
  Record<string, never>,
  S
>;
...
export type FormatCompare<T extends string | number> = (
  data1: T,
  data2: T,
) => number | undefined;

export type AsyncFormatValidator<T extends string | number> = (
  data: T,
) => Promise<boolean>;
