...
test("stringify()", (t) => {
  ...
  t.test("stringifies bigints", { skip: !hasBigInt }, (st) => {
    var three = BigInt(3);
    var encodeWithN = (value, defaultEncoder, charset) => {
  ...
  t.test(
    "encodes dot in key of object when encodeDotInKeys and allowDots is provided",
    (st) => {
      st.equal(
        qs.stringify(
  ...
  t.test(
    "should encode dot in key of object, and automatically set allowDots to `true` when encodeDotInKeys is true and allowDots in undefined",
    (st) => {
      st.equal(
        qs.stringify(
          { "name.obj.subobject": { "first.godly.name": "John", last: "Doe" } },
  ...
  t.test(
    "should encode dot in key of object when encodeDotInKeys and allowDots is provided, and nothing else when encodeValuesOnly is provided",
    (st) => {
      st.equal(
        qs.stringify(
          { "name.obj": { first: "John", last: "Doe" } },
  ...
  t.test("throws when `commaRoundTrip` is not a boolean", (st) => {
    st["throws"](
      () => {
        qs.stringify({}, { commaRoundTrip: "not a boolean" });
      },
      TypeError,
  ...
  t.test("throws when `encodeDotInKeys` is not a boolean", (st) => {
    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { encodeDotInKeys: "foobar" });
    }, TypeError);

    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { encodeDotInKeys: 0 });
    }, TypeError);

    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { encodeDotInKeys: NaN });
    }, TypeError);

    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { encodeDotInKeys: null });
    }, TypeError);
  ...
  t.test(
    "`allowDots` option: stringifies a nested object with dots notation",
    (st) => {
  ...
  t.test("omits object key/value pair when value is empty array", (st) => {
  ...
  t.test(
    "should not omit object key/value pair when value is empty array and when asked",
    (st) => {
      st.equal(qs.stringify({ a: [], b: "zz" }), "b=zz");
      st.equal(
        qs.stringify({ a: [], b: "zz" }, { allowEmptyArrays: false }),
        "b=zz",
      );
      st.equal(
        qs.stringify({ a: [], b: "zz" }, { allowEmptyArrays: true }),
        "a[]&b=zz",
      );
  ...
  t.test("should throw when allowEmptyArrays is not of type boolean", (st) => {
    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { allowEmptyArrays: "foobar" });
    }, TypeError);

    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { allowEmptyArrays: 0 });
    }, TypeError);

    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { allowEmptyArrays: NaN });
    }, TypeError);

    st["throws"](() => {
      qs.stringify({ a: [], b: "zz" }, { allowEmptyArrays: null });
    }, TypeError);
  ...
  t.test("stringifies an array value with one item vs multiple items", (st) => {
    ...
    st.test("array with a single item", (s2t) => {
      ...
      s2t.equal(
        qs.stringify({ a: ["c"] }, { encodeValuesOnly: true }),
        "a[0]=c",
      );
    ...
    st.test("array with multiple items", (s2t) => {
      ...
      s2t.equal(
        qs.stringify({ a: ["c", "d"] }, { encodeValuesOnly: true }),
        "a[0]=c&a[1]=d",
      );
    ...
    st.test("array with multiple items with a comma inside", (s2t) => {
      s2t.equal(
        qs.stringify(
          { a: ["c,d", "e"] },
          { encodeValuesOnly: true, arrayFormat: "comma" },
      ...
      s2t.equal(
        qs.stringify({ a: ["c,d", "e"] }, { arrayFormat: "comma" }),
        "a=c%2Cd%2Ce",
      );
  ...
  t.test("stringifies comma and empty array values", (st) => {
    st.equal(
      qs.stringify(
        { a: [",", "", "c,d%"] },
  ...
  t.test("stringifies comma and empty non-array values", (st) => {
    st.equal(
      qs.stringify(
        { a: ",", b: "", c: "c,d%" },
        { encode: false, arrayFormat: "indices" },
  ...
  t.test("uses indices notation for arrays when arrayFormat=indices", (st) => {
    st.equal(
  ...
  t.test("uses repeat notation for arrays when arrayFormat=repeat", (st) => {
    st.equal(
  ...
  t.test(
    "uses brackets notation for arrays when arrayFormat=brackets",
    (st) => {
      st.equal(
  ...
  t.test("stringifies an empty array in different arrayFormat", (st) => {
    st.equal(
      qs.stringify({ a: [], b: [null], c: "c" }, { encode: false }),
      "b[0]=&c=c",
    );
    // arrayFormat default
    st.equal(
  ...
  t.test("stringifies a null object", { skip: !hasProto }, (st) => {
  ...
  t.test("RFC 1738 serialization", (st) => {
    ...
    st.equal(
      qs.stringify({ "foo(ref)": "bar" }, { format: qs.formats.RFC1738 }),
      "foo(ref)=bar",
    );
  ...
  t.test("`charsetSentinel` option", (st) => {
    st.equal(
  ...
  t.test("allows for encoding keys and values differently", (st) => {
    var encoder = (str, defaultEncoder, charset, type) => {
      if (type === "key") {
    ...
    st.deepEqual(
      qs.stringify({ KeY: "vAlUe" }, { encoder: encoder }),
      "key=VALUE",
    );
    st.end();
  });
  ...
  t.test("objects inside arrays", (st) => {
    ...
    st.equal(
      qs.stringify(obj, { encode: false, arrayFormat: "brackets" }),
      "a[b][c]=d&a[b][e]=f",
      "no array, bracket",
    );
    st.equal(
      qs.stringify(obj, { encode: false, arrayFormat: "indices" }),
      "a[b][c]=d&a[b][e]=f",
      "no array, indices",
    );
    st.equal(
      qs.stringify(obj, { encode: false, arrayFormat: "repeat" }),
      "a[b][c]=d&a[b][e]=f",
      "no array, repeat",
    );
    st.equal(
      qs.stringify(obj, { encode: false, arrayFormat: "comma" }),
      "a[b][c]=d&a[b][e]=f",
      "no array, comma",
    );

    st.equal(
      qs.stringify(withArray, { encode: false }),
      "a[b][0][c]=d&a[b][0][e]=f",
      "array, no arrayFormat",
    );
    st.equal(
      qs.stringify(withArray, { encode: false, arrayFormat: "brackets" }),
      "a[b][][c]=d&a[b][][e]=f",
      "array, bracket",
    );
    st.equal(
      qs.stringify(withArray, { encode: false, arrayFormat: "indices" }),
      "a[b][0][c]=d&a[b][0][e]=f",
      "array, indices",
    );
    st.equal(
      qs.stringify(withArray, { encode: false, arrayFormat: "repeat" }),
      "a[b][c]=d&a[b][e]=f",
      "array, repeat",
    );
    st.equal(
  ...
  t.test("encodes a very long string", (st) => {
    var chars = [];
    var expected = [];
    for (var i = 0; i < 5e3; i++) {
      chars.push(" " + i);
...
test("stringifies empty keys", (t) => {
  emptyTestCases.forEach((testCase) => {
    t.test(
      "stringifies an object with empty string key with " + testCase.input,
      (st) => {
        st.deepEqual(
          qs.stringify(testCase.withEmptyKeys, {
            encode: false,
            arrayFormat: "indices",
          }),
  ...
  t.test("edge case with object/arrays", (st) => {
    st.deepEqual(
      qs.stringify({ "": { "": [2, 3] } }, { encode: false }),
      "[][0]=2&[][1]=3",
    );
    st.deepEqual(
      qs.stringify({ "": { "": [2, 3], a: 2 } }, { encode: false }),
      "[][0]=2&[][1]=3&[a]=2",
    );
    st.deepEqual(
      qs.stringify(
  ...
  t.test("stringifies non-string keys", (st) => {
    var S = Object("abc");
    S.toString = () => "d";
    var actual = qs.stringify(
      { a: "b", false: {}, 1e22: "c", d: "e" },
      {
