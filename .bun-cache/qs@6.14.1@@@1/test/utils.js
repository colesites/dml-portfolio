...
test("merge()", (t) => {
  ...
  t.test("with overflow objects (from arrayLimit)", (st) => {
    st.test("merges primitive into overflow object at next index", (s2t) => {
      ...
      s2t.deepEqual(
        merged,
        { 0: "a", 1: "b", 2: "c" },
        "adds primitive at next numeric index",
      );
      s2t.end();
    });

    st.test(
      "merges primitive into regular object with numeric keys normally",
      (s2t) => {
        var obj = { 0: "a", 1: "b" };
        s2t.notOk(
          utils.isOverflow(obj),
          "plain object is not marked as overflow",
        );
    ...
    st.test(
      "merges primitive into object with non-numeric keys normally",
      (s2t) => {
        var obj = { foo: "bar" };
        ...
        s2t.deepEqual(
          merged,
          { foo: "bar", baz: true },
          "adds primitive as key with value true",
        );
        s2t.end();
      },
    );

    st.test("merges overflow object into primitive", (s2t) => {
      // Create an overflow object via combine
    ...
    st.test(
      "merges overflow object with multiple values into primitive",
      (s2t) => {
        // Create an overflow object via combine
        ...
        s2t.deepEqual(
          merged,
          { 0: "a", 1: "b", 2: "c" },
          "shifts all source indices by 1",
        );
        s2t.end();
      },
    );

    st.test("merges regular object into primitive as array", (s2t) => {
      var obj = { foo: "bar" };
      var merged = utils.merge("a", obj);
      s2t.deepEqual(
        merged,
        ["a", { foo: "bar" }],
        "creates array with primitive and object",
      );
      s2t.end();
    });
...
test("combine()", (t) => {
  ...
  t.test("with arrayLimit", (st) => {
    st.test("under the limit", (s2t) => {
      var combined = utils.combine(["a", "b"], "c", 10, false);
      s2t.deepEqual(
        combined,
        ["a", "b", "c"],
        "returns array when under limit",
      );
      s2t.ok(Array.isArray(combined), "result is an array");
      s2t.end();
    });

    st.test("exactly at the limit stays as array", (s2t) => {
      var combined = utils.combine(["a", "b"], "c", 3, false);
      s2t.deepEqual(
        combined,
        ["a", "b", "c"],
        "stays as array when exactly at limit",
      );
      s2t.ok(Array.isArray(combined), "result is an array");
      s2t.end();
    });

    st.test("over the limit", (s2t) => {
    ...
    st.test("with arrayLimit 0", (s2t) => {
      var combined = utils.combine([], "a", 0, false);
      s2t.deepEqual(
        combined,
        { 0: "a" },
        "converts single element to object with arrayLimit 0",
      );
      s2t.notOk(Array.isArray(combined), "result is not an array");
      s2t.end();
    });

    st.test("with plainObjects option", (s2t) => {
      var combined = utils.combine(["a"], "b", 1, true);
      var expected = { __proto__: null, 0: "a", 1: "b" };
      s2t.deepEqual(
        combined,
        expected,
        "converts to object with null prototype",
      );
      s2t.equal(
  ...
  t.test("with existing overflow object", (st) => {
    st.test("adds to existing overflow object at next index", (s2t) => {
      // Create overflow object first via combine
      var overflow = utils.combine(["a"], "b", 1, false);
      s2t.ok(
        utils.isOverflow(overflow),
        "initial object is marked as overflow",
      );
      ...
      s2t.deepEqual(
        combined,
        { 0: "a", 1: "b", 2: "c" },
        "adds value at next numeric index",
      );
      s2t.end();
    });

    st.test(
      "does not treat plain object with numeric keys as overflow",
      (s2t) => {
        var plainObj = { 0: "a", 1: "b" };
        s2t.notOk(
          utils.isOverflow(plainObj),
          "plain object is not marked as overflow",
        );
        ...
        s2t.deepEqual(
          combined,
          [{ 0: "a", 1: "b" }, "c"],
          "concatenates as regular values",
        );
        s2t.end();
      },
    );
...
test("decode", (t) => {
  t.equal(utils.decode("a+b"), "a b", "decodes + to space");

  t.equal(utils.decode("name%2Eobj"), "name.obj", "decodes a string");
  t.equal(
...
test("encode", (t) => {
  forEach(v.nullPrimitives, (nullish) => {
    t["throws"](
      () => {
        utils.encode(nullish);
      },
      TypeError,
  ...
  t.deepEqual(
    utils.encode({ length: 0 }),
    { length: 0 },
    "empty arraylike returns itself",
  );
  ...
  t.equal(utils.encode("(abc)"), "%28abc%29", "encodes parentheses");
  t.equal(
    utils.encode({ toString: () => "(abc)" }),
    "%28abc%29",
  ...
  t.equal(utils.encode(longString), expectedString, "encodes a long string");

  t.equal(utils.encode("\x28\x29"), "%28%29", "encodes parens normally");
  t.equal(
...
test("isBuffer()", (t) => {
  ...
  var buffer =
    Buffer.from && Buffer.alloc ? Buffer.from("abc") : new Buffer("abc");
...
test("isRegExp()", (t) => {
  ...
  t.equal(utils.isRegExp(/a/g), true, "new RegExp is a RegExp");
  t.equal(utils.isRegExp(new Date()), false, "Date is not a RegExp");

  forEach(v.primitives, (primitive) => {
    t.equal(
      utils.isRegExp(primitive),
      false,
      inspect(primitive) + " is not a RegExp",
    );
  });
