...
function detectLockfile() {
  const packageDir = escalade(".", (dir, names) => {
  ...
  const lockfileNpm = join(packageDir, "package-lock.json");
  const lockfileShrinkwrap = join(packageDir, "npm-shrinkwrap.json");
  const lockfileYarn = join(packageDir, "yarn.lock");
  const lockfilePnpm = join(packageDir, "pnpm-lock.yaml");
  const lockfileBun = join(packageDir, "bun.lock");
  const lockfileBunBinary = join(packageDir, "bun.lockb");
  ...
  if (existsSync(lockfilePnpm)) {
    ...
    const lock = { file: lockfileYarn, mode: "yarn" };
...
function diffBrowsers(old, current) {
  const browsers = Object.keys(old).concat(
    Object.keys(current).filter((browser) => old[browser] === undefined),
  );
  return browsers
    .map((browser) => {
      const oldVersions = old[browser] || [];
      const currentVersions = current[browser] || [];
      const common = oldVersions.filter((v) => currentVersions.includes(v));
      const added = currentVersions.filter((v) => !common.includes(v));
      const removed = oldVersions.filter((v) => !common.includes(v));
      return removed
...
function updateNpmLockfile(lock, latest) {
  const metadata = { latest, versions: [] };
  const content = deletePackage(JSON.parse(lock.content), metadata);
...
const yarnVersionRe = /version "(.*?)"/;

function updateYarnLockfile(lock, latest) {
  const blocks = lock.content.split(/(\n{2,})/).map((block) => {
    return block.split("\n");
  });
  const versions = {};
  blocks.forEach((lines) => {
    if (lines[0].indexOf("caniuse-lite@") !== -1) {
      const match = yarnVersionRe.exec(lines[1]);
  ...
  const content = blocks.map((lines) => lines.join("\n")).join("");
...
function updatePackageManually(print, lock, latest) {
  const lockfileData = updateLockfile(lock, latest);
  const caniuseVersions = Object.keys(lockfileData.versions).sort();
...
module.exports = function updateDB(print = defaultPrint) {
  ...
  const latest = getLatestInfo(lock);
  ...
  if (lock.mode === "yarn" && lock.version !== 1) {
    ...
    const lockContent = readFileSync(lock.file).toString();
    const packages = lockContent.includes("baseline-browser-mapping")
  ...
  if (listError) {
    ...
    const changes = diffBrowsers(oldList, newList);
    if (changes) {
