...
test("bad indent options", (t) => {
  forEach(
    [undefined, true, false, -1, 1.2, Infinity, -Infinity, NaN],
    (indent) => {
      t["throws"](
        () => {
...
test("Map", { skip: typeof Map !== "function" }, (t) => {
  ...
  t.equal(
    inspect(new Map(), { indent: 2 }),
    "Map (0) {}",
    "empty Map should show as empty (two)",
  );
  t.equal(
    inspect(new Map(), { indent: "\t" }),
    "Map (0) {}",
  ...
  t.equal(
    inspect(nestedMap, { indent: 2 }),
    expectedNestedSpaces,
    "Map containing a Map should work (two)",
  );
  t.equal(
    inspect(nestedMap, { indent: "\t" }),
    expectedNestedTabs,
...
test("Set", { skip: typeof Set !== "function" }, (t) => {
  ...
  t.equal(
    inspect(new Set(), { indent: 2 }),
    "Set (0) {}",
    "empty Set should show as empty (two)",
  );
  t.equal(
    inspect(new Set(), { indent: "\t" }),
    "Set (0) {}",
  ...
  t.equal(
    inspect(nestedSet, { indent: 2 }),
    expectedNestedSpaces,
    "Set containing a Set should work (two)",
  );
  t.equal(
    inspect(nestedSet, { indent: "\t" }),
    expectedNestedTabs,
