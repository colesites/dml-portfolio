...
function applyElicitationDefaults(schema, data) {
  ...
  if (
    ...
    for (const key of Object.keys(props)) {
      ...
      if (obj[key] === undefined && Object.hasOwn(propSchema, "default")) {
...
class Client extends protocol_js_1.Protocol {
  ...
  constructor(_clientInfo, options) {
    ...
    this._jsonSchemaValidator =
      ...
      new ajv_provider_js_1.AjvJsonSchemaValidator();
  ...
  registerCapabilities(capabilities) {
    ...
    this._capabilities = (0, protocol_js_1.mergeCapabilities)(
      this._capabilities,
      capabilities,
    );
  }
  ...
  setRequestHandler(requestSchema, handler) {
    ...
    if (method === "elicitation/create") {
      const wrappedHandler = async (request, extra) => {
        const validatedRequest = (0, zod_compat_js_1.safeParse)(
          types_js_1.ElicitRequestSchema,
          request,
        );
        ...
        if (params.task) {
          const taskValidationResult = (0, zod_compat_js_1.safeParse)(
        ...
        const requestedSchema =
          params.mode === "form" ? params.requestedSchema : undefined;
        if (
          params.mode === "form" &&
    ...
    if (method === "sampling/createMessage") {
      const wrappedHandler = async (request, extra) => {
        const validatedRequest = (0, zod_compat_js_1.safeParse)(
          types_js_1.CreateMessageRequestSchema,
          request,
        );
        if (!validatedRequest.success) {
          const errorMessage =
  ...
  async connect(transport, options) {
    ...
    try {
      ...
      if (
        !types_js_1.SUPPORTED_PROTOCOL_VERSIONS.includes(result.protocolVersion)
      ) {
        throw new Error(
  ...
  assertTaskCapability(method) {
    (0, helpers_js_1.assertToolsCallTaskCapability)(
  ...
  assertTaskHandlerCapability(method) {
    ...
    (0, helpers_js_1.assertClientRequestTaskCapability)(
  ...
  async complete(params, options) {
    return this.request(
      ...
      types_js_1.CompleteResultSchema,
      options,
    );
  }
  ...
  async callTool(
    params,
    resultSchema = types_js_1.CallToolResultSchema,
    options,
  ) {
  ...
  _setupListChangedHandler(listType, notificationSchema, options, fetcher) {
    ...
    const parseResult =
      types_js_1.ListChangedOptionsBaseSchema.safeParse(options);
