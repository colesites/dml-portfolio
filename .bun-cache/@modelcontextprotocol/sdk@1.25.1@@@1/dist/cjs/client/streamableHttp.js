...
class StreamableHTTPClientTransport {
  constructor(url, opts) {
    ...
    this._fetchWithInit = (0, transport_js_1.createFetchWithInit)(
      opts?.fetch,
      opts?.requestInit,
    );
  ...
  async _commonHeaders() {
    ...
    const extraHeaders = (0, transport_js_1.normalizeHeaders)(
    ...
    const processStream = async () => {
      ...
      try {
        ...
        const reader = stream
          ...
          .pipeThrough(
            new stream_1.EventSourceParserStream({
              onRetry: (retryMs) => {
        ...
        while (true) {
          ...
          if (!event.event || event.event === "message") {
            try {
              const message = types_js_1.JSONRPCMessageSchema.parse(
    ...
    try {
      ...
      if (resumptionToken) {
        ...
        this._startOrAuthSse({
          resumptionToken,
          replayMessageId: (0, types_js_1.isJSONRPCRequest)(message)
            ? message.id
            : undefined,
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && this._authProvider) {
          ...
          const { resourceMetadataUrl, scope } = (0,
          auth_js_1.extractWWWAuthenticateParams)(response);
        ...
        if (response.status === 403 && this._authProvider) {
          const { resourceMetadataUrl, scope, error } = (0,
          auth_js_1.extractWWWAuthenticateParams)(response);
