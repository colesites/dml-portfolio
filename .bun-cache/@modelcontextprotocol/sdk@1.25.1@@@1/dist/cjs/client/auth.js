...
async function parseErrorResponse(input) {
  ...
  try {
    ...
    const errorClass =
      errors_js_1.OAUTH_ERRORS[error] || errors_js_1.ServerError;
...
async function auth(provider, options) {
  try {
    ...
    if (
      error instanceof errors_js_1.InvalidClientError ||
      error instanceof errors_js_1.UnauthorizedClientError
    ) {
...
async function authInternal(
  provider,
  { serverUrl, authorizationCode, scope, resourceMetadataUrl, fetchFn },
) {
  ...
  if (!clientInformation) {
    ...
    if (clientMetadataUrl && !isHttpsUrl(clientMetadataUrl)) {
      throw new errors_js_1.InvalidClientMetadataError(
  ...
  if (tokens?.refresh_token) {
    try {
      ...
      if (
        !(error instanceof errors_js_1.OAuthError) ||
...
async function selectResourceURL(serverUrl, provider, resourceMetadata) {
  const defaultResource = (0, auth_utils_js_1.resourceUrlFromServerUrl)(
    serverUrl,
  );
  ...
  if (
    !(0, auth_utils_js_1.checkResourceAllowed)({
...
async function discoverOAuthProtectedResourceMetadata(
  ...
  return auth_js_2.OAuthProtectedResourceMetadataSchema.parse(
    await response.json(),
  );
}
/**
...
async function discoverMetadataWithFallback(
  ...
  const protocolVersion =
    opts?.protocolVersion ?? types_js_1.LATEST_PROTOCOL_VERSION;
  let url;
  if (opts?.metadataUrl) {
...
async function discoverAuthorizationServerMetadata(
  ...
  for (const { url: endpointUrl, type } of urlsToTry) {
    ...
    if (type === "oauth") {
      ...
      return auth_js_1.OpenIdProviderDiscoveryMetadataSchema.parse(
        await response.json(),
      );
    }
  }
...
async function fetchToken(
  provider,
  authorizationServerUrl,
  { metadata, resource, authorizationCode, fetchFn } = {},
) {
...
async function registerClient(
  authorizationServerUrl,
  { metadata, clientMetadata, fetchFn },
) {
  let registrationUrl;
  if (metadata) {
  ...
  return auth_js_2.OAuthClientInformationFullSchema.parse(
    await response.json(),
  );
}
