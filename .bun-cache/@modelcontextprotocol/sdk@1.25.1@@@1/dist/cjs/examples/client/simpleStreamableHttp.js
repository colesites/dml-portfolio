  ...
  try {
    ...
    client.setRequestHandler(
      ...
      async (request) => {
        ...
        console.log(
          `Related Task: ${request.params._meta?.[types_js_1.RELATED_TASK_META_KEY]?.taskId}`,
        );
    ...
    transport = new streamableHttp_js_1.StreamableHTTPClientTransport(
      new URL(serverUrl),
      {
        sessionId: sessionId,
      },
  ...
  try {
    ...
    if (toolsResult.tools.length === 0) {
      ...
      for (const tool of toolsResult.tools) {
        console.log(
          `  - id: ${tool.name}, name: ${(0, metadataUtils_js_1.getDisplayName)(tool)}, description: ${tool.description}`,
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (promptsResult.prompts.length === 0) {
      ...
      for (const prompt of promptsResult.prompts) {
        console.log(
          `  - id: ${prompt.name}, name: ${(0, metadataUtils_js_1.getDisplayName)(prompt)}, description: ${prompt.description}`,
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (resourcesResult.resources.length === 0) {
      ...
      for (const resource of resourcesResult.resources) {
        console.log(
          `  - id: ${resource.name}, name: ${(0, metadataUtils_js_1.getDisplayName)(resource)}, description: ${resource.uri}`,
        );
      }
    }
  } catch (error) {
