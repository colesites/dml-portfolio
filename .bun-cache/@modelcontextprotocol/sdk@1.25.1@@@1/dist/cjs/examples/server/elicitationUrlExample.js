...
const oauthMetadata = (0, demoInMemoryOAuthProvider_js_1.setupAuthServer)({
  authServerUrl,
  mcpServerUrl,
  strictResource: true,
});
...
app.use(
  (0, router_js_1.mcpAuthMetadataRouter)({
    oauthMetadata,
...
authMiddleware = (0, bearerAuth_js_1.requireBearerAuth)({
  ...
  requiredScopes: [],
  resourceMetadataUrl: (0, router_js_1.getOAuthProtectedResourceMetadataUrl)(
    mcpServerUrl,
  ),
});
/**
...
app.post("/confirm-payment", express_1.default.urlencoded(), (req, res) => {
  ...
  if (action === "confirm") {
    ...
    console.log(
      `💳 Payment confirmed for cart ${cartId || "unknown"} by user ${userSession.name} (session ${sessionId})`,
    );
    ...
    console.log(
      `💳 Payment cancelled for cart ${cartId || "unknown"} by user ${userSession.name} (session ${sessionId})`,
    );
