...
class Protocol {
  constructor(_options) {
    ...
    this.setNotificationHandler(
      types_js_1.ProgressNotificationSchema,
      (notification) => {
    ...
    if (this._taskStore) {
      this.setRequestHandler(
        types_js_1.GetTaskRequestSchema,
        async (request, extra) => {
      ...
      this.setRequestHandler(
        types_js_1.GetTaskPayloadRequestSchema,
        async (request, extra) => {
                ...
                if (
                  ...
                  if (resolver) {
                    ...
                    if (queuedMessage.type === "response") {
                      ...
                      const error = new types_js_1.McpError(
      ...
      this.setRequestHandler(
        types_js_1.ListTasksRequestSchema,
        async (request, extra) => {
          try {
      ...
      this.setRequestHandler(
        types_js_1.CancelTaskRequestSchema,
        async (request, extra) => {
          try {
  ...
  _resetTimeout(messageId) {
    ...
    if (info.maxTotalTimeout && totalElapsed >= info.maxTotalTimeout) {
      ...
      throw types_js_1.McpError.fromError(
        types_js_1.ErrorCode.RequestTimeout,
  ...
  async connect(transport) {
    ...
    this._transport.onmessage = (message, extra) => {
      ...
      if (
        (0, types_js_1.isJSONRPCResultResponse)(message) ||
        (0, types_js_1.isJSONRPCErrorResponse)(message)
      ) {
  ...
  _onclose() {
    ...
    const error = types_js_1.McpError.fromError(
      types_js_1.ErrorCode.ConnectionClosed,
      "Connection closed",
    );
  ...
  _onrequest(request, extra) {
    ...
    const relatedTaskId =
      request.params?._meta?.[types_js_1.RELATED_TASK_META_KEY]?.taskId;
    if (handler === undefined) {
      const errorResponse = {
        jsonrpc: "2.0",
        id: request.id,
    ...
    const taskCreationParams = (0, types_js_1.isTaskAugmentedRequestParams)(
      request.params,
    )
      ? request.params.task
      : undefined;
  ...
  _onresponse(response) {
    ...
    if (
      (0, types_js_1.isJSONRPCResultResponse)(response) &&
      response.result &&
  ...
  async *requestStream(request, resultSchema, options) {
    ...
    if (!task) {
      try {
        ...
        yield {
          type: "error",
          error:
            error instanceof types_js_1.McpError
              ? error
              : new types_js_1.McpError(
                  types_js_1.ErrorCode.InternalError,
                  String(error),
                ),
        };
    ...
    try {
      ...
      const createResult = await this.request(
        request,
        types_js_1.CreateTaskResultSchema,
        options,
      );
      ...
      while (true) {
        ...
        const pollInterval =
          task.pollInterval ?? this._options?.defaultTaskPollInterval ?? 1000;
  ...
  request(request, resultSchema, options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cancel = (reason) => {
        ...
        const error =
          reason instanceof types_js_1.McpError
            ? reason
            : new types_js_1.McpError(
                types_js_1.ErrorCode.RequestTimeout,
                String(reason),
      ...
      this._responseHandlers.set(messageId, (response) => {
        ...
        try {
          const parseResult = (0, zod_compat_js_1.safeParse)(
            resultSchema,
            response.result,
          );
  ...
  async getTask(params, options) {
    ...
    return this.request(
      ...
      types_js_1.GetTaskResultSchema,
      options,
    );
  }
  /**
  ...
  async listTasks(params, options) {
    ...
    return this.request(
      ...
      types_js_1.ListTasksResultSchema,
      options,
    );
  }
  /**
  ...
  async cancelTask(params, options) {
    ...
    return this.request(
      ...
      types_js_1.CancelTaskResultSchema,
      options,
    );
  }
  /**
  ...
  setRequestHandler(requestSchema, handler) {
    const method = (0, zod_json_schema_compat_js_1.getMethodLiteral)(
      requestSchema,
    );
    this.assertRequestHandlerCapability(method);
    this._requestHandlers.set(method, (request, extra) => {
      const parsed = (0, zod_json_schema_compat_js_1.parseWithCompat)(
        requestSchema,
        request,
      );
    ...
    if (this._taskMessageQueue) {
      ...
      for (const message of messages) {
        if (
          ...
          (0, types_js_1.isJSONRPCRequest)(message.message)
        ) {
