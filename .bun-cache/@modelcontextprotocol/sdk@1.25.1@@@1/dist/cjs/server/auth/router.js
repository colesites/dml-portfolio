...
exports.getOAuthProtectedResourceMetadataUrl =
...
const createOAuthMetadata = (options) => {
  ...
  const metadata = {
    ...
    authorization_endpoint: new URL(authorization_endpoint, baseUrl || issuer)
      .href,
...
function mcpAuthRouter(options) {
  ...
  router.use(
    ...
    (0, authorize_js_1.authorizationHandler)({
  ...
  if (oauthMetadata.registration_endpoint) {
    router.use(
      ...
      (0, register_js_1.clientRegistrationHandler)({
  ...
  if (oauthMetadata.revocation_endpoint) {
    router.use(
      ...
      (0, revoke_js_1.revocationHandler)({
        provider: options.provider,
...
function mcpAuthMetadataRouter(options) {
  ...
  router.use(
    ...
    (0, metadata_js_1.metadataHandler)(protectedResourceMetadata),
  );
  // Always add this for OAuth Authorization Server metadata per RFC 8414
  router.use(
    "/.well-known/oauth-authorization-server",
    (0, metadata_js_1.metadataHandler)(options.oauthMetadata),
  );
  return router;
}
/**
