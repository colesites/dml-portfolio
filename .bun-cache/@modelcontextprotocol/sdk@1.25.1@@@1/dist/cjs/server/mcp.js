...
class McpServer {
  ...
  setToolRequestHandlers() {
    ...
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.ListToolsRequestSchema),
    );
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.CallToolRequestSchema),
    );
      ...
      tools: Object.entries(this._registeredTools)
        ...
        .map(([name, tool]) => {
          const toolDefinition = {
            ...
            inputSchema: (() => {
              const obj = (0, zod_compat_js_1.normalizeObjectSchema)(
                tool.inputSchema,
              );
              return obj
          ...
          if (tool.outputSchema) {
            ...
            if (obj) {
              toolDefinition.outputSchema = (0,
              zod_json_schema_compat_js_1.toJsonSchemaCompat)(obj, {
                strictUnions: true,
                pipeStrategy: "output",
              });
            }
  ...
  async validateToolInput(tool, args, toolName) {
    ...
    const inputObj = (0, zod_compat_js_1.normalizeObjectSchema)(
      tool.inputSchema,
    );
    const schemaToParse = inputObj ?? tool.inputSchema;
    const parseResult = await (0, zod_compat_js_1.safeParseAsync)(
      schemaToParse,
      args,
    );
  ...
  async validateToolOutput(tool, result, toolName) {
    ...
    const outputObj = (0, zod_compat_js_1.normalizeObjectSchema)(
      tool.outputSchema,
    );
    const parseResult = await (0, zod_compat_js_1.safeParseAsync)(
      outputObj,
  ...
  setCompletionRequestHandler() {
    ...
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.CompleteRequestSchema),
    );
    this.server.registerCapabilities({
      completions: {},
    });
    this.server.setRequestHandler(
      types_js_1.CompleteRequestSchema,
      async (request) => {
  ...
  setResourceRequestHandlers() {
    ...
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.ListResourcesRequestSchema),
    );
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.ListResourceTemplatesRequestSchema),
    );
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.ReadResourceRequestSchema),
    );
    this.server.registerCapabilities({
      resources: {
        listChanged: true,
    ...
    this.server.setRequestHandler(
      types_js_1.ListResourcesRequestSchema,
      async (request, extra) => {
    ...
    this.server.setRequestHandler(
      types_js_1.ListResourceTemplatesRequestSchema,
      async () => {
  ...
  setPromptRequestHandlers() {
    ...
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.ListPromptsRequestSchema),
    );
    this.server.assertCanSetRequestHandler(
      getMethodValue(types_js_1.GetPromptRequestSchema),
    );
    this.server.registerCapabilities({
      prompts: {
        listChanged: true,
    ...
    this.server.setRequestHandler(
      ...
      async (request, extra) => {
        ...
        if (prompt.argsSchema) {
          const argsObj = (0, zod_compat_js_1.normalizeObjectSchema)(
            prompt.argsSchema,
          );
          const parseResult = await (0, zod_compat_js_1.safeParseAsync)(
            argsObj,
          ...
          if (!parseResult.success) {
            const error =
              "error" in parseResult ? parseResult.error : "Unknown error";
            const errorMessage = (0, zod_compat_js_1.getParseErrorMessage)(
              error,
            );
            throw new types_js_1.McpError(
              types_js_1.ErrorCode.InvalidParams,
  ...
  _createRegisteredPrompt(name, title, description, argsSchema, callback) {
    const registeredPrompt = {
      ...
      argsSchema:
        argsSchema === undefined
          ? undefined
          : (0, zod_compat_js_1.objectFromShape)(argsSchema),
      callback,
      enabled: true,
      ...
      update: (updates) => {
        ...
        if (typeof updates.argsSchema !== "undefined")
          registeredPrompt.argsSchema = (0, zod_compat_js_1.objectFromShape)(
            updates.argsSchema,
          );
        if (typeof updates.callback !== "undefined")
    ...
    if (argsSchema) {
      const hasCompletable = Object.values(argsSchema).some((field) => {
        const inner =
          field instanceof zod_1.ZodOptional ? field._def?.innerType : field;
    ...
    const registeredTool = {
      ...
      update: (updates) => {
        ...
        if (typeof updates.paramsSchema !== "undefined")
          registeredTool.inputSchema = (0, zod_compat_js_1.objectFromShape)(
            updates.paramsSchema,
          );
        if (typeof updates.outputSchema !== "undefined")
          registeredTool.outputSchema = (0, zod_compat_js_1.objectFromShape)(
