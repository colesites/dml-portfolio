...
class Server extends protocol_js_1.Protocol {
  ...
  constructor(_serverInfo, options) {
    ...
    this.LOG_LEVEL_SEVERITY = new Map(
      types_js_1.LoggingLevelSchema.options.map((level, index) => [
        level,
        index,
      ]),
    );
    ...
    this.setRequestHandler(types_js_1.InitializeRequestSchema, (request) =>
      this._oninitialize(request),
    );
    this.setNotificationHandler(types_js_1.InitializedNotificationSchema, () =>
      this.oninitialized?.(),
    );
    if (this._capabilities.logging) {
      this.setRequestHandler(
        types_js_1.SetLevelRequestSchema,
        async (request, extra) => {
  ...
  async _oninitialize(request) {
    ...
    const protocolVersion = types_js_1.SUPPORTED_PROTOCOL_VERSIONS.includes(
      requestedVersion,
    )
      ? requestedVersion
  ...
  async createMessage(params, options) {
    ...
    if (params.tools) {
      return this.request(
        ...
        types_js_1.CreateMessageResultWithToolsSchema,
        options,
      );
    }
    return this.request(
  ...
  async listRoots(params, options) {
    return this.request(
      ...
      types_js_1.ListRootsResultSchema,
      options,
    );
  }
  /**
