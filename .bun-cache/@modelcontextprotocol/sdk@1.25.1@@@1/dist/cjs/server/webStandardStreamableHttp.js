...
class WebStandardStreamableHTTPServerTransport {
  ...
  async handlePostRequest(req, options) {
    try {
      ...
      try {
        if (Array.isArray(rawMessage)) {
          messages = rawMessage.map((msg) =>
            types_js_1.JSONRPCMessageSchema.parse(msg),
          );
        } else {
      ...
      const isInitializationRequest = messages.some(
        types_js_1.isInitializeRequest,
      );
      ...
      const initRequest = messages.find((m) =>
        (0, types_js_1.isInitializeRequest)(m),
      );
      const clientProtocolVersion = initRequest
        ? initRequest.params.protocolVersion
        : (req.headers.get("mcp-protocol-version") ??
          types_js_1.DEFAULT_NEGOTIATED_PROTOCOL_VERSION);
      ...
      for (const message of messages) {
        ...
        if (
          (0, types_js_1.isJSONRPCRequest)(message) &&
          this._eventStore &&
  ...
  validateProtocolVersion(req) {
    ...
    if (
      ...
      !types_js_1.SUPPORTED_PROTOCOL_VERSIONS.includes(protocolVersion)
    ) {
      return this.createJsonErrorResponse(
        400,
        -32000,
        `Bad Request: Unsupported protocol version: ${protocolVersion} (supported versions: ${types_js_1.SUPPORTED_PROTOCOL_VERSIONS.join(", ")})`,
      );
    }
    return undefined;
  }
...
exports.WebStandardStreamableHTTPServerTransport =
