...
function applyStructuredPatch(source, patch, options) {
  ...
  if (
    ...
    if (
      (0, string_js_1.hasOnlyWinLineEndings)(source) &&
      (0, line_endings_js_1.isUnix)(patch)
    ) {
      patch = (0, line_endings_js_1.unixToWin)(patch);
    } else if (
      (0, string_js_1.hasOnlyUnixLineEndings)(source) &&
      (0, line_endings_js_1.isWin)(patch)
    ) {
  ...
  for (var i = 0; i < hunks.length; i++) {
    ...
    for (var maxErrors = 0; maxErrors <= fuzzFactor; maxErrors++) {
      ...
      var iterator = (0, distance_iterator_js_1.default)(
        toPos,
        minLine,
        maxLine,
      );
...
function applyPatches(uniDiff, options) {
  var spDiff =
    typeof uniDiff === "string" ? (0, parse_js_1.parsePatch)(uniDiff) : uniDiff;
  var currentIndex = 0;
