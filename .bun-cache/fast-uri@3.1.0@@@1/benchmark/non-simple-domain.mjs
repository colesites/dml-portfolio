...
console.assert(
  nonSimpleDomain(exampleCom) === false,
  "example.com should be a simple domain",
);
console.assert(
  nonSimpleDomain(exaumlmpleCom) === true,
  "exämple.com should not be a simple domain",
);
console.assert(
  nonSimpleDomain(longDomain) === false,
  `${longDomain} should be a simple domain?`,
);

benchNonSimpleDomain.add("nonSimpleDomain", () => {
