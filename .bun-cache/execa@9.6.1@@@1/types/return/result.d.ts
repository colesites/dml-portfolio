...
> = InstanceType<typeof CommonResult<IsSync, OptionsType>> &
  OmitErrorIfReject<OptionsType["reject"]>;

type OmitErrorIfReject<RejectOption extends CommonOptions["reject"]> = {
  [ErrorProperty in ErrorProperties]: RejectOption extends false
    ? unknown
    : never;
};
...
*/
export type Result<OptionsType extends Options = Options> = SuccessResult<
  false,
  OptionsType
>;
...
When the subprocess fails, it is rejected with an `ExecaError` instead.
*/
export type SyncResult<OptionsType extends SyncOptions = SyncOptions> =
  SuccessResult<true, OptionsType>;
