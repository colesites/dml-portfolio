...
export const normalizeStdioOption = (
  { stdio, ipc, buffer, ...options },
  verboseInfo,
  isSync,
) => {
  const stdioArray = getStdioArray(stdio, options).map(
    (stdioOption, fdNumber) => addDefaultValue(stdioOption, fdNumber),
  );
  return isSync
...
const hasAlias = (options) =>
  STANDARD_STREAMS_ALIASES.some((alias) => options[alias] !== undefined);
...
const isOutputPipeOnly = (stdioOption) =>
  stdioOption === "pipe" ||
