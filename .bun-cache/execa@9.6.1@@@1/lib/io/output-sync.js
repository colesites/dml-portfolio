...
import {
  ...
  joinToString,
  joinToUint8Array,
} from "../utils/uint-array.js";
import { logLinesSync, shouldLogOutput } from "../verbose/output.js";
import { truncateMaxBufferSync } from "./max-buffer.js";

// Apply `stdout`/`stderr` options, after spawning, in sync mode
export const transformOutputSync = ({
  fileDescriptors,
  syncResult: { output },
  options,
  isMaxBuffer,
...
const writeToFiles = (serializedResult, stdioItems, outputFiles) => {
  for (const { path, append } of stdioItems.filter(({ type }) =>
    FILE_TYPES.has(type),
  )) {
