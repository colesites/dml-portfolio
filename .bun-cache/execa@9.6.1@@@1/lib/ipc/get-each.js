...
const abortOnStrictError = async ({
  ipcEmitter,
  isSubprocess,
  controller,
  state,
  ...
  try {
    const [error] = await once(ipcEmitter, "strict:error", {
...
const iterateOnMessages = async function* ({
  anyProcess,
  channel,
  ipcEmitter,
  isSubprocess,
  ...
  try {
    for await (const [message] of on(ipcEmitter, "message", {
      signal: controller.signal,
    })) {
