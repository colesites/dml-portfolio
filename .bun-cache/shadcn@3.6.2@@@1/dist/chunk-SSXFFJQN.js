...
import a from "dedent";
import { z } from "zod";
import { zodToJsonSchema } from "zod-to-json-schema";
import { _, aa, ma, u as u$1 } from "./chunk-I3JXSMK6.js";

async function k(n, { withFallback: e } = { withFallback: false }) {
  const r = await detect({ programmatic: true, cwd: n });
  if (r === "yarn@berry") return "yarn";
  if (r === "pnpm@6") return "pnpm";
  if (r === "bun") return "bun";
  if (r === "deno") return "deno";
  if (!e) return r ?? "npm";
  const i = process.env.npm_config_user_agent || "";
  return i.startsWith("yarn")
    ? "yarn"
    : i.startsWith("pnpm")
      ? "pnpm"
      : i.startsWith("bun")
...
async function b(n) {
  const e = await k(n);
  return e === "pnpm" ? "pnpm dlx" : e === "bun" ? "bunx" : "npx";
}
var C = "shadcn@latest";
async function g(n) {
  return `${await b(process.cwd())} ${C} ${n}`;
}
async function u(n = process.cwd()) {
  return { registries: (await aa(n, { useCache: false })).registries };
}
function y(n, e) {
  let { query: r, registries: i } = e || {},
    s = n.items.map((c) => {
      const f = [`- ${c.name}`];
      return (
        c.type && f.push(`(${c.type})`),
        c.description && f.push(`- ${c.description}`),
        c.registry && f.push(`[${c.registry}]`),
        f.push(`
  Add command: \`${g(`add ${c.addCommandArgument}`)}\``),
        f.join(" ")
      );
    }),
    o = `Found ${n.pagination.total} items`;
  r && (o += ` matching "${r}"`),
    i && i.length > 0 && (o += ` in registries ${i.join(", ")}`),
    (o += ":");
  let m = `Showing items ${n.pagination.offset + 1}-${Math.min(n.pagination.offset + n.pagination.limit, n.pagination.total)} of ${n.pagination.total}:`,
    d = `${o}
...
`)}`;
  return (
    n.pagination.hasMore &&
      (d += `

More items available. Use offset: ${n.pagination.offset + n.pagination.limit} to see the next page.`),
    d
  );
}
function w(n) {
...
`
      ...
      e.files && e.files.length > 0
        ? `**Files:** ${e.files.length} file(s)`
        : "",
      e.dependencies && e.dependencies.length > 0
        ? `**Dependencies:** ${e.dependencies.join(", ")}`
        : "",
      e.devDependencies && e.devDependencies.length > 0
        ? `**Dev Dependencies:** ${e.devDependencies.join(", ")}`
        : "",
    ]
      .filter(Boolean)
      .join(`
...
j.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
...
j.setRequestHandler(CallToolRequestSchema, async (n) => {
  try {
    if (!n.params.arguments) throw new Error("No tool arguments provided.");
    switch (n.params.name) {
      case "get_project_registries": {
        const e = await u(process.cwd());
        return e?.registries
          ? {
              content: [
                {
                  type: "text",
                  text: a`The following registries are configured in the current project:

                ${Object.keys(e.registries)
                  .map((r) => `- ${r}`)
                  .join(`
`)}
          ...
          : {
              content: [
                {
                  type: "text",
                  text: a`No components.json found or no registries configured.
      ...
      case "search_items_in_registries": {
        const r = z
            ...
            .parse(n.params.arguments),
          i = await ma(r.registries, {
            query: r.query,
            limit: r.limit,
            offset: r.offset,
            config: await u(process.cwd()),
            useCache: !1,
          });
        return i.items.length === 0
          ? {
              content: [
                {
                  type: "text",
                  text: a`No items found matching "${r.query}" in registries ${r.registries.join(", ")}, Try searching with a different query or registry.`,
                },
              ],
            }
          : {
              content: [
                {
                  type: "text",
                  text: y(i, { query: r.query, registries: r.registries }),
                },
              ],
            };
      }
        ...
        return i.items.length === 0
          ? {
              content: [
                {
                  type: "text",
                  text: a`No items found in registries ${r.registries.join(", ")}.`,
                },
              ],
            }
          : {
              content: [
                { type: "text", text: y(i, { registries: r.registries }) },
              ],
            };
      }
        ...
        if (i?.length === 0)
          return {
            content: [
              {
                type: "text",
                text: a`No items found for: ${r.items.join(", ")}
...
`)}`,
      ...
      case "get_item_examples_from_registries": {
        const r = z
            .object({ query: z.string(), registries: z.array(z.string()) })
            .parse(n.params.arguments),
          i = await u(),
          s = await ma(r.registries, {
            query: r.query,
            config: i,
            useCache: !1,
          });
        if (s.items.length === 0)
          return {
            content: [
              {
                type: "text",
                text: a`No examples found for query "${r.query}".
        ...
        const o = s.items.map((d) => d.addCommandArgument),
          m = await _(o, { config: i, useCache: !1 });
        return { content: [{ type: "text", text: $(m, r.query) }] };
      }
      case "get_add_command_for_items": {
        ...
        return {
          content: [
            { type: "text", text: await g(`add ${e.items.join(" ")}`) },
          ],
        };
      }
      case "get_audit_checklist":
        return {
          content: [
            {
              type: "text",
              text: a`## Component Audit Checklist
      ...
      default:
        throw new Error(`Tool ${n.params.name} not found`);
    }
  } catch (e) {
    if (e instanceof z.ZodError)
      return {
        content: [
          {
            type: "text",
            text: a`Invalid input parameters:
              ${e.errors
...
\u{1F4A1} ${e.suggestion}`),
        e.context &&
          (i += `

Context: ${JSON.stringify(e.context, null, 2)}`),
        {
          content: [{ type: "text", text: a`Error (${e.code}): ${i}` }],
          isError: true,
        }
      );
    }
    const r = e instanceof Error ? e.message : String(e);
    return { content: [{ type: "text", text: a`Error: ${r}` }], isError: true };
  }
});
export { k as a, j as b }; //# sourceMappingURL=chunk-SSXFFJQN.js.map
