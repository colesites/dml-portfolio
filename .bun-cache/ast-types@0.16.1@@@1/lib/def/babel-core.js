...
function default_1(fork) {
  ...
  (_a = def("StringLiteral")
    .bases("Literal")
    .build("value")
    .field("value", String)).field.apply(
    _a,
    makeLiteralExtra(String, (val) => JSON.stringify(val)),
  );
  (_b = def("NumericLiteral")
    .bases("Literal")
    .build("value")
    .field("value", Number)
    .field("raw", or(String, null), defaults["null"])).field.apply(
    _b,
    makeLiteralExtra(Number),
  );
  (_c = def("BigIntLiteral")
    ...
    .field("value", or(String, Number))).field.apply(
    _c,
    makeLiteralExtra(String, (val) => val + "n"),
  );
  ...
  (_d = def("DecimalLiteral")
    ...
    .field("value", String)).field.apply(
    _d,
    makeLiteralExtra(String, (val) => val + "m"),
  );
  def("NullLiteral")
    .bases("Literal")
    .build()
  ...
  (_e = def("RegExpLiteral")
    ...
    .apply(
      _e,
      makeLiteralExtra(or(RegExp, isUndefined), (exp) =>
        "/".concat(exp.pattern, "/").concat(exp.flags || ""),
      ),
    )
    .field(
      "regex",
