...
function scopePlugin(fork) {
  ...
  Scope.isEstablishedBy = (node) =>
    ScopeType.check(node) || TypeParameterScopeType.check(node);
  ...
  function recursiveScanScope(path, bindings, scopeTypes) {
    ...
    if (!node) {
      ...
      types.eachField(node, (name, child) => {
  ...
  function addPattern(patternPath, bindings) {
    ...
    if (namedTypes.Identifier.check(pattern)) {
      ...
      patternPath.get("elements").each((elementPath) => {
