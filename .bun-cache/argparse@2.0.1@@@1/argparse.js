...
function _parse_opts(args, descriptor) {
  ...
  if (args.length) {
    const from = Object.entries(descriptor).filter(
    ...
    class HelpFormatter {
      ...
      add_argument(action) {
        if (action.help !== SUPPRESS) {
          ...
          const invocation_length = Math.max(
      ...
      _format_usage(usage, actions, groups, prefix) {
        ...
        if (usage !== undefined) {
          ...
          const action_usage = this._format_actions_usage(
      ...
      _format_actions_usage(actions, groups) {
        ...
        for (const i of Object.keys(inserts)
          .map(Number)
          .sort((a, b) => b - a)) {
      ...
      _expand_help(action) {
        ...
        if (params.choices !== undefined) {
          const choices_str = _choices_to_array(params.choices)
            .map(String)
            .join(", ");
...
HelpFormatter.prototype._Section = _callable(
  class _Section {
    ...
    format_help() {
      ...
      const item_help = this.formatter._join_parts(
...
const _StoreTrueAction = _callable(
  class _StoreTrueAction extends _StoreConstAction {
    constructor() {
      const [option_strings, dest, default_value, required, help] = _parse_opts(
        arguments,
        {
          option_strings: no_default,
          dest: no_default,
...
const _HelpAction = _callable(
  class _HelpAction extends Action {
    constructor() {
      const [option_strings, dest, default_value, help] = _parse_opts(
        arguments,
        {
          option_strings: no_default,
          dest: SUPPRESS,
...
const _VersionAction = _callable(
  class _VersionAction extends Action {
    constructor() {
      const [option_strings, version, dest, default_value, help] = _parse_opts(
        arguments,
        {
          option_strings: no_default,
          version: undefined,
...
const _SubParsersAction = _camelcase_alias(
  _callable(
    class _SubParsersAction extends Action {
      ...
      add_parser() {
        const [name, kwargs] = _parse_opts(arguments, {
          name: no_default,
          "**kwargs": no_default,
        });
...
const _ActionsContainer = _camelcase_alias(
  _callable(
    class _ActionsContainer {
      constructor() {
        const [description, prefix_chars, argument_default, conflict_handler] =
          _parse_opts(arguments, {
      ...
      _get_positional_kwargs() {
        const [dest, kwargs] = _parse_opts(arguments, {
          dest: no_default,
          "**kwargs": no_default,
        });
      ...
      _get_optional_kwargs() {
        const [args, kwargs] = _parse_opts(arguments, {
          "*args": no_default,
          "**kwargs": no_default,
        });
      ...
      _handle_conflict_error(action, conflicting_actions) {
        ...
        const conflict_string = conflicting_actions
...
const _ArgumentGroup = _callable(
  class _ArgumentGroup extends _ActionsContainer {
    constructor() {
      const [container, title, description, kwargs] = _parse_opts(arguments, {
...
const _MutuallyExclusiveGroup = _callable(
  class _MutuallyExclusiveGroup extends _ArgumentGroup {
    constructor() {
      const [container, required] = _parse_opts(arguments, {
        container: no_default,
        required: false,
      });
  ...
  _callable(
    class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
      ...
      constructor() {
        ...
        const default_prefix = prefix_chars.includes("-")
          ? "-"
          : prefix_chars[0];
        if (this.add_help) {
          this.add_argument(
      ...
      // ==================================
      add_subparsers() {
        const [kwargs] = _parse_opts(arguments, {
          "**kwargs": no_default,
        });
        ...
        // eslint-disable-next-line new-cap
        const action = new parsers_class(
      ...
      _parse_known_args(arg_strings, namespace) {
        ...
        for (const mutex_group of this._mutually_exclusive_groups) {
          ...
          for (const [i, mutex_action] of Object.entries(
        ...
        const consume_positionals = (start_index) => {
          ...
          const arg_counts = this._match_arguments_partial(
            positionals,
            selected_pattern,
          );
        ...
        extras = [];
        let start_index = 0;
        const max_option_string_index = Math.max(
          -1,
        ...
        while (start_index <= max_option_string_index) {
          ...
          if (!(start_index in option_string_indices)) {
            const strings = arg_strings.slice(
              start_index,
      ...
      _match_argument(action, arg_strings_pattern) {
        ...
        const match = arg_strings_pattern.match(
          new RegExp("^" + nargs_pattern),
        );
      ...
      _parse_optional(arg_string) {
        ...
        if (arg_string.includes("=")) {
          const [option_string, explicit_arg] = _string_split(
            arg_string,
            "=",
            1,
          );
      ...
      _get_option_tuples(option_string) {
        ...
        if (
          ...
          if (this.allow_abbrev) {
            ...
            for (const option_string of Object.keys(
              this._option_string_actions,
            )) {
      ...
      parse_known_intermixed_args(args = undefined, namespace = undefined) {
        ...
        let extras;
        const positionals = this._get_positional_actions();
        const a = positionals.filter((action) =>
