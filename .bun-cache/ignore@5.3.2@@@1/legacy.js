...
var sanitizeRange = function sanitizeRange(range) {
  return range.replace(REGEX_REGEXP_RANGE, (match, from, to) =>
        ...
        : close === "]"
          ? endEscape.length % 2 === 0
            ? // A normal case, and it is a range notation
              ...
              // '[bar\\\\]'
              "["
                .concat(sanitizeRange(range))
                .concat(endEscape, "]") // Invalid range notaton
            : // '[bar\\]' -> '[bar\\\\]'
              "[]"
          : "[]",
  ],
...
var makeRegex = function makeRegex(pattern, ignoreCase) {
  ...
  if (!source) {
    source = REPLACERS.reduce((prev, _ref) => {
