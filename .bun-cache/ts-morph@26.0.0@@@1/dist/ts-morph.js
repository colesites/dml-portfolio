...
var CodeBlockWriter__default =
  /*#__PURE__*/ _interopDefaultCompat(CodeBlockWriter);
...
class AdvancedIterator {
  ...
  get current() {
    if (this.#nextCount === 0)
      throw new common.errors.InvalidOperationError(
  ...
  get previous() {
    ...
    return this.#buffer[
      (this.#bufferIndex + this.#buffer.length - 1) % this.#buffer.length
    ];
  }
  get peek() {
    if (this.#isDone)
  ...
  *rest() {
    while (!this.done) yield this.next();
  }
  #advance() {
...
function getNodeByNameOrFindFunction(items, nameOrFindFunc) {
  ...
  else findFunc = nameOrFindFunc;
  return items.find(findFunc);
}
function nodeHasName(node, name) {
  if (node.getNameNode == null) return false;
...
class CommentNodeParser {
  ...
  static getOrParseChildren(container, sourceFile) {
    if (isSyntaxList(container)) container = container.parent;
  ...
  static shouldParseChildren(container) {
    return (
      commentNodeParserKinds.has(container.kind) &&
  ...
  static getContainerBodyPos(container, sourceFile) {
    if (common.ts.isSourceFile(container)) return 0;
    if (
      common.ts.isClassDeclaration(container) ||
...
function* getNodes(container, sourceFile) {
  ...
  if (childNodes.length === 0) {
    ...
    if (bodyStartPos != null) yield* getCommentNodes(bodyStartPos, false);
  } else {
  ...
  function* getCommentNodes(pos, stopAtJsDoc) {
    ...
    const maxEnd =
      sourceFileText.length === pos || sourceFileText[pos] === "}"
        ? pos
        : common.StringUtils.getLineStartFromPos(sourceFileText, pos);
    ...
    function skipTrailingLine() {
      ...
      while (pos < lineEnd) {
        ...
        if (commentKind != null) {
          ...
          !common.StringUtils.isWhitespace(sourceFileText[pos]) &&
          sourceFileText[pos] !== ","
        )
          return;
        else pos++;
      }
      while (common.StringUtils.startsWithNewLine(sourceFileText[pos])) pos++;
    }
    function* getLeadingComments() {
      while (pos < sourceFileText.length) {
        ...
        if (commentKind != null) {
          ...
          if (isJsDoc && stopAtJsDoc) return;
          else yield parseForComment(commentKind);
          skipTrailingLine();
    ...
    function getCommentKind() {
      ...
      if (nextChar === "/") return CommentKind.SingleLine;
    ...
    function parseSingleLineComment() {
      ...
      return createComment(
        ...
        common.SyntaxKind.SingleLineCommentTrivia,
      );
    }
    function skipSingleLineComment() {
      pos += 2;
      while (
        pos < sourceFileText.length &&
        sourceFileText[pos] !== "\n" &&
    ...
    function parseMultiLineComment(isJsDoc) {
      ...
      return createComment(
        ...
        common.SyntaxKind.MultiLineCommentTrivia,
      );
    }
  ...
  function getContainerChildren() {
    if (
      common.ts.isSourceFile(container) ||
      common.ts.isBlock(container) ||
      common.ts.isModuleBlock(container) ||
      common.ts.isCaseClause(container) ||
      common.ts.isDefaultClause(container)
  ...
  function getCreationFunction() {
    ...
    function getCtor() {
      if (isStatementContainerNode(container)) return CompilerCommentStatement;
      if (common.ts.isClassLike(container)) return CompilerCommentClassElement;
      if (
        common.ts.isInterfaceDeclaration(container) ||
        common.ts.isTypeLiteralNode(container)
      )
      ...
      throw new common.errors.NotImplementedError(
        `Not implemented comment node container type: ${common.getSyntaxKindName(container.kind)}`,
      );
    }
  }
}
...
class ExtendedParser {
  ...
  static hasParsedTokens(node) {
    return (
      getChildrenSaver.has(node) || node.kind == common.SyntaxKind.SyntaxList
    );
  }
  ...
  static getCompilerChildren(node, sourceFile) {
    ...
    function isStatementMemberOrPropertyHoldingSyntaxList() {
      if (node.kind !== common.ts.SyntaxKind.SyntaxList) return false;
      const parent = node.parent;
      if (!CommentNodeParser.shouldParseChildren(parent)) return false;
      return (
        CommentNodeParser.getContainerBodyPos(parent, sourceFile) === node.pos
      );
    }
  }
}
function mergeInComments(nodes, otherNodes) {
  ...
  for (const child of otherNodes) {
    if (
      child.kind !== common.SyntaxKind.SingleLineCommentTrivia &&
      child.kind !== common.SyntaxKind.MultiLineCommentTrivia
    )
      continue;
...
function isComment(node) {
  return (
    node.kind === common.ts.SyntaxKind.SingleLineCommentTrivia ||
    node.kind === common.ts.SyntaxKind.MultiLineCommentTrivia
  );
}

function getParentSyntaxList(node, sourceFile) {
  if (node.kind === common.SyntaxKind.EndOfFileToken) return undefined;
  const parent = node.parent;
  if (parent == null) return undefined;
  const { pos, end } = node;
  for (const child of ExtendedParser.getCompilerChildren(parent, sourceFile)) {
    if (child.pos > end || child === node) return undefined;
    if (
      child.kind === common.SyntaxKind.SyntaxList &&
      child.pos <= pos &&
      child.end >= end
    )
      return child;
...
function isNodeAmbientOrInAmbientContext(node) {
  ...
  for (const ancestor of node._getAncestorsIterator(false)) {
    if (checkNodeIsAmbient(ancestor)) return true;
  }
  return false;
}
function checkNodeIsAmbient(node) {
  const isThisAmbient =
    (node.getCombinedModifierFlags() & common.ts.ModifierFlags.Ambient) ===
    common.ts.ModifierFlags.Ambient;
  return (
    isThisAmbient ||
...
class ModuleUtils {
  ...
  static getReferencedSourceFileFromSymbol(symbol) {
    ...
    if (
      ...
      declarations[0].getKind() !== common.SyntaxKind.SourceFile
    )
      return undefined;
...
function printNode(node, sourceFileOrOptions, secondOverloadOptions) {
  const isFirstOverload =
    sourceFileOrOptions == null ||
    sourceFileOrOptions.kind !== common.SyntaxKind.SourceFile;
  ...
  if (sourceFile == null) return printer.printFile(node);
  else
    return printer.printNode(
      options.emitHint ?? common.EmitHint.Unspecified,
      node,
      sourceFile,
    );
  function getSourceFile() {
    if (isFirstOverload) {
      if (node.kind === common.SyntaxKind.SourceFile) return undefined;
      const topParent = getNodeSourceFile();
      if (topParent == null) {
        const scriptKind = getScriptKind();
        return common.ts.createSourceFile(
          `print.${getFileExt(scriptKind)}`,
          "",
          common.ScriptTarget.Latest,
          false,
          scriptKind,
        );
      }
    ...
    function getFileExt(scriptKind) {
      if (
        scriptKind === common.ScriptKind.JSX ||
        scriptKind === common.ScriptKind.TSX
      )
        return "tsx";
      return "ts";
    }
  ...
  function getNodeSourceFile() {
    ...
    while (topNode != null && topNode.parent != null) topNode = topNode.parent;
    return topNode;
  }
  function getOptions() {
    return (
      (isFirstOverload ? sourceFileOrOptions : secondOverloadOptions) || {}
    );
  }
}
  ...
  _getIndentSizeInSpaces() {
    ...
    switch (indentationText) {
      ...
      case exports.IndentationText.Tab:
        return 4;
      default:
        return common.errors.throwNotImplementedForNeverValueError(
          indentationText,
        );
    }
  }
...
function fillDefaultEditorSettings(settings, manipulationSettings) {
  setValueIfUndefined(
    settings,
    "convertTabsToSpaces",
    manipulationSettings.getIndentationText() !== exports.IndentationText.Tab,
  );
  setValueIfUndefined(
    settings,
    "newLineCharacter",
...
function printTextFromStringOrWriter(writer, textOrWriterFunction) {
  if (typeof textOrWriterFunction === "string")
    ...
    for (let i = 0; i < textOrWriterFunction.length; i++) {
      if (i > 0) writer.newLineIfLastNot();
...
class EnableableLogger {
  ...
  log(text) {
    if (this.#enabled) this.logInternal(text);
  }
  warn(text) {
    if (this.#enabled) this.warnInternal(text);
  }
}
...
function isValidVariableName(variableName) {
  if (bannedNames.has(variableName)) return false;
  if (isAlphaNumeric(variableName)) return true;
...
class LazyReferenceCoordinator {
  ...
  constructor(factory) {
    const onSourceFileModified = (sourceFile) => {
      if (!sourceFile.wasForgotten()) this.#dirtySourceFiles.add(sourceFile);
    };
  ...
  refreshSourceFileIfDirty(sourceFile) {
    if (!this.#dirtySourceFiles.has(sourceFile)) return;
    sourceFile._referenceContainer.refresh();
    ...
    this.#sourceFile._context.compilerFactory.forgetNodesCreatedInBlock(
      (remember) => {
        for (const literal of this.#sourceFile.getImportStringLiterals()) {
          const sourceFile = this.#getSourceFileForLiteral(literal);
          remember(literal);
          if (sourceFile == null) this.#unresolvedLiterals.push(literal);
          else this.#addNodeInThis(literal, sourceFile);
        }
      },
    );
  }
...
const [tsMajor, tsMinor, tsPatch] = common.ts.version
  .split(".")
  .map((v) => parseInt(v, 10));
...
function callBaseGetStructure(basePrototype, node, structure) {
  ...
  if (structure != null) Object.assign(newStructure, structure);
  return newStructure;
}

function callBaseSet(basePrototype, node, structure) {
  if (basePrototype.set != null) basePrototype.set.call(node, structure);
}
...
function AmbientableNode(Base) {
  return class extends Base {
    ...
    getDeclareKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getDeclareKeyword(),
        message ?? "Expected to find a declare keyword.",
        this,
      );
    }
    getDeclareKeyword() {
...
function getClassMemberFormatting(parent, member) {
  ...
  if (hasBody$1(member)) return FormattingKind.Blankline;
  return FormattingKind.Newline;
}
function hasBody$1(node) {
  if (Node.isBodyable(node) && node.getBody() != null) return true;
  if (Node.isBodied(node)) return true;
  return false;
}
...
function hasBody(node) {
  if (Node.isBodyable(node) && node.hasBody()) return true;
  if (Node.isBodied(node)) return true;
  return (
    Node.isInterfaceDeclaration(node) ||
...
function getStatementedNodeChildFormatting(parent, member) {
  if (hasBody(member)) return FormattingKind.Blankline;
  return FormattingKind.Newline;
}
...
function getTextFromTextChanges(sourceFile, textChanges) {
  ...
  function getSortedTextChanges() {
    return textChanges
      .map((edit, index) => ({ edit: toWrappedTextChange(edit), index }))
      .sort((a, b) => {
        const aStart = a.edit.getSpan().getStart();
        const bStart = b.edit.getSpan().getStart();
        const difference = aStart - bStart;
        if (difference === 0) return a.index < b.index ? -1 : 1;
        return difference < 0 ? -1 : 1;
      });
  }
  function toWrappedTextChange(change) {
    if (change instanceof TextChange) return change;
    else return new TextChange(change);
  }
}
...
function getNewInsertCode(opts) {
  ...
  const indentationText =
    opts.indentationText ?? parent.getChildIndentationText();
  const newLineKind =
    parent._context.manipulationSettings.getNewLineKindAsString();
  return (
    getFormattingKindTextWithIndent(previousFormattingKind) +
    getChildCode() +
    getFormattingKindTextWithIndent(nextFormattingKind)
  );
...
function getAppendCommaPos(text) {
  ...
  try {
    ...
    if (token === common.ts.SyntaxKind.EndOfFileToken) return -1;
    const templateStack = [];
...
function getPosAtEndOfPreviousLine(fullText, pos) {
  while (pos > 0) {
    pos--;
    if (fullText[pos] === "\n") {
      if (fullText[pos - 1] === "\r") return pos - 1;
      return pos;
    }
  }
  return pos;
...
function getPosAtStartOfLineOrNonWhitespace(fullText, pos) {
  while (pos > 0) {
    ...
    if (currentChar === "\n") return pos + 1;
    else if (currentChar !== " " && currentChar !== "\t") return pos + 1;
  }
  return pos;
}
...
function getEndPosFromIndex(index, parent, children, fullText) {
  ...
  if (index === children.length) {
    if (Node.isSourceFile(parent)) endPos = parent.getEnd();
    else if (Node.isCaseClause(parent) || Node.isDefaultClause(parent))
      ...
      if (closeBraceToken == null) endPos = parent.getEnd();
      else endPos = closeBraceToken.getStart();
    }
  } else {
...
function getParentContainerOrThrow(parent) {
  if (Node.isModuleDeclaration(parent)) {
    ...
    return innerBody;
  } else if (Node.isBodied(parent)) return parent.getBody();
  else if (Node.isBodyable(parent)) return parent.getBodyOrThrow();
  else return parent;
}
...
function getNodesToReturn(oldChildren, newChildren, index, allowCommentNodes) {
  const oldChildCount =
    typeof oldChildren === "number" ? oldChildren : oldChildren.length;
...
function getRangeWithoutCommentsFromArray(array, index, length, expectedKind) {
  ...
  while (index < array.length && children.length < length) {
    ...
    if (
      childKind !== common.SyntaxKind.SingleLineCommentTrivia &&
      childKind !== common.SyntaxKind.MultiLineCommentTrivia
    ) {
      if (childKind !== expectedKind) {
        throw new common.errors.NotImplementedError(
          `Unexpected! Inserting syntax kind of ${common.getSyntaxKindName(expectedKind)}` +
...
function verifyAndGetIndex(index, length) {
  ...
  if (newIndex < 0)
    throw new common.errors.InvalidOperationError(
      `Invalid index: The max negative index is ${length * -1}, but ${index} was specified.`,
    );
  if (index > length)
    throw new common.errors.InvalidOperationError(
  ...
  forgetNodeIfNecessary(currentNode) {
    if (this.#compilerFactory.hasCompilerNode(currentNode))
      this.#compilerFactory
        .getExistingNodeFromCompilerNode(currentNode)
        .forget();
  }
...
class StraightReplacementNodeHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    if (currentNode.getKind() !== newNode.kind) {
      ...
      if (
        kinds.includes(common.ts.SyntaxKind.Identifier) &&
        kinds.includes(common.ts.SyntaxKind.PrivateIdentifier)
      ) {
        currentNode.forget();
        return;
      }
...
class ChangeChildOrderParentHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    const currentChildrenInNewOrder =
      this.#getChildrenInNewOrder(currentChildren);
    common.errors.throwIfNotEqual(
      newChildren.length,
...
class DefaultParentHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    const [currentChildren, newChildren] =
      this.#helper.getCompilerChildrenAsIterators(
        currentNode,
        newNode,
        newSourceFile,
      );
  ...
  #handleCustomMappings(newParentNode) {
    if (this.#customMappings == null) return;
...
class ForgetChangedNodeHandler {
  ...
  #handleChildren(currentNode, newNode, newSourceFile) {
    const [currentNodeChildren, newNodeChildrenArray] =
      this.#helper.getChildrenFast(currentNode, newNode, newSourceFile);
    ...
    for (const currentNodeChild of currentNodeChildren) {
      ...
      if (nextNodeChildResult.done) {
        const existingNode =
          this.#compilerFactory.getExistingNodeFromCompilerNode(
            currentNodeChild,
          );
        if (existingNode != null) existingNode.forget();
      } else {
        this.#helper.handleForValues(
          this,
          currentNodeChild,
...
class ParentFinderReplacementNodeHandler extends StraightReplacementNodeHandler {
  ...
  constructor(compilerFactory, parentNodeHandler, changingParent) {
    ...
    this.#changingParentParent =
      changingParent.getParentSyntaxList() ?? changingParent.getParent();
    this.#parentsAtSamePos =
      this.#changingParentParent != null &&
      this.#changingParentParent.getPos() === changingParent.getPos();
  ...
  #isParentNode(newNode, newSourceFile) {
    ...
    if (!positionsAndKindsEqual) return false;
    if (!this.#parentsAtSamePos) return true;
    return (
      getAncestorLength(this.#changingParent.compilerNode) ===
...
function areNodesEqual(a, b) {
  ...
  if (a.pos === b.getPos() && a.kind === b.getKind()) return true;
  return false;
}
...
class RangeHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    const currentNodeChildren = new AdvancedIterator(
      common.ArrayUtils.toIterator(children[0]),
    );
    const newNodeChildren = new AdvancedIterator(
      common.ArrayUtils.toIterator(children[1]),
    );
    while (
...
class RangeParentHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    const currentSourceFile = currentNode._sourceFile.compilerNode;
    const [currentNodeChildren, newNodeChildren] =
      this.#helper.getCompilerChildrenAsIterators(
        currentNode,
        newNode,
        newSourceFile,
...
function getRealEnd(node, sourceFile) {
  if (
    node.kind >= common.ts.SyntaxKind.FirstJSDocNode &&
    node.kind <= common.ts.SyntaxKind.LastJSDocNode
  ) {
    return getPreviousMatchingPos(
      ...
      (charCode) =>
        ...
        !common.StringUtils.isWhitespaceCharCode(charCode),
    );
  }
  return node.end;
}
...
class RenameNodeHandler extends StraightReplacementNodeHandler {
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    if (
      currentNodeKind === common.SyntaxKind.ShorthandPropertyAssignment &&
      newNodeKind === common.SyntaxKind.PropertyAssignment
    ) {
      ...
      currentNodeKind === common.SyntaxKind.ExportSpecifier &&
      newNodeKind === common.SyntaxKind.ExportSpecifier &&
      ...
      currentNodeKind === common.SyntaxKind.ImportSpecifier &&
      newNodeKind === common.SyntaxKind.ImportSpecifier &&
      currentNode.compilerNode.propertyName == null &&
      newNode.propertyName != null
    ) {
      handleImportOrExportSpecifier(this.compilerFactory);
    ...
    function handleImportOrExportSpecifier(compilerFactory) {
      function getNameText(node) {
        return node.kind === common.SyntaxKind.Identifier
          ? node.escapedText
          : node.text;
      }
      ...
      const newIdentifier =
        getNameText(newPropertyName) === getNameText(currentName.compilerNode)
          ? newName
          : newPropertyName;
...
class UnwrapParentHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    for (const child of ExtendedParser.getCompilerChildren(
...
class NodeHandlerFactory {
  getDefault(opts) {
    ...
    const replacingNodes =
      opts.replacingNodes == null ? undefined : [...opts.replacingNodes];
    ...
    if (changingParent === sourceFile) return parentHandler;
    else
  ...
  getForChildIndex(opts) {
    ...
    common.errors.throwIfOutOfRange(
      childIndex,
    ...
    if (childCount < 0)
      common.errors.throwIfOutOfRange(
        childCount,
        [childIndex - parentChildren.length, 0],
        "opts.childCount",
      );
    let i = 0;
  ...
  getForChangingChildOrder(opts) {
    ...
    if (changingParent === sourceFile) return changeChildOrderParentHandler;
    else
      return new ParentFinderReplacementNodeHandler(
        compilerFactory,
  ...
  getForUnwrappingNode(unwrappingNode) {
    const changingParent =
      unwrappingNode.getParentSyntaxList() || unwrappingNode.getParentOrThrow();
    ...
    if (changingParent === sourceFile) return unwrapParentHandler;
    else
      return new ParentFinderReplacementNodeHandler(
        compilerFactory,
        unwrapParentHandler,
...
function getSpacingBetweenNodes(opts) {
  ...
  if (previousSibling == null || nextSibling == null) return "";
...
class ChangingChildOrderTextManipulator {
  ...
  getNewText(inputText) {
    ...
    function fillPrefixText() {
      ...
      if (lowerIndex === 0 && !isParentSourceFile) finalText += newLineKind;
    }
    ...
    function fillTextForIndex(index) {
      if (index === oldIndex) fillSpacingForRemoval();
      else {
    ...
    function fillSpacingCommon(spacingOpts) {
      ...
      if (spacing === twoNewLines) {
        if (finalText.endsWith(twoNewLines)) return;
        else if (finalText.endsWith(newLineKind)) finalText += newLineKind;
        else finalText += twoNewLines;
      } else if (spacing === newLineKind) {
        if (finalText.endsWith(newLineKind)) return;
        else finalText += newLineKind;
      } else if (spacing === " ") {
        if (finalText.endsWith(" ")) return;
        else finalText += " ";
      } else {
        finalText += spacing;
      }
...
function getTextForError(newText, pos, length = 0) {
  ...
  endPos =
    endPos === -1 ? newText.length : Math.min(newText.length, endPos + 100);
  let text = "";
  text += newText.substring(startPos, endPos);
  if (startPos !== 0) text = "..." + text;
  if (endPos !== newText.length) text += "...";
  return text;
}
...
class InsertionTextManipulator {
  ...
  getNewText(inputText) {
    ...
    return (
      inputText.substring(0, insertPos) +
      newText +
...
class RemoveChildrenTextManipulator {
  ...
  getNewText(inputText) {
    ...
    function getRemovalPos() {
      if (opts.customRemovalPos != null) return opts.customRemovalPos;
    ...
    function getRemovalEnd() {
      if (opts.customRemovalEnd != null) return opts.customRemovalEnd;
    ...
    function getCharRemovalFunction(removeSpaces, removeNewLines) {
      return (char) => {
        if (
          ...
          return false;
        if (removeSpaces && !charNotSpaceOrTab(char)) return false;
        return true;
      };
    }
...
class RemoveChildrenWithFormattingTextManipulator {
  ...
  getNewText(inputText) {
    ...
    const fullText = sourceFile.getFullText();
    const newLineKind =
      sourceFile._context.manipulationSettings.getNewLineKindAsString();
    ...
    function getRemovalEnd() {
      ...
      if (parent.getEnd() === lastChild.getEnd()) return lastChild.getEnd();
      ...
      if (previousSibling == null) return triviaEnd;
      else return lastChild.getEnd();
    }
  }
  getTextForError(newText) {
    return getTextForError(newText, this.#removalPos);
...
class RenameLocationTextManipulator {
  ...
  getNewText(inputText) {
    const renameLocations = [...this.#renameLocations].sort(
      (a, b) => b.getTextSpan().getStart() - a.getTextSpan().getStart(),
    );
  ...
  getTextForError(newText) {
    if (this.#renameLocations.length === 0) return newText;
    return (
      "..." +
...
function getReplacementText(node) {
  ...
  return common.StringUtils.indent(text, -1, {
    ...
    indentSizeInSpaces:
      sourceFile._context.manipulationSettings._getIndentSizeInSpaces(),
  ...
  function getInnerBraceRange() {
    ...
    function getBodyNodeOrThrow() {
      if (Node.isModuleDeclaration(node)) {
        ...
        return bodyNode;
      } else if (Node.isBodied(node)) return node.getBody();
      else if (Node.isBodyable(node)) return node.getBodyOrThrow();
      else
        common.errors.throwNotImplementedForSyntaxKindError(
          node.getKind(),
          node,
        );
    }
...
function doManipulation(sourceFile, textManipulator, nodeHandler, newFilePath) {
  ...
  try {
    const replacementSourceFile =
      sourceFile._context.compilerFactory.createCompilerSourceFileFromText(
    ...
    const errorDetails =
      ...
      "\n" +
      "Text: " +
      JSON.stringify(textManipulator.getTextForError(newFileText)) +
      "\n" +
      "Stack: " +
      err.stack;
...
function insertIntoCommaSeparatedNodes(opts) {
  ...
  const isContained =
    ...
    (parentNextSibling.getKind() === common.SyntaxKind.CloseBraceToken ||
      parentNextSibling.getKind() === common.SyntaxKind.CloseBracketToken);
  let { newText } = opts;
  if (previousNode != null) {
    prependCommaAndSeparator();
    if (nextNonCommentNode != null || opts.useTrailingCommas)
      appendCommaAndSeparator();
    else if (opts.useNewLines || opts.surroundWithSpaces) appendSeparator();
    else appendIndentation();
    const nextEndStart =
      nextNode == null
        ? isContained
          ? parentNextSibling.getStart(true)
          : parent.getEnd()
    ...
    if (opts.useNewLines || opts.surroundWithSpaces) prependSeparator();
    if (nextNonCommentNode != null || opts.useTrailingCommas)
      appendCommaAndSeparator();
    ...
    if (opts.useNewLines || opts.surroundWithSpaces) {
      prependSeparator();
      if (opts.useTrailingCommas) appendCommaAndSeparator();
      else appendSeparator();
    } else {
      appendIndentation();
    }
  ...
  function prependCommaAndSeparator() {
    ...
    if (
      ...
      previousNodeNextSibling.getKind() === common.SyntaxKind.CommaToken
    ) {
    ...
    function appendCommentNodeTexts() {
      const lastCommentRangeEnd =
        getLastCommentRangeEnd(previousNode) || previousNode.getEnd();
    ...
    function appendNodeTrailingCommentRanges(node) {
      ...
      if (lastCommentRangeEnd == null) return;
      text += originalSourceFileText.substring(
        node.getEnd(),
        lastCommentRangeEnd,
      );
  ...
  function getPreviousNonCommentNode() {
    for (let i = insertIndex - 1; i >= 0; i--) {
      if (!Node.isCommentNode(currentNodes[i])) return currentNodes[i];
    }
    return undefined;
  }
  ...
  function appendSeparator() {
    if (!common.StringUtils.endsWithNewLine(newText)) newText += separator;
    appendIndentation();
  }
  function appendIndentation() {
    if (opts.useNewLines || common.StringUtils.endsWithNewLine(newText)) {
      ...
      else newText += parent.getParentOrThrow().getIndentationText();
    }
  }
}
...
function insertIntoBracesOrSourceFileWithGetChildren(opts) {
  if (opts.structures.length === 0) return [];
  ...
  function getPreviousJsDocCount() {
    ...
    for (let i = index - 1; i >= 0; i--) {
      ...
      if (Node.isCommentNode(node)) {
        commentCount++;
        if (node.getText().startsWith("/**")) count = commentCount;
      } else {
        break;
      }
    }
...
function removeChildren(opts) {
  ...
  doManipulation(
    ...
    new NodeHandlerFactory().getForChildIndex({
      parent:
        children[0].getParentSyntaxList() || children[0].getParentOrThrow(),
...
function removeCommaSeparatedChild(child) {
  ...
  function addNextCommaIfAble() {
    ...
    if (commaToken != null) childrenToRemove.push(commaToken);
  }
  function addPreviousCommaIfAble() {
    ...
    const precedingComma = child.getPreviousSiblingIfKind(
      common.SyntaxKind.CommaToken,
    );
    if (precedingComma != null) childrenToRemove.unshift(precedingComma);
  }
}
function removeOverloadableStatementedNodeChild(node) {
  ...
  else removeStatementedNodeChildren([...node.getOverloads(), node]);
}
...
function ArgumentedNode(Base) {
  return class extends Base {
    getArguments() {
      return (
        this.compilerNode.arguments?.map((a) =>
    ...
    insertArguments(index, argumentTexts) {
      if (argumentTexts instanceof Function) argumentTexts = [argumentTexts];
      if (common.ArrayUtils.isNullOrEmpty(argumentTexts)) return [];
      ...
      insertIntoCommaSeparatedNodes({
        parent: this.getFirstChildByKindOrThrow(
          common.SyntaxKind.OpenParenToken,
        ).getNextSiblingIfKindOrThrow(common.SyntaxKind.SyntaxList),
  ...
  return class extends Base {
    ...
    getAsyncKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find an async keyword.",
        this,
      );
    }
    setIsAsync(value) {
    ...
    set(structure) {
      callBaseSet(Base.prototype, this, structure);
      if (structure.isAsync != null) this.setIsAsync(structure.isAsync);
      return this;
    }
    getStructure() {
      return callBaseGetStructure(Base.prototype, this, {
  ...
  return class extends Base {
    ...
    getAwaitKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find an await token.",
      );
    }
    setIsAwaited(value) {
    ...
    set(structure) {
      callBaseSet(Base.prototype, this, structure);
      if (structure.isAwaited != null) this.setIsAwaited(structure.isAwaited);
      return this;
    }
  };
}
function getAwaitInsertPos(node) {
  if (node.getKind() === common.SyntaxKind.ForOfStatement)
    return node
      .getFirstChildByKindOrThrow(common.SyntaxKind.ForKeyword)
      .getEnd();
  throw new common.errors.NotImplementedError(
...
function getBodyText(writer, textOrWriterFunction) {
  ...
  if (
    typeof textOrWriterFunction !== "string" ||
    textOrWriterFunction.length > 0
  ) {
    writer.indent(() => {
      printTextFromStringOrWriter(writer, textOrWriterFunction);
    });
...
function getBodyTextWithoutLeadingIndentation(body) {
  ...
  return common.StringUtils.removeIndentation(fullText, {
    indentSizeInSpaces:
      body._context.manipulationSettings._getIndentSizeInSpaces(),
...
class TextRange {
  ...
  #throwIfForgotten() {
    if (this.#compilerObject != null) return;
    const message =
      "Attempted to get a text range that was forgotten. " +
...
class Node {
  ...
  forgetDescendants() {
    for (const child of this._getChildrenInCacheIterator()) child.forget();
  }
  _forgetOnlyThis() {
    if (this.wasForgotten()) return;
    const parent = this.getParent();
    if (parent != null) parent._wrappedChildCount--;
  ...
  _hasWrappedChildren() {
    return (
      ...
      this.#compilerNode?.kind === common.SyntaxKind.SyntaxList
    );
  }
  _replaceCompilerNodeFromFactory(compilerNode) {
    if (compilerNode == null) this.#storeTextForForgetting();
    this._clearInternals();
    this.#compilerNode = compilerNode;
  }
  #storeTextForForgetting() {
    const sourceFileCompilerNode =
      this._sourceFile && this._sourceFile.compilerNode;
    const compilerNode = this.#compilerNode;
    if (sourceFileCompilerNode == null || compilerNode == null) return;
  ...
  getSymbolOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getSymbol(),
      message ?? "Could not find the node's symbol.",
      this,
    );
  }
  getSymbol() {
    ...
    if (typeCheckerSymbol != null) return typeCheckerSymbol;
  ...
  getLocalOrThrow(name, message) {
    return common.errors.throwIfNullOrUndefined(
      this.getLocal(name),
      message ?? (() => `Expected to find local symbol with name: ${name}`),
      this,
    );
  }
  getLocal(name) {
    const locals = this.#getCompilerLocals();
    if (locals == null) return undefined;
  ...
  getLocals() {
    ...
    return Array.from(locals.values()).map((symbol) =>
  ...
  isInStringAtPos(pos) {
    ...
    class InStringRangeComparer {
      compareTo(value) {
        if (pos <= value[0]) return -1;
        if (pos >= value[1] - 1) return 1;
        return 0;
      }
    }
    return (
      common.ArrayUtils.binarySearch(
        this.#childStringRanges,
  ...
  asKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `Expected the node to be of kind ${common.getSyntaxKindName(kind)}, but it was ${common.getSyntaxKindName(this.getKind())}.`),
      this,
    );
  }
  isKind(kind) {
  ...
  getFirstChildOrThrow(condition, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Could not find a child that matched the specified condition.",
      this,
    );
  }
  ...
  getFirstDescendant(condition) {
    for (const descendant of this._getDescendantsIterator()) {
      if (condition == null || condition(descendant)) return descendant;
    }
    return undefined;
  }
  getPreviousSiblingOrThrow(condition, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Could not find the previous sibling.",
      this,
    );
  }
  ...
  getNextSiblingOrThrow(condition, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Could not find the next sibling.",
      this,
    );
  }
  ...
  *_getChildrenInCacheIterator() {
  ...
  getChildSyntaxListOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "A child syntax list was expected.",
      this,
    );
  }
  getChildSyntaxList() {
    ...
    if (Node.isBodyable(node) || Node.isBodied(node)) {
      do {
        const bodyNode = Node.isBodyable(node)
          ? node.getBody()
          : node.getBody();
        if (bodyNode == null) return undefined;
        node = bodyNode;
      } while (
  ...
  forEachDescendant(cbNode, cbNodeArray) {
    ...
    const nodeCallback = (node) => {
      ...
      if (!node.wasForgotten()) return forEachChildForNode(node);
      return undefined;
    };
    const arrayCallback =
      cbNodeArray == null
  ...
  *_getDescendantsIterator() {
  ...
  getDescendantStatements() {
    ...
    function handleNode(thisNode, node) {
      if (handleStatements(thisNode, node)) return;
        ...
        else handleNode(thisNode, arrowFunction.body);
      } else {
        handleChildren(thisNode, node);
      }
    }
    function handleStatements(thisNode, node) {
      if (node.statements == null) return false;
    ...
    function handleChildren(thisNode, node) {
      common.ts.forEachChild(node, (childNode) =>
        handleNode(thisNode, childNode),
      );
    }
  }
  getChildCount() {
  ...
  getChildAtPos(pos) {
    if (pos < this.getPos() || pos >= this.getEnd()) return undefined;
      ...
      do {
        ...
        if (nextNode != null) {
          if (nextNode.getStart() === start && nextNode.getWidth() === width)
            foundNode = nextNode;
          else if (foundNode != null) break;
        }
      } while (nextNode != null);
      if (foundNode != null) remember(foundNode);
    });
    return foundNode;
  }
  getPos() {
  ...
  getNonWhitespaceStart() {
    return this._context.compilerFactory.forgetNodesCreatedInBlock(() => {
      ...
      const parentTakesPrecedence =
        parent != null && !Node.isSourceFile(parent) && parent.getPos() === pos;
      if (parentTakesPrecedence) return this.getStart(true);
      let startSearchPos;
  ...
  getText(includeJsDocCommentOrOptions) {
    const options =
      typeof includeJsDocCommentOrOptions === "object"
        ? includeJsDocCommentOrOptions
        : undefined;
    const includeJsDocComments =
      includeJsDocCommentOrOptions === true ||
      (options != null && options.includeJsDocComments);
    const trimLeadingIndentation =
      options != null && options.trimLeadingIndentation;
    const startPos = this.getStart(includeJsDocComments);
    const text = this._sourceFile
      .getFullText()
      .substring(startPos, this.getEnd());
  ...
  getNodeProperty(propertyName) {
    ...
    else if (isNode(property)) return this._getNodeFromCompilerNode(property);
  ...
  *_getAncestorsIterator(includeSyntaxLists) {
  ...
  getParentOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getParent(),
      message ?? "Expected to find a parent.",
      this,
    );
  }
  getParentWhileOrThrow(condition, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "The initial parent did not match the provided condition.",
      this,
    );
  }
  ...
  getParentWhileKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `The initial parent was not a syntax kind of ${common.getSyntaxKindName(kind)}.`),
      this,
    );
  }
  ...
  getParentSyntaxListOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected the parent to be a syntax list.",
      this,
    );
  }
  getParentSyntaxList() {
    ...
    if (
      kind === common.SyntaxKind.SingleLineCommentTrivia ||
      kind === common.SyntaxKind.MultiLineCommentTrivia
    )
  ...
  getIndentationText(offset = 0) {
    return this.#getIndentationTextForLevel(
  ...
  getStartLineNumber(includeJsDocComments) {
    return common.StringUtils.getLineNumberAtPos(
      this._sourceFile.getFullText(),
  ...
  prependWhitespace(textOrWriterFunction) {
    insertWhiteSpaceTextAtPos(
      ...
      this.getStart(true),
      textOrWriterFunction,
      common.nameof(this, "prependWhitespace"),
    );
  }
  appendWhitespace(textOrWriterFunction) {
    insertWhiteSpaceTextAtPos(
      ...
      this.getEnd(),
      textOrWriterFunction,
      common.nameof(this, "appendWhitespace"),
    );
  }
  formatText(settings = {}) {
    const formattingEdits =
      this._context.languageService.getFormattingEditsForRange(
  ...
  transform(visitNode) {
    ...
    function handleTransformation(oldNode, newNode) {
      if (oldNode === newNode && newNode.emitNode == null) return;
      ...
      while (
        ...
        transformations.pop();
      const wrappedNode =
        compilerFactory.getExistingNodeFromCompilerNode(oldNode);
      ...
      if (wrappedNode != null) {
        if (oldNode.kind !== newNode.kind) wrappedNode.forget();
        else wrappedNode.forgetDescendants();
      }
    }
    function getTransformedText(replaceRange) {
      ...
      for (const transform of transformations) {
        ...
        finalText += printer.printNode(
          common.ts.EmitHint.Unspecified,
          transform.compilerNode,
          transform.compilerNode.getSourceFile() ?? compilerSourceFile,
        );
  ...
  getLeadingCommentRanges() {
    return (
      this.#leadingCommentRanges ||
      (this.#leadingCommentRanges = this.#getCommentsAtPos(
        this.getFullStart(),
        (text, pos) => {
          const comments = common.ts.getLeadingCommentRanges(text, pos) || [];
          if (
            this.getKind() === common.SyntaxKind.SingleLineCommentTrivia ||
            this.getKind() === common.SyntaxKind.MultiLineCommentTrivia
          ) {
  ...
  getTrailingCommentRanges() {
    return (
      ...
      (this.#trailingCommentRanges = this.#getCommentsAtPos(
        this.getEnd(),
        common.ts.getTrailingCommentRanges,
      ))
    );
  }
  #getCommentsAtPos(pos, getComments) {
    if (this.getKind() === common.SyntaxKind.SourceFile) return [];
    return (getComments(this._sourceFile.getFullText(), pos) ?? []).map(
      (r) => new CommentRange(r, this._sourceFile),
    );
  }
  ...
  getFirstChildByKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `A child of the kind ${common.getSyntaxKindName(kind)} was expected.`),
      this,
    );
  }
  ...
  getFirstChildIfKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `A first child of the kind ${common.getSyntaxKindName(kind)} was expected.`),
      this,
    );
  }
  ...
  getLastChildIfKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `A last child of the kind ${common.getSyntaxKindName(kind)} was expected.`),
      this,
    );
  }
  ...
  getChildAtIndexIfKindOrThrow(index, kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `Child at index ${index} was expected to be ${common.getSyntaxKindName(kind)}`),
      this,
    );
  }
  ...
  getPreviousSiblingIfKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `A previous sibling of kind ${common.getSyntaxKindName(kind)} was expected.`),
      this,
    );
  }
  getNextSiblingIfKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `A next sibling of kind ${common.getSyntaxKindName(kind)} was expected.`),
      this,
    );
  }
  ...
  getParentIfOrThrow(condition, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "The parent did not match the provided condition.",
      this,
    );
  }
  ...
  getParentIfKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `The parent was not a syntax kind of ${common.getSyntaxKindName(kind)}.`),
      this,
    );
  }
  ...
  getFirstAncestorByKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `Expected an ancestor with a syntax kind of ${common.getSyntaxKindName(kind)}.`),
      this,
    );
  }
  getFirstAncestorByKind(kind) {
    for (const parent of this._getAncestorsIterator(
      kind === common.SyntaxKind.SyntaxList,
    )) {
  ...
  getFirstAncestor(condition) {
    for (const ancestor of this._getAncestorsIterator(false)) {
      if (condition == null || condition(ancestor)) return ancestor;
    }
    return undefined;
  }
  ...
  getFirstDescendantByKindOrThrow(kind, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ??
        (() =>
          `A descendant of kind ${common.getSyntaxKindName(kind)} was expected to be found.`),
      this,
    );
  }
  ...
  _getCompilerChildrenFast() {
    return ExtendedParser.hasParsedTokens(this.compilerNode)
  ...
  *_getCompilerDescendantsOfKindIterator(kind) {
    const children = useParseTreeSearchForKind(this, kind)
      ...
      : this._getCompilerChildren();
    for (const child of children) {
      if (child.kind === kind) yield child;
      ...
      for (const descendant of descendants) {
        if (descendant.kind === kind) yield descendant;
      }
    }
  }
  ...
  _getCompilerFirstChild(condition) {
    for (const child of this._getCompilerChildren()) {
      if (condition == null || condition(child)) return child;
    }
    return undefined;
  }
  ...
  _getCompilerPreviousSiblings() {
    ...
    for (const child of parent._getCompilerChildren()) {
      if (child === this.compilerNode) break;
  ...
  _getCompilerPreviousSibling(condition) {
    for (const sibling of this._getCompilerPreviousSiblings()) {
      if (condition == null || condition(sibling)) return sibling;
    }
    return undefined;
  }
  ...
  _getTextWithQueuedChildIndentation(textOrWriterFunc) {
    const writer = this._getWriterWithQueuedChildIndentation();
    if (typeof textOrWriterFunc === "string") writer.write(textOrWriterFunc);
    else textOrWriterFunc(writer);
    return writer.toString();
  }
  _getWriter() {
  ...
  _ensureBound() {
    if (this.compilerNode.symbol != null) return;
    this.getSymbol();
  }
  ...
  static isCommentStatement(node) {
    return (
      node?.compilerNode?._commentKind === exports.CommentNodeKind.Statement
    );
  }
  static isCommentClassElement(node) {
    return (
      node?.compilerNode?._commentKind === exports.CommentNodeKind.ClassElement
    );
  }
  static isCommentTypeElement(node) {
    return (
      node?.compilerNode?._commentKind === exports.CommentNodeKind.TypeElement
    );
  }
  static isCommentObjectLiteralElement(node) {
    return (
      node?.compilerNode?._commentKind ===
      exports.CommentNodeKind.ObjectLiteralElement
    );
  }
  static isCommentEnumMember(node) {
    return (
      node?.compilerNode?._commentKind == exports.CommentNodeKind.EnumMember
    );
  }
  ...
  static isArrayLiteralExpression = Node.is(
    common.SyntaxKind.ArrayLiteralExpression,
  );
  ...
  static isClassStaticBlockDeclaration = Node.is(
    common.SyntaxKind.ClassStaticBlockDeclaration,
  );
  static isCommaListExpression = Node.is(common.SyntaxKind.CommaListExpression);
  static isComputedPropertyName = Node.is(
    common.SyntaxKind.ComputedPropertyName,
  );
  static isConditionalExpression = Node.is(
    common.SyntaxKind.ConditionalExpression,
  );
  ...
  static isElementAccessExpression = Node.is(
    common.SyntaxKind.ElementAccessExpression,
  );
  ...
  static isExpressionWithTypeArguments = Node.is(
    common.SyntaxKind.ExpressionWithTypeArguments,
  );
  static isExtendsClauseable(node) {
    return node?.getKind() === common.SyntaxKind.InterfaceDeclaration;
  }
  static isExternalModuleReference = Node.is(
    common.SyntaxKind.ExternalModuleReference,
  );
  ...
  static isImportEqualsDeclaration = Node.is(
    common.SyntaxKind.ImportEqualsDeclaration,
  );
  ...
  static isInterfaceDeclaration = Node.is(
    common.SyntaxKind.InterfaceDeclaration,
  );
  ...
  static isJSDocNonNullableType = Node.is(
  ...
  static isJsxSelfClosingElement = Node.is(
  ...
  static isNoSubstitutionTemplateLiteral = Node.is(
    common.SyntaxKind.NoSubstitutionTemplateLiteral,
  );
  ...
  static isParenthesizedExpression = Node.is(
    common.SyntaxKind.ParenthesizedExpression,
  );
  static isParenthesizedTypeNode(node) {
    return node?.getKind() === common.SyntaxKind.ParenthesizedType;
  }
  static isPartiallyEmittedExpression = Node.is(
    common.SyntaxKind.PartiallyEmittedExpression,
  );
  static isPostfixUnaryExpression = Node.is(
    common.SyntaxKind.PostfixUnaryExpression,
  );
  static isPrefixUnaryExpression = Node.is(
    common.SyntaxKind.PrefixUnaryExpression,
  );
  ...
  static isPropertyAccessExpression = Node.is(
    common.SyntaxKind.PropertyAccessExpression,
  );
  ...
  static isRegularExpressionLiteral = Node.is(
    common.SyntaxKind.RegularExpressionLiteral,
  );
  ...
  static isShorthandPropertyAssignment = Node.is(
    common.SyntaxKind.ShorthandPropertyAssignment,
  );
  ...
  static isTaggedTemplateExpression = Node.is(
    common.SyntaxKind.TaggedTemplateExpression,
  );
  ...
  static isTypeAliasDeclaration = Node.is(
    common.SyntaxKind.TypeAliasDeclaration,
  );
  ...
  static isVariableDeclarationList = Node.is(
    common.SyntaxKind.VariableDeclarationList,
  );
...
function useParseTreeSearchForKind(thisNodeOrSyntaxKind, searchingKind) {
  return (
    searchingKind >= common.SyntaxKind.FirstNode &&
    searchingKind < common.SyntaxKind.FirstJSDocNode &&
    getThisKind() !== common.SyntaxKind.SyntaxList
  );
  function getThisKind() {
    if (typeof thisNodeOrSyntaxKind === "number") return thisNodeOrSyntaxKind;
...
function renameNode(node, newName, options) {
  ...
  if (node.getText() === newName) return;
  ...
  for (const [
    sourceFile,
    locations,
  ] of renameLocationsBySourceFile.getEntries()) {
...
function setBodyTextForNode(body, textOrWriterFunction) {
  ...
  const openBrace = body.getFirstChildByKindOrThrow(
    common.SyntaxKind.OpenBraceToken,
  );
...
function BodyableNode(Base) {
  return class extends Base {
    getBodyOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getBody(),
        message ?? "Expected to find the node's body.",
        this,
      );
    }
    getBody() {
    ...
    addBody() {
      ...
      const semiColon = this.getLastChildByKind(
...
function ChildOrderableNode(Base) {
  return class extends Base {
    setOrder(order) {
      const childIndex = this.getChildIndex();
      const parent =
        this.getParentSyntaxList() || this.getParentSyntaxListOrThrow();
      common.errors.throwIfOutOfRange(
        order,
        [0, parent.getChildCount() - 1],
        "order",
      );
      if (childIndex === order) return this;
      changeChildOrder({
        parent,
...
function DecoratableNode(Base) {
  return class extends Base {
    ...
    insertDecorators(index, structures) {
      if (common.ArrayUtils.isNullOrEmpty(structures)) return [];
      ...
      const decoratorCode = getNewInsertCode({
        ...
        previousFormattingKind:
          previousDecorator == null ? FormattingKind.None : formattingKind,
        nextFormattingKind:
          previousDecorator == null ? formattingKind : FormattingKind.None,
      });
      insertIntoParentTextRange({
        parent:
          decorators[0]?.getParentSyntaxListOrThrow() ??
          this.getModifiers()[0]?.getParentSyntaxListOrThrow() ??
          this,
        insertPos:
          index === 0
            ? (decorators[0] ?? this).getStart()
...
function getCompilerNodeDecorators(node) {
  return common.ts.canHaveDecorators(node)
    ? (common.ts.getDecorators(node) ?? [])
    : [];
}
function getDecoratorLines(node, structures) {
  ...
  for (const structure of structures) {
    ...
    const structurePrinter =
      node._context.structurePrinterFactory.forDecorator();
...
function areDecoratorsOnSameLine(parent, currentDecorators) {
  if (currentDecorators.length === 1) {
    ...
    if (
      previousNode != null &&
      previousNode.getStartLinePos() === currentDecorators[0].getStartLinePos()
    )
      return true;
  }
  if (currentDecorators.length <= 1)
  ...
  for (let i = 1; i < currentDecorators.length; i++) {
    if (currentDecorators[i].getStartLinePos() !== startLinePos) return false;
  }
  return true;
}
  ...
  return class extends Base {
    getDotDotDotTokenOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a dot dot dot token (...).",
        this,
      );
    }
    getDotDotDotToken() {
  ...
  return class extends Base {
    ...
    getExclamationTokenNodeOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find an exclamation token.",
        this,
      );
    }
    setHasExclamationToken(value) {
      ...
      if (value === hasExclamationToken) return this;
      if (value) {
        if (Node.isQuestionTokenable(this)) this.setHasQuestionToken(false);
        const colonNode = this.getFirstChildByKind(
          common.SyntaxKind.ColonToken,
        );
        if (colonNode == null)
          throw new common.errors.InvalidOperationError(
  ...
  return class extends Base {
    ...
    getDefaultKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a default keyword.",
        this,
      );
    }
    isExported() {
      if (this.hasExportKeyword()) return true;
      const thisSymbol = this.getSymbol();
      const sourceFileSymbol = this.getSourceFile().getSymbol();
      if (thisSymbol == null || sourceFileSymbol == null) return false;
      return sourceFileSymbol
        .getExports()
        .some((e) => e === thisSymbol || e.getAliasedSymbol() === thisSymbol);
    }
    isDefaultExport() {
      if (this.hasDefaultKeyword()) return true;
      const thisSymbol = this.getSymbol();
      if (thisSymbol == null) return false;
      const defaultExportSymbol = this.getSourceFile().getDefaultExportSymbol();
      if (defaultExportSymbol == null) return false;
      if (thisSymbol === defaultExportSymbol) return true;
    ...
    isNamedExport() {
      ...
      function isDefaultExport() {
        ...
        if (defaultExportSymbol == null) return false;
        return (
          thisSymbol === defaultExportSymbol ||
...
function apply$1(Base) {
  return class extends Base {
    setIsDefaultExport(value) {
      if (value === this.isDefaultExport()) return this;
      ...
      function shouldWriteAsSeparateStatement() {
        if (
          Node.isEnumDeclaration(this) ||
          Node.isModuleDeclaration(this) ||
          Node.isTypeAliasDeclaration(this)
        )
          return true;
        if (Node.isAmbientable(this) && this.isAmbient()) return true;
        return false;
      }
    }
    setIsExported(value) {
...
class Printer {
  printTextOrWriterFunc(writer, textOrWriterFunc) {
    if (typeof textOrWriterFunc === "string") writer.write(textOrWriterFunc);
    else if (textOrWriterFunc != null) textOrWriterFunc(writer);
  }
  getNewWriter(writer) {
  ...
  getTextWithQueuedChildIndentation(writer, textOrWriterFunc) {
    const queuedChildIndentationWriter =
      this.getNewWriterWithQueuedChildIndentation(writer);
...
class ModifierableNodeStructurePrinter extends Printer {
  printText(writer, structure) {
    ...
    if (structure.isDefaultExport) writer.write("export default ");
    else if (structure.isExported) writer.write("export ");
    if (structure.hasDeclareKeyword) writer.write("declare ");
    if (scope != null) writer.write(`${scope} `);
    if (structure.isStatic) writer.write("static ");
    if (structure.hasOverrideKeyword) writer.write("override ");
    if (structure.isAbstract) writer.write("abstract ");
    if (structure.isAsync) writer.write("async ");
    if (structure.isReadonly) writer.write("readonly ");
    if (structure.hasAccessorKeyword) writer.write("accessor ");
  }
}
...
class ReturnTypedNodeStructurePrinter extends Printer {
  ...
  printText(writer, structure) {
    ...
    if (returnType == null && this.#alwaysWrite === false) return;
...
class TypedNodeStructurePrinter extends Printer {
  ...
  printText(writer, structure) {
    let { type } = structure;
    if (type == null && this.#alwaysWrite === false) return;
    type = type ?? "any";
...
class CommaSeparatedStructuresPrinter extends Printer {
  ...
  printText(writer, structures) {
    printTextWithSeparator(this.#printer, writer, structures, () =>
      writer.spaceIfLastNot(),
    );
  }
}
function printTextWithSeparator(printer, writer, structures, separator) {
  ...
  if (structures instanceof Function || typeof structures === "string")
    ...
    for (let i = commaAppendPositions.length - 1; i >= 0; i--) {
      ...
      else if (!foundFirst) foundFirst = true;
      else writer.unsafeInsert(pos, ",");
    }
  }
}
...
class GetAndSetAccessorStructurePrinter {
  ...
  printGetAndSet(writer, getAccessors, setAccessors, isAmbient) {
    ...
    for (const getAccessor of getAccessors) {
      ...
      const setAccessorIndex = setAccessors.findIndex(
        (item) => item.name === getAccessor.name,
      );
      if (setAccessorIndex >= 0) {
        this.#conditionalSeparator(writer, isAmbient);
        this.#setAccessorPrinter.printText(
          writer,
          setAccessors[setAccessorIndex],
        );
  ...
  #conditionalSeparator(writer, isAmbient) {
    if (writer.isAtStartOfFirstLineOfBlock()) return;
    if (isAmbient) writer.newLine();
...
class ClassDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.inlineBlock(() => {
      this.factory
        ...
        .printTexts(writer, structure.properties);
      ...
      if (!common.ArrayUtils.isNullOrEmpty(structure.methods)) {
        this.#conditionalSeparator(writer, isAmbient);
        this.factory
          .forMethodDeclaration({ isAmbient })
          .printTexts(writer, structure.methods);
      }
    });
  }
  #printHeader(writer, structure) {
    ...
    this.factory
      ...
      .printTextsWithBrackets(writer, structure.typeParameters);
    writer.space();
    writer.hangingIndent(() => {
      ...
      if (structure.implements != null) {
        const implementsText =
          structure.implements instanceof Array
            ? structure.implements
                .map((i) => this.getText(writer, i))
                .join(", ")
  ...
  #printCtors(writer, structure, isAmbient) {
    if (common.ArrayUtils.isNullOrEmpty(structure.ctors)) return;
    for (const ctor of structure.ctors) {
      this.#conditionalSeparator(writer, isAmbient);
      this.factory
        .forConstructorDeclaration({ isAmbient })
        .printText(writer, ctor);
    }
  }
  #printStaticBlocks(writer, structure) {
    if (common.ArrayUtils.isNullOrEmpty(structure.staticBlocks)) return;
  ...
  #printGetAndSet(writer, structure, isAmbient) {
    ...
    const combinedPrinter = new GetAndSetAccessorStructurePrinter(
      getAccessorWriter,
      setAccessorWriter,
    );
    combinedPrinter.printGetAndSet(
      writer,
      structure.getAccessors,
      structure.setAccessors,
      isAmbient,
    );
  }
  #conditionalSeparator(writer, isAmbient) {
...
((StructureKind) => {
  ...
  StructureKind[(StructureKind["ConstructSignature"] = 4)] =
    "ConstructSignature";
  ...
  StructureKind[(StructureKind["ExportDeclaration"] = 11)] =
    "ExportDeclaration";
  ...
  StructureKind[(StructureKind["ImportDeclaration"] = 16)] =
    "ImportDeclaration";
  ...
  StructureKind[(StructureKind["PropertyAssignment"] = 32)] =
    "PropertyAssignment";
  StructureKind[(StructureKind["PropertySignature"] = 33)] =
    "PropertySignature";
  ...
  StructureKind[(StructureKind["VariableDeclaration"] = 40)] =
    "VariableDeclaration";
  StructureKind[(StructureKind["VariableStatement"] = 41)] =
    "VariableStatement";
...
function forParameteredNode(structure, callback) {
  return forAll(
    ...
    callback,
    exports.StructureKind.Parameter,
  );
}
function forTypeParameteredNode(structure, callback) {
  return forAllIfStructure(
    ...
    callback,
    exports.StructureKind.TypeParameter,
  );
}
...
function forClassLikeDeclarationBase(structure, callback) {
  return (
    ...
    forAll(
      structure.staticBlocks,
      callback,
      exports.StructureKind.ClassStaticBlock,
    ) ||
    forAll(structure.properties, callback, exports.StructureKind.Property) ||
    forAll(
      structure.getAccessors,
      callback,
      exports.StructureKind.GetAccessor,
    ) ||
    forAll(
      structure.setAccessors,
      callback,
      exports.StructureKind.SetAccessor,
    ) ||
...
function forDecoratableNode(structure, callback) {
  return forAll(
    ...
    callback,
    exports.StructureKind.Decorator,
  );
}
...
function forConstructorDeclaration(structure, callback) {
  return (
    ...
    forAll(
      structure.overloads,
      callback,
      exports.StructureKind.ConstructorOverload,
    )
  );
}
...
function forExportDeclaration(structure, callback) {
  return (
    forAllIfStructure(
      ...
      callback,
      exports.StructureKind.ExportSpecifier,
    ) ||
    forAll(
      structure.attributes,
      callback,
      exports.StructureKind.ImportAttribute,
    )
  );
}
...
function forFunctionDeclaration(structure, callback) {
  return (
    ...
    forAll(
      structure.overloads,
      callback,
      exports.StructureKind.FunctionOverload,
    )
  );
}
...
function forImportDeclaration(structure, callback) {
  return (
    forAllIfStructure(
      ...
      callback,
      exports.StructureKind.ImportSpecifier,
    ) ||
    forAll(
      structure.attributes,
      callback,
...
function forTypeElementMemberedNode(structure, callback) {
  return (
    forAll(
      ...
      callback,
      exports.StructureKind.CallSignature,
    ) ||
    forAll(
    ...
    forAll(
      structure.methods,
      callback,
      exports.StructureKind.MethodSignature,
    ) ||
    forAll(
      structure.properties,
      callback,
      exports.StructureKind.PropertySignature,
    ) ||
...
function forJsxElement(structure, callback) {
  return (
    forAllUnknownKindIfStructure(structure.attributes, callback) ||
...
function forVariableStatement(structure, callback) {
  return (
    ...
    forAll(
      ...
      callback,
      exports.StructureKind.VariableDeclaration,
    )
  );
}
...
function ensureKind(structure, kind) {
  if (structure.kind == null) structure.kind = kind;
  return structure;
}
...
function isLastNonWhitespaceCharCloseBrace(writer) {
  return (
    writer.iterateLastCharCodes((charCode) => {
      if (charCode === CharCodes.CLOSE_BRACE) return true;
...
class ClassMemberStructurePrinter extends Printer {
  ...
  printText(writer, member) {
    ...
    switch (member.kind) {
      case exports.StructureKind.Method:
        if (!this.#options.isAmbient) ensureBlankLine();
        this.#factory
          .forMethodDeclaration(this.#options)
          .printText(writer, member);
        break;
      case exports.StructureKind.Property:
      ...
      case exports.StructureKind.GetAccessor:
        if (!this.#options.isAmbient) ensureBlankLine();
        this.#factory
          .forGetAccessorDeclaration(this.#options)
          .printText(writer, member);
        break;
      case exports.StructureKind.SetAccessor:
        if (!this.#options.isAmbient) ensureBlankLine();
        this.#factory
          .forSetAccessorDeclaration(this.#options)
          .printText(writer, member);
        break;
      case exports.StructureKind.Constructor:
        if (!this.#options.isAmbient) ensureBlankLine();
        this.#factory
          .forConstructorDeclaration(this.#options)
          .printText(writer, member);
        break;
      case exports.StructureKind.ClassStaticBlock:
        ensureBlankLine();
    ...
    function ensureBlankLine() {
      if (!writer.isAtStartOfFirstLineOfBlock()) writer.blankLineIfLastNot();
    }
  }
}
...
class ClassStaticBlockDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.space().inlineBlock(() => {
      this.factory
        .forStatementedNode({ isAmbient: false })
        .printText(writer, structure);
    });
  }
}
...
class ConstructorDeclarationStructurePrinter extends NodePrinter {
  ...
  printTexts(writer, structures) {
    ...
    for (let i = 0; i < structures.length; i++) {
      if (i > 0) {
        if (this.#options.isAmbient) writer.newLine();
        else writer.blankLine();
      }
      this.printText(writer, structures[i]);
    }
  ...
  printTextInternal(writer, structure) {
    ...
    this.#printHeader(writer, structure);
    if (this.#options.isAmbient) writer.write(";");
    else {
      writer.space().inlineBlock(() => {
        this.factory
          .forStatementedNode(this.#options)
          .printText(writer, structure);
      });
    }
  ...
  #printHeader(writer, structure) {
    ...
    this.factory
      ...
      .printTextsWithParenthesis(writer, structure.parameters);
  }
}
...
class GetAccessorDeclarationStructurePrinter extends NodePrinter {
  ...
  constructor(factory, options) {
    super(factory);
    this.#options = options;
    this.#multipleWriter = this.#options.isAmbient
      ? new NewLineFormattingStructuresPrinter(this)
      : new BlankLineFormattingStructuresPrinter(this);
  }
  printTexts(writer, structures) {
    if (structures != null) this.#multipleWriter.printText(writer, structures);
  }
  printTextInternal(writer, structure) {
    this.factory.forJSDoc().printDocs(writer, structure.docs);
    this.factory.forDecorator().printTexts(writer, structure.decorators);
    ...
    if (this.#options.isAmbient || structure.isAbstract) writer.write(";");
    else {
...
class DecoratorStructurePrinter extends NodePrinter {
  ...
  #printArguments(writer, structure) {
    if (structure.arguments == null) return;
    writer.write("(");
    const args =
      structure.arguments instanceof Array
        ? structure.arguments
        : [structure.arguments];
...
class JSDocStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    function getText(jsdocPrinter) {
      ...
      if (typeof structure === "function") structure(tempWriter);
      else {
        if (structure.description)
          printTextFromStringOrWriter(tempWriter, structure.description);
        if (structure.tags && structure.tags.length > 0) {
          if (tempWriter.getLength() > 0) tempWriter.newLine();
          jsdocPrinter.factory
            .forJSDocTag({ printStarsOnNewLine: false })
...
class JSDocTagStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (i > 0) {
        writer.newLine();
        if (this.#options.printStarsOnNewLine) writer.write(` *`);
      }
      if (lines[i].length > 0) {
        if (this.#options.printStarsOnNewLine && i > 0) writer.space();
      ...
      if (typeof structure === "function") structure(tempWriter);
        ...
        tempWriter.unsafeInsert(
          ...
          `@${structure.tagName}` +
            ...
            !common.StringUtils.startsWithNewLine(currentText)
              ? " "
              : ""),
        );
      }
...
class EnumMemberStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    if (
      ...
      common.StringUtils.isQuoted(structure.name)
    )
      writer.write(structure.name);
    else writer.quote(structure.name);
...
class FunctionDeclarationStructurePrinter extends NodePrinter {
  ...
  #printHeader(writer, name, structure) {
    ...
    writer.conditionalWrite(structure.isGenerator, "*");
    if (!common.StringUtils.isNullOrWhitespace(name)) writer.write(` ${name}`);
    this.factory
      .forTypeParameterDeclaration()
      .printTextsWithBrackets(writer, structure.typeParameters);
    this.factory
...
class ParameterDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.conditionalWrite(structure.hasQuestionToken, "?");
    this.factory
      .forTypedNode(":", structure.hasQuestionToken)
      .printText(writer, structure);
    this.factory
      .forInitializerExpressionableNode()
      .printText(writer, structure);
...
class InterfaceDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    if (structure.extends != null) {
      const extendsText =
        structure.extends instanceof Array
...
class TypeElementMemberedNodeStructurePrinter extends Printer {
  ...
  #conditionalSeparator(writer, structures) {
    if (
      !common.ArrayUtils.isNullOrEmpty(structures) &&
  ...
  printTextInternal(writer, structure) {
    if (isJsxAttribute())
      ...
      this.factory
        ...
        .printTextWithoutTrivia(writer, structure);
    else throw common.errors.throwNotImplementedForNeverValueError(structure);
    function isJsxAttribute(struct) {
      return (
        structure.kind == null ||
        structure.kind === exports.StructureKind.JsxAttribute
      );
    }
  }
}
...
class JsxAttributeStructurePrinter extends NodePrinter {
  printTextInternal(writer, structure) {
    ...
    else writer.write(structure.name);
...
class JsxChildDeciderStructurePrinter extends NodePrinter {
  printTextInternal(writer, structure) {
    ...
    else common.errors.throwNotImplementedForNeverValueError(structure);
    function isJsxElement(struct) {
      return (
        struct.kind == null || struct.kind === exports.StructureKind.JsxElement
      );
    }
  }
}
...
class ExportAssignmentStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    this.factory.forJSDoc().printDocs(writer, structure.docs);
    writer.write("export");
    if (structure.isExportEquals !== false) writer.write(" = ");
...
class ExportDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    const hasModuleSpecifier =
      structure.moduleSpecifier != null && structure.moduleSpecifier.length > 0;
    const hasNamedImport =
      structure.namedExports != null && structure.namedExports.length > 0;
    ...
    writer.write("export");
    if (structure.isTypeOnly) writer.write(" type");
    if (structure.namedExports != null && structure.namedExports.length > 0) {
      writer.space();
      this.factory
        .forNamedImportExportSpecifier()
        .printTextsWithBraces(writer, structure.namedExports);
    ...
    if (structure.attributes) {
      ...
      this.factory
        .forImportAttribute()
        .printAttributes(writer, structure.attributes);
    }
    writer.write(";");
  }
}
...
class ImportDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    const hasNamedImport =
      structure.namedImports != null && structure.namedImports.length > 0;
    ...
    if (structure.namedImports != null && structure.namedImports.length > 0) {
      ...
      this.factory
        .forNamedImportExportSpecifier()
        .printTextsWithBraces(writer, structure.namedImports);
    }
    writer.conditionalWrite(
      structure.defaultImport != null ||
        hasNamedImport ||
...
class ModuleDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    if (
      ...
      structure.declarationKind !== exports.ModuleDeclarationKind.Global
    )
      writer.write(
        `${structure.declarationKind || "namespace"} ${structure.name}`,
      );
    else writer.write("global");
    if (
      structure.hasDeclareKeyword &&
      common.StringUtils.isQuoted(structure.name.trim()) &&
      Object.hasOwn(structure, common.nameof(structure, "statements")) &&
  ...
  #validateAndGetStructure(structure) {
    if (common.StringUtils.isQuoted(structure.name.trim())) {
      if (
        structure.declarationKind === exports.ModuleDeclarationKind.Namespace
      ) {
        throw new common.errors.InvalidOperationError(
...
class NamedImportExportSpecifierStructurePrinter extends NodePrinter {
  ...
  printTextsWithBraces(writer, structures) {
    ...
    if (
      ...
      !common.StringUtils.startsWithNewLine(specifierText)
    )
      writer.space();
    writer.write(specifierText);
    if (
      formatSettings.insertSpaceAfterOpeningAndBeforeClosingNonemptyBraces &&
      !common.StringUtils.endsWithNewLine(specifierText)
    )
      writer.space();
    writer.write("}");
  }
  printTexts(writer, structures) {
    if (structures instanceof Function) this.printText(writer, structures);
    else this.#multipleWriter.printText(writer, structures);
  }
  printTextInternal(writer, structure) {
    const specifierWriter = this.getNewWriterWithQueuedChildIndentation(writer);
    if (typeof structure === "string") specifierWriter.write(structure);
    else if (structure instanceof Function) structure(specifierWriter);
    else {
      if (structure.isTypeOnly) writer.write("type ");
      specifierWriter.write(structure.name);
      if (!common.StringUtils.isNullOrWhitespace(structure.alias)) {
        if (!specifierWriter.isLastNewLine()) specifierWriter.space();
...
class StatementedNodeStructurePrinter extends Printer {
  ...
  printText(writer, structure) {
    this.#factory
      .forStatement(this.#options)
      .printTexts(writer, structure.statements);
  }
}
  ...
  printText(writer, statement) {
    ...
    switch (statement.kind) {
      case exports.StructureKind.Function:
        if (!this.#options.isAmbient) ensureBlankLine();
        this.#factory
          .forFunctionDeclaration(this.#options)
          .printText(writer, statement);
        break;
      case exports.StructureKind.Class:
        ensureBlankLine();
        this.#factory
          .forClassDeclaration(this.#options)
          .printText(writer, statement);
        break;
      case exports.StructureKind.Interface:
        ensureBlankLine();
      ...
      case exports.StructureKind.Module:
        ensureBlankLine();
        this.#factory
          .forModuleDeclaration(this.#options)
          .printText(writer, statement);
        break;
      case exports.StructureKind.Enum:
        ensureBlankLine();
...
class VariableStatementStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.hangingIndent(() => {
      this.factory.forModifierableNode().printText(writer, structure);
      writer.write(
        `${structure.declarationKind || exports.VariableDeclarationKind.Let} `,
      );
      ...
      this.factory
        ...
        .printTexts(writer, structure.declarations);
      writer.write(";");
    });
  }
}
...
class TypeParameterDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.hangingIndent(() => {
      if (structure.isConst) writer.write("const ");
  ...
  return class extends Base {
    getExtends() {
      const extendsClause = this.getHeritageClauseByKind(
        common.SyntaxKind.ExtendsKeyword,
      );
    ...
    insertExtends(index, texts) {
      ...
      if (originalExtends.length > 0) {
        ...
        if (!isLastSpace) insertText = " " + insertText;
        insertIntoParentTextRange({
          parent: this,
  ...
  return class extends Base {
    ...
    getAsteriskTokenOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find an asterisk token.",
        this,
      );
    }
...
function getAsteriskInsertPos(node) {
  if (node.getKind() === common.SyntaxKind.FunctionDeclaration)
    return node
      .getFirstChildByKindOrThrow(common.SyntaxKind.FunctionKeyword)
      .getEnd();
...
function HeritageClauseableNode(Base) {
  return class extends Base {
    getHeritageClauses() {
      ...
      return (
        heritageClauses?.map((c) => this._getNodeFromCompilerNode(c)) ?? []
      );
    }
    getHeritageClauseByKindOrThrow(kind, message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ??
          (() =>
            `Expected to have heritage clause of kind ${common.getSyntaxKindName(kind)}.`),
        this,
      );
    }
    getHeritageClauseByKind(kind) {
      return this.getHeritageClauses().find(
        (c) => c.compilerNode.token === kind,
      );
    }
  };
}
  ...
  return class extends Base {
    getImplements() {
      const implementsClause = this.getHeritageClauseByKind(
        common.SyntaxKind.ImplementsKeyword,
      );
  ...
  return class extends Base {
    ...
    getInitializerIfKindOrThrow(kind, message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ??
          `Expected to find an initializer of kind '${common.getSyntaxKindName(kind)}'.`,
        this,
      );
    }
    ...
    getInitializerOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getInitializer(),
        message ?? "Expected to find an initializer.",
        this,
      );
    }
    getInitializer() {
...
function apply(Base) {
  return class extends Base {
    ...
    setInitializer(textOrWriterFunction) {
      ...
      if (this.hasInitializer()) this.removeInitializer();
      const semiColonToken = this.getLastChildIfKind(
        common.SyntaxKind.SemicolonToken,
      );
      insertIntoParentTextRange({
        insertPos:
          semiColonToken != null ? semiColonToken.getPos() : this.getEnd(),
        parent: this,
        newText: ` = ${text}`,
      });
      return this;
    ...
    set(structure) {
      ...
      if (structure.initializer != null)
        ...
        Object.hasOwn(structure, common.nameof(structure, "initializer"))
      )
...
function JSDocableNode(Base) {
  return class extends Base {
    ...
    insertJsDocs(index, structures) {
      ...
      const insertPos =
        index === nodes.length ? this.getStart() : nodes[index].getStart();
      insertIntoParentTextRange({
        insertPos,
        parent: this,
        newText: code,
  ...
  return class extends Base {
    ...
    getFirstModifierByKindOrThrow(kind, message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ??
          (() =>
            `Expected a modifier of syntax kind: ${common.getSyntaxKindName(kind)}`),
        this,
      );
    }
    ...
    toggleModifier(text, value) {
      if (value == null) value = !this.hasModifier(text);
      if (value) this.addModifier(text);
      else this.removeModifier(text);
      return this;
    }
    addModifier(text) {
      const rawModifiers = this.getModifiers();
      const modifiers = this.getModifiers().filter(
        (m) => m.getKind() !== common.SyntaxKind.Decorator,
      );
      const existingModifier = modifiers.find((m) => m.getText() === text);
      if (existingModifier != null) return existingModifier;
      const insertPos = getInsertPos(this);
      let startPos;
      let newText;
      const isFirstModifier =
        modifiers.length === 0 || insertPos === modifiers[0].getStart();
      if (isFirstModifier) {
        newText = text + " ";
        startPos = insertPos;
      } else {
      ...
      function getInsertPos(node) {
        ...
        for (const addAfterText of getAddAfterModifierTexts(text)) {
          for (let i = 0; i < modifiers.length; i++) {
            ...
            if (modifier.getText() === addAfterText) {
              if (pos < modifier.getEnd()) pos = modifier.getEnd();
              break;
            }
          }
        }
        ...
        function getInitialInsertPos() {
          if (modifiers.length > 0) return modifiers[0].getStart();
          if (node.getKind() === common.SyntaxKind.ArrowFunction)
            return node.getStart();
          for (const child of node._getChildrenIterator()) {
            if (
              child.getKind() === common.SyntaxKind.SyntaxList ||
              common.ts.isJSDocCommentContainingNode(child.compilerNode)
            )
              continue;
  ...
  return class extends Base {
    ...
    getImportDeclaration(conditionOrModuleSpecifier) {
      ...
      function getCondition() {
        ...
        else return conditionOrModuleSpecifier;
      }
    }
    ...
    getExportedDeclarations() {
      ...
      for (const symbol of exportSymbols) {
        for (const declaration of symbol.getDeclarations()) {
          ...
          if (existingArray != null) existingArray.push(...declarations);
          else result.set(symbol.getName(), declarations);
        }
      }
      return result;
      function* getDeclarationHandlingImportsAndExports(declaration) {
        if (Node.isExportSpecifier(declaration)) {
          ...
          if (
            expression == null ||
            expression.getKind() !== common.SyntaxKind.Identifier
          ) {
            yield expression;
            return;
          }
    ...
    removeDefaultExport(defaultExportSymbol) {
      defaultExportSymbol =
        defaultExportSymbol || this.getDefaultExportSymbol();
      if (defaultExportSymbol == null) return this;
...
function getNodeForReferences(node) {
  ...
  if (Node.isExportable(node)) return node.getDefaultKeyword() || node;
  return node;
}
  ...
  return class extends Base {
    rename(newName, options) {
      ...
      function getNodeToRename(thisNode) {
        if (
          Node.isIdentifier(thisNode) ||
          Node.isPrivateIdentifier(thisNode) ||
          Node.isStringLiteral(thisNode)
        )
          return thisNode;
        else if (thisNode.getNameNode != null) {
          const node = thisNode.getNameNode();
          common.errors.throwIfNullOrUndefined(
            node,
...
function NameableNodeInternal(Base) {
  return class extends Base {
    ...
    getNameNodeOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getNameNode(),
        message ?? "Expected to have a name node.",
        this,
      );
    }
    getName() {
    ...
    getNameOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getName(),
        message ?? "Expected to have a name.",
        this,
      );
    }
    rename(newName) {
      if (newName === this.getName()) return this;
      if (common.StringUtils.isNullOrWhitespace(newName)) {
        this.removeName();
        return this;
      }
      const nameNode = this.getNameNode();
      if (nameNode == null) addNameNode(this, newName);
    ...
    set(structure) {
      callBaseSet(Base.prototype, this, structure);
      if (structure.name != null) {
        common.errors.throwIfWhitespaceOrNotString(
          structure.name,
          "structure.name",
        );
        const nameNode = this.getNameNode();
        if (nameNode == null) addNameNode(this, structure.name);
        else nameNode.replaceWithText(structure.name);
      } else if (Object.hasOwn(structure, common.nameof(structure, "name"))) {
        this.removeName();
      }
      return this;
    }
...
function addNameNode(node, newName) {
  if (Node.isClassDeclaration(node) || Node.isClassExpression(node)) {
    const classKeyword = node.getFirstChildByKindOrThrow(
      common.SyntaxKind.ClassKeyword,
    );
    insertIntoParentTextRange({
  ...
  return class extends Base {
    ...
    getOverrideKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find an override keyword.",
        this,
      );
    }
...
function ParameteredNode(Base) {
  return class extends Base {
    ...
    insertParameters(index, structures) {
      ...
      const writer = this._getWriterWithQueuedChildIndentation();
      const structurePrinter =
        this._context.structurePrinterFactory.forParameterDeclaration();
  ...
  return class extends Base {
    ...
    getQuestionDotTokenNodeOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a question dot token.",
        this,
      );
    }
    setHasQuestionDotToken(value) {
      ...
      if (value) {
        ...
        else removeChildren({ children: [questionDotTokenNode] });
      }
      return this;
      function getInsertPos() {
        if (Node.isCallExpression(this))
          return this.getFirstChildByKindOrThrow(
            common.SyntaxKind.OpenParenToken,
          ).getStart();
        if (Node.isElementAccessExpression(this))
          return this.getFirstChildByKindOrThrow(
            common.SyntaxKind.OpenBracketToken,
          ).getStart();
        common.errors.throwNotImplementedForSyntaxKindError(
  ...
  return class extends Base {
    ...
    getQuestionTokenNodeOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a question token.",
        this,
      );
    }
    setHasQuestionToken(value) {
      ...
      if (value === hasQuestionToken) return this;
      if (value) {
      ...
      return this;
      function getInsertPos() {
        if (Node.hasName(this)) return this.getNameNode().getEnd();
        const colonNode = this.getFirstChildByKind(
          common.SyntaxKind.ColonToken,
        );
        if (colonNode != null) return colonNode.getStart();
        const semicolonToken = this.getLastChildByKind(
          common.SyntaxKind.SemicolonToken,
        );
        if (semicolonToken != null) return semicolonToken.getStart();
        return this.getEnd();
      }
    }
    set(structure) {
  ...
  return class extends Base {
    ...
    getReadonlyKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a readonly keyword.",
        this,
      );
    }
    setIsReadonly(value) {
  ...
  return class extends Base {
    ...
    getReturnTypeNodeOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a return type node.",
        this,
      );
    }
      ...
      function getEndNode(thisNode) {
        if (thisNode.getKind() === common.SyntaxKind.IndexSignature)
          return thisNode.getFirstChildByKindOrThrow(
            common.SyntaxKind.CloseBracketToken,
          );
        return thisNode.getFirstChildByKindOrThrow(
          common.SyntaxKind.CloseParenToken,
        );
      }
    }
    removeReturnType() {
      const returnTypeNode = this.getReturnTypeNode();
      if (returnTypeNode == null) return this;
    ...
    set(structure) {
      ...
      else if (Object.hasOwn(structure, common.nameof(structure, "returnType")))
...
function ScopeableNode(Base) {
  return class extends Base {
    ...
    set(structure) {
      callBaseSet(Base.prototype, this, structure);
      if (Object.hasOwn(structure, common.nameof(structure, "scope")))
  ...
  return class extends Base {
    ...
    getStaticKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find a static keyword.",
        this,
      );
    }
    setIsStatic(value) {
    ...
    set(structure) {
      callBaseSet(Base.prototype, this, structure);
      if (structure.isStatic != null) this.setIsStatic(structure.isStatic);
      return this;
    }
    getStructure() {
      return callBaseGetStructure(Base.prototype, this, {
...
function TextInsertableNode(Base) {
  return class extends Base {
    ...
    removeText(pos, end) {
      if (pos == null) this.replaceText(getValidRange(this), "");
      else this.replaceText([pos, end], "");
      return this;
    }
...
function getValidRange(thisNode) {
  ...
  const openBrace = Node.isSourceFile(rangeNode)
    ? undefined
    : rangeNode.getPreviousSiblingIfKind(common.SyntaxKind.OpenBraceToken);
  const closeBrace =
    openBrace == null
      ? undefined
      : rangeNode.getNextSiblingIfKind(common.SyntaxKind.CloseBraceToken);
  if (openBrace != null && closeBrace != null)
    return [openBrace.getEnd(), closeBrace.getStart()];
  else return [rangeNode.getPos(), rangeNode.getEnd()];
  function getRangeNode() {
    if (Node.isSourceFile(thisNode)) return thisNode;
...
function TypeArgumentedNode(Base) {
  return class extends Base {
    getTypeArguments() {
      if (this.compilerNode.typeArguments == null) return [];
    ...
    insertTypeArguments(index, argumentTexts) {
      ...
      if (typeArguments.length === 0) {
        ...
        insertIntoCommaSeparatedNodes({
          parent: this.getFirstChildByKindOrThrow(
            common.SyntaxKind.LessThanToken,
          ).getNextSiblingIfKindOrThrow(common.SyntaxKind.SyntaxList),
    ...
    removeTypeArgument(typeArgOrIndex) {
      ...
      const typeArgToRemove =
        typeof typeArgOrIndex === "number"
        ...
        removeChildren({
          children: [
            ...
            childSyntaxList.getNextSiblingIfKindOrThrow(
              common.SyntaxKind.GreaterThanToken,
            ),
          ],
        });
      } else {
...
function TypedNode(Base) {
  return class extends Base {
    ...
    getTypeNodeOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getTypeNode(),
        message ?? "Expected to find a type node.",
        this,
      );
    }
    setType(textOrWriterFunction) {
      ...
      if (common.StringUtils.isNullOrWhitespace(text)) return this.removeType();
      const typeNode = this.getTypeNode();
      if (typeNode != null && typeNode.getText() === text) return this;
      ...
      if (separatorNode == null) {
        ...
        newText =
          (separatorSyntaxKind === common.SyntaxKind.EqualsToken
            ? " = "
            : ": ") + text;
      } else {
      ...
      function getInsertPosWhenNoType(node) {
        const identifier =
          node.getFirstChildByKind(common.SyntaxKind.Identifier) ??
          node.getFirstChildByKind(common.SyntaxKind.ArrayBindingPattern) ??
      ...
      function isQuestionOrExclamation(node) {
        ...
        const kind = node.getKind();
        return (
          kind === common.SyntaxKind.QuestionToken ||
          kind === common.SyntaxKind.ExclamationToken
        );
      }
    }
    set(structure) {
      callBaseSet(Base.prototype, this, structure);
      if (structure.type != null) this.setType(structure.type);
      else if (Object.hasOwn(structure, common.nameof(structure, "type")))
        this.removeType();
      return this;
    }
    removeType() {
      if (this.getKind() === common.SyntaxKind.TypeAliasDeclaration)
        throw new common.errors.NotSupportedError(
          `Cannot remove the type of a type alias. Use ${common.nameof("setType")} instead.`,
        );
      const typeNode = this.getTypeNode();
      if (typeNode == null) return this;
  ...
  return class extends Base {
    ...
    insertMembers(index, members) {
      return insertIntoBracesOrSourceFileWithGetChildrenWithComments({
        ...
        write: (writer) => {
          ...
          const memberPrinter =
            this._context.structurePrinterFactory.forTypeElementMember();
    ...
    getConstructSignatures() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.ConstructSignature)
    ...
    getCallSignatures() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.CallSignature)
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    ...
    getIndexSignatures() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.IndexSignature)
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    addMethod(structure) {
    ...
    getMethods() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.MethodSignature)
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    ...
    getProperties() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.PropertySignature)
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    ...
    getGetAccessors() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.GetAccessor)
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    ...
    getSetAccessors() {
      return this.compilerNode.members
        .filter((m) => m.kind === common.SyntaxKind.SetAccessor)
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    getMembers() {
...
function TypeParameteredNode(Base) {
  return class extends Base {
    ...
    insertTypeParameters(index, structures) {
      ...
      const writer = this._getWriterWithQueuedChildIndentation();
      const structurePrinter =
        this._context.structurePrinterFactory.forTypeParameterDeclaration();
...
function getInsertPos(node) {
  const namedNode = node;
  if (namedNode.getNameNode != null) return namedNode.getNameNode().getEnd();
  else if (
    Node.isCallSignatureDeclaration(node) ||
    Node.isFunctionTypeNode(node)
  )
    return node
      .getFirstChildByKindOrThrow(common.SyntaxKind.OpenParenToken)
      .getStart();
  else
    throw new common.errors.NotImplementedError(
...
class BindingElement extends BindingElementBase {
  getPropertyNameNodeOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a property name node.",
      this,
    );
  }
  getPropertyNameNode() {
  ...
  return class extends Base {
    ...
    getAbstractKeywordOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find an abstract keyword.",
        this,
      );
    }
...
class ArrayLiteralExpression extends PrimaryExpression {
  ...
  insertElements(index, textsOrWriterFunction, options = {}) {
    ...
    function getUseNewLines(node) {
      if (options.useNewLines != null) return options.useNewLines;
      if (elements.length > 1) return allElementsOnDifferentLines();
...
function ExpressionableNode(Base) {
  return class extends Base {
    ...
    getExpressionOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getExpression(),
        message ?? "Expected to find an expression.",
        this,
      );
    }
    ...
    getExpressionIfKindOrThrow(kind, message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ??
          `An expression with the kind kind ${common.getSyntaxKindName(kind)} was expected.`,
        this,
      );
    }
  };
  ...
  return class extends Base {
    ...
    getExpressionIfKindOrThrow(kind, message) {
      return common.errors.throwIfNullOrUndefined(
        this.getExpressionIfKind(kind),
        message ??
          `An expression of the kind ${common.getSyntaxKindName(kind)} was expected.`,
        this,
      );
    }
...
class ElementAccessExpression extends ElementAccessExpressionBase {
  ...
  getArgumentExpressionOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an argument expression.",
      this,
    );
  }
}
...
class ObjectLiteralExpression extends ObjectLiteralExpressionBase {
  ...
  getProperty(nameOrFindFunction) {
    ...
    if (typeof nameOrFindFunction === "string") {
      findFunc = (prop) => {
        if (prop[common.nameof("getName")] == null) return false;
  ...
  insertProperties(index, structures) {
    ...
    const structurePrinter =
      this._context.structurePrinterFactory.forObjectLiteralExpressionProperty();
    structurePrinter.printTexts(writer, structures);
    insertIntoCommaSeparatedNodes({
  ...
  insertPropertyAssignments(index, structures) {
    return this.#insertProperty(index, structures, () =>
...
class PropertyAssignment extends PropertyAssignmentBase {
  removeInitializer() {
    ...
    const newText =
      sourceFileText.substring(insertPos, colonToken.getPos()) +
  ...
  set(structure) {
    ...
    if (structure.initializer != null)
      this.setInitializer(structure.initializer);
    else if (Object.hasOwn(structure, common.nameof(structure, "initializer")))
...
class ShorthandPropertyAssignment extends ShorthandPropertyAssignmentBase {
  ...
  getObjectAssignmentInitializerOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an object assignment initializer.",
      this,
    );
  }
  ...
  setInitializer(text) {
    ...
    return parent.getChildAtIndexIfKindOrThrow(
      childIndex,
      common.SyntaxKind.PropertyAssignment,
    );
  }
  set(structure) {
  ...
  getValueSymbolOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getValueSymbol(),
      message ?? "Expected to find a value symbol.",
      this,
    );
  }
}
...
function StatementedNode(Base) {
  return class extends Base {
    ...
    getStatementByKind(kind) {
      const statement = this._getCompilerStatementsWithComments().find(
        (s) => s.kind === kind,
      );
    ...
    getStatementByKindOrThrow(kind, message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ??
          `Expected to find a statement with syntax kind ${common.getSyntaxKindName(kind)}.`,
        this,
      );
    }
    ...
    insertStatements(index, statements) {
      ...
      const writerFunction = (writer) => {
        const statementsPrinter =
          this._context.structurePrinterFactory.forStatement({
            isAmbient: isNodeAmbientOrInAmbientContext(this),
          });
        statementsPrinter.printTexts(writer, statements);
      };
      return getChildSyntaxList
        .call(this)
        .insertChildText(index, writerFunction);
      function getChildSyntaxList() {
        const childSyntaxList = this.getChildSyntaxListOrThrow();
        if (Node.isCaseClause(this) || Node.isDefaultClause(this)) {
          const block = childSyntaxList.getFirstChildIfKind(
            common.SyntaxKind.Block,
          );
          if (block != null) return block.getChildSyntaxListOrThrow();
        }
    ...
    removeStatements(indexRange) {
      ...
      common.errors.throwIfRangeOutOfRange(
        indexRange,
        [0, statements.length],
        "indexRange",
      );
    ...
    getFunctions() {
      return this.getStatements()
        .filter(Node.isFunctionDeclaration)
      ...
      function getFindFunction() {
        if (typeof nameOrFindFunction === "string")
          return (statement) =>
            statement
              .getDeclarations()
              .some((d) => nodeHasName(d, nameOrFindFunction));
    ...
    getStructure() {
      ...
      if (Node.isBodyable(this) && !this.hasBody())
        ...
        structure.statements = this.getStatements().map((s) => {
          if (Node._hasStructure(s)) return s.getStructure();
    ...
    set(structure) {
      if (
        ...
        if (statementCount > 0) this.removeStatements([0, statementCount - 1]);
      }
      callBaseSet(Base.prototype, this, structure);
    ...
    _getCompilerStatementsWithComments() {
      const statementsContainer = this._getCompilerStatementsContainer();
      if (statementsContainer == null) return [];
      else {
        return ExtendedParser.getContainerArray(
          statementsContainer,
          this._sourceFile.compilerNode,
    ...
    _standardWrite(writer, info, writeStructures, opts = {}) {
      ...
      if (
        ...
        writer.blankLine();
      else writer.newLineIfLastNot();
    }
  };
}
function addBodyIfNotExists(node) {
  if (Node.isBodyable(node) && !node.hasBody()) node.addBody();
}
...
class BreakStatement extends Statement {
  ...
  getLabelOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getLabel(),
      message ?? "Expected to find a label.",
      this,
    );
  }
}
...
class CatchClause extends CatchClauseBase {
  ...
  getVariableDeclarationOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a variable declaration.",
      this,
    );
  }
}
...
class ForStatement extends ForStatementBase {
  ...
  getConditionOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getCondition(),
      message ?? "Expected to find a condition.",
      this,
    );
  }
  getIncrementor() {
  ...
  getIncrementorOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getIncrementor(),
      message ?? "Expected to find an incrementor.",
      this,
    );
  }
}
...
class IfStatement extends IfStatementBase {
  ...
  remove() {
    ...
    if (Node.isIfStatement(this.getParentOrThrow()))
      nodes.push(
        this.getPreviousSiblingIfKindOrThrow(common.SyntaxKind.ElseKeyword),
      );
    nodes.push(this);
...
class TryStatement extends TryStatementBase {
  ...
  getCatchClauseOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getCatchClause(),
      message ?? "Expected to find a catch clause.",
      this,
    );
  }
  getFinallyBlock() {
  ...
  getFinallyBlockOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a finally block.",
      this,
    );
  }
}
...
class ExportAssignment extends ExportAssignmentBase {
  ...
  setIsExportEquals(value) {
    if (this.isExportEquals() === value) return this;
    if (value)
      this.getFirstChildByKindOrThrow(
        common.SyntaxKind.DefaultKeyword,
      ).replaceWithText("=");
    else
      this.getFirstChildByKindOrThrow(
        common.SyntaxKind.EqualsToken,
  ...
  set(structure) {
    ...
    if (structure.expression != null) this.setExpression(structure.expression);
...
class ExportDeclaration extends ExportDeclarationBase {
  ...
  setIsTypeOnly(value) {
    if (this.isTypeOnly() === value) return this;
    if (value) {
      insertIntoParentTextRange({
        parent: this,
        insertPos: (
          this.getNodeProperty("exportClause") ??
          this.getFirstChildByKindOrThrow(common.SyntaxKind.AsteriskToken)
        ).getStart(),
        newText: "type ",
      });
    } else {
      const typeKeyword = this.getFirstChildByKindOrThrow(
        common.ts.SyntaxKind.TypeKeyword,
      );
      removeChildren({
  ...
  getNamespaceExportOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a namespace export.",
      this,
    );
  }
  setNamespaceExport(name) {
    const exportClause = this.getNodeProperty("exportClause");
    const newText = common.StringUtils.isNullOrWhitespace(name)
      ? "*"
      : `* as ${name}`;
    if (exportClause == null) {
      const asteriskToken = this.getFirstChildByKindOrThrow(
        common.SyntaxKind.AsteriskToken,
      );
      insertIntoParentTextRange({
  ...
  setModuleSpecifier(textOrSourceFile) {
    const text =
      typeof textOrSourceFile === "string"
        ? textOrSourceFile
  ...
  getModuleSpecifier() {
    ...
    if (moduleSpecifier == null) return undefined;
  ...
  getModuleSpecifierSourceFileOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? `A module specifier source file was expected.`,
      this,
    );
  }
  getModuleSpecifierSourceFile() {
    const stringLiteral = this.getLastChildByKind(
  ...
  removeModuleSpecifier() {
    ...
    removeChildren({
      children: [
        this.getFirstChildByKindOrThrow(common.SyntaxKind.FromKeyword),
        moduleSpecifier,
      ],
  ...
  hasNamedExports() {
    return (
      this.compilerNode.exportClause?.kind === common.SyntaxKind.NamedExports
    );
  }
  ...
  insertNamedExports(index, namedExports) {
    if (
      ...
      common.ArrayUtils.isNullOrEmpty(namedExports)
    )
      return [];
    const originalNamedExports = this.getNamedExports();
    const writer = this._getWriterWithIndentation();
    const namedExportStructurePrinter =
      this._context.structurePrinterFactory.forNamedImportExportSpecifier();
    ...
    if (exportClause == null) {
      ...
      insertIntoCommaSeparatedNodes({
        parent: this.getFirstChildByKindOrThrow(
          common.SyntaxKind.NamedExports,
        ).getFirstChildByKindOrThrow(common.SyntaxKind.SyntaxList),
  ...
  toNamespaceExport() {
    ...
    if (namedExportsNode == null) return this;
    insertIntoParentTextRange({
      parent: this,
      newText: "*",
  ...
  setAttributes(elements) {
    ...
    if (attributes) {
      if (elements) attributes.setElements(elements);
      else attributes.remove();
    } else if (elements) {
      const printer =
        this._context.structurePrinterFactory.forImportAttribute();
  ...
  set(structure) {
    ...
    if (structure.namedExports != null) {
      ...
      Object.hasOwn(structure, common.nameof(structure, "namedExports")) &&
    ...
    if (structure.moduleSpecifier != null)
      ...
      Object.hasOwn(structure, common.nameof(structure, "moduleSpecifier"))
    )
      this.removeModuleSpecifier();
    if (
    ...
    if (structure.isTypeOnly != null) this.setIsTypeOnly(structure.isTypeOnly);
    if (Object.hasOwn(structure, common.nameof(structure, "attributes")))
  ...
  getStructure() {
    ...
    return callBaseGetStructure(ExportDeclarationBase.prototype, this, {
      ...
      attributes: attributes
        ? attributes.getElements().map((e) => e.getStructure())
        : undefined,
    });
  }
}
function setEmptyNamedExport(node) {
  ...
  if (namedExportsNode != null) {
    if (node.getNamedExports().length === 0) return;
    replaceNode = namedExportsNode;
  } else {
    replaceNode = node.getFirstChildByKindOrThrow(
      common.SyntaxKind.AsteriskToken,
    );
  }
  insertIntoParentTextRange({
...
class ExportSpecifier extends ExportSpecifierBase {
  setName(name) {
    ...
    if (isValidVariableName(name)) nameNode.replaceWithText(name);
    else nameNode.replaceWithText(`"${name.replaceAll('"', '\\"')}"`);
    return this;
  }
  getName() {
    const nameNode = this.getNameNode();
  ...
  setAlias(alias) {
    ...
    } else if (isValidVariableName(alias)) aliasNode.replaceWithText(alias);
    else aliasNode.replaceWithText(`"${alias.replaceAll('"', '\\"')}"`);
    return this;
  }
  removeAlias() {
    const aliasNode = this.getAliasNode();
    if (aliasNode == null) return this;
    removeChildren({
      children: [
        this.getFirstChildByKindOrThrow(common.SyntaxKind.AsKeyword),
        aliasNode,
      ],
      removePrecedingSpaces: true,
      removePrecedingNewLines: true,
  ...
  getAliasNode() {
    if (this.compilerNode.propertyName == null) return undefined;
    return this._getNodeFromCompilerNode(this.compilerNode.name);
  }
  isTypeOnly() {
    return this.compilerNode.isTypeOnly;
  ...
  setIsTypeOnly(value) {
    ...
    if (value) {
      ...
      removeChildren({
        children: [
          this.getFirstChildByKindOrThrow(common.ts.SyntaxKind.TypeKeyword),
        ],
        removeFollowingSpaces: true,
      });
    }
  ...
  getExportDeclaration() {
    return this.getFirstAncestorByKindOrThrow(
      common.SyntaxKind.ExportDeclaration,
    );
  }
  getLocalTargetSymbolOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getLocalTargetSymbol(),
      message ?? `The export specifier's local target symbol was expected.`,
      this,
    );
  }
  ...
  remove() {
    ...
    if (
      ...
      exportDeclaration.getNamespaceExport() == null
    )
      removeCommaSeparatedChild(this);
    else exportDeclaration.toNamespaceExport();
  }
  set(structure) {
    callBaseSet(ExportSpecifierBase.prototype, this, structure);
    if (structure.isTypeOnly != null) this.setIsTypeOnly(structure.isTypeOnly);
    if (structure.name != null) this.setName(structure.name);
    if (structure.alias != null) this.setAlias(structure.alias);
    else if (Object.hasOwn(structure, common.nameof(structure, "alias")))
      this.removeAlias();
    return this;
  }
  getStructure() {
...
class ExternalModuleReference extends ExternalModuleReferenceBase {
  getReferencedSourceFileOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find the referenced source file.",
      this,
    );
  }
  isRelative() {
    const expression = this.getExpression();
    if (expression == null || !Node.isStringLiteral(expression)) return false;
...
class ImportAttribute extends ImportAttributeBase {
  ...
  set(structure) {
    ...
    if (structure.value) this.getValue().replaceWithText(structure.value);
    return this;
  }
  getStructure() {
...
class ImportClause extends ImportClauseBase {
  ...
  getDefaultImportOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a default import.",
      this,
    );
  }
  getDefaultImport() {
  ...
  getNamedBindingsOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an import declaration's named bindings.",
      this,
    );
  }
  getNamedBindings() {
  ...
  getNamespaceImportOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a namespace import.",
      this,
    );
  }
  getNamespaceImport() {
  ...
  getNamedImports() {
    const namedBindings = this.getNamedBindings();
    if (namedBindings == null || !Node.isNamedImports(namedBindings)) return [];
...
class ImportDeclaration extends ImportDeclarationBase {
  ...
  setDefaultImport(text) {
    ...
    const importKeyword = this.getFirstChildByKindOrThrow(
      common.SyntaxKind.ImportKeyword,
    );
    const importClause = this.getImportClause();
    if (importClause == null) {
      insertIntoParentTextRange({
  ...
  removeNamespaceImport() {
    const namespaceImport = this.getNamespaceImport();
    if (namespaceImport == null) return this;
    removeChildren({
    ...
    function getChildrenToRemove() {
      ...
      if (defaultImport == null)
        return [
          ...
          this.getLastChildByKindOrThrow(common.SyntaxKind.FromKeyword),
        ];
      else
        return [
          defaultImport.getNextSiblingIfKindOrThrow(
            common.SyntaxKind.CommaToken,
          ),
          namespaceImport,
        ];
  ...
  removeDefaultImport() {
    const importClause = this.getImportClause();
    if (importClause == null) return this;
    const defaultImport = importClause.getDefaultImport();
    if (defaultImport == null) return this;
    ...
    if (hasOnlyDefaultImport) {
      if (importClause.isTypeOnly()) {
        ...
        removeChildren({
          children: [
            importClause,
            importClause.getNextSiblingIfKindOrThrow(
              common.SyntaxKind.FromKeyword,
            ),
          ],
          removePrecedingSpaces: true,
          removePrecedingNewLines: true,
  ...
  insertNamedImports(index, namedImports) {
    if (
      ...
      common.ArrayUtils.isNullOrEmpty(namedImports)
    )
      return [];
    const originalNamedImports = this.getNamedImports();
    const writer = this._getWriterWithQueuedIndentation();
    const namedImportStructurePrinter =
      this._context.structurePrinterFactory.forNamedImportExportSpecifier();
    const importClause = this.getImportClause();
    ...
    if (originalNamedImports.length === 0) {
      ...
      insertIntoCommaSeparatedNodes({
        parent: importClause
          .getFirstChildByKindOrThrow(common.SyntaxKind.NamedImports)
          .getFirstChildByKindOrThrow(common.SyntaxKind.SyntaxList),
  ...
  removeNamedImports() {
    ...
    if (
      ...
      namedImportsNode.getKind() !== common.SyntaxKind.NamedImports
    )
      return this;
    const defaultImport = this.getDefaultImport();
    if (defaultImport != null) {
  ...
  getImportClauseOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an import clause.",
      this,
    );
  }
  getImportClause() {
  ...
  set(structure) {
    ...
    if (structure.defaultImport != null)
      ...
      Object.hasOwn(structure, common.nameof(structure, "defaultImport"))
    )
      this.removeDefaultImport();
    if (Object.hasOwn(structure, common.nameof(structure, "namedImports")))
      this.removeNamedImports();
    if (structure.namespaceImport != null)
      this.setNamespaceImport(structure.namespaceImport);
    else if (
      Object.hasOwn(structure, common.nameof(structure, "namespaceImport"))
    )
...
function setEmptyNamedImport(node) {
  ...
  const namedImportStructurePrinter =
    node._context.structurePrinterFactory.forNamedImportExportSpecifier();
  ...
  if (importClause == null) {
    insertIntoParentTextRange({
      insertPos: node
        .getFirstChildByKindOrThrow(common.SyntaxKind.ImportKeyword)
        .getEnd(),
      parent: node,
...
function getErrorWhenNamespaceImportsExist() {
  return new common.errors.InvalidOperationError(
    "Cannot add a named import to an import declaration that has a namespace import.",
  );
}

const createBase$s = (ctor) =>
  ExportableNode(ModifierableNode(JSDocableNode(NamedNode(ctor))));
...
class ImportEqualsDeclaration extends ImportEqualsDeclarationBase {
  ...
  isExternalModuleReferenceRelative() {
    ...
    if (!Node.isExternalModuleReference(moduleReference)) return false;
  ...
  setExternalModuleReference(textOrSourceFile) {
    ...
    if (
      ...
      moduleReference
        ...
        .replaceWithText((writer) => writer.quote(text));
    else
  ...
  getExternalModuleReferenceSourceFile() {
    const moduleReference = this.getModuleReference();
    if (!Node.isExternalModuleReference(moduleReference)) return undefined;
...
class ImportSpecifier extends ImportSpecifierBase {
  ...
  removeAlias() {
    const aliasIdentifier = this.getAliasNode();
    if (aliasIdentifier == null) return this;
    removeChildren({
      children: [
        this.getFirstChildByKindOrThrow(common.SyntaxKind.AsKeyword),
        aliasIdentifier,
  ...
  getImportDeclaration() {
    return this.getFirstAncestorByKindOrThrow(
      common.SyntaxKind.ImportDeclaration,
    );
  }
  remove() {
    ...
    if (
      ...
      (importDeclaration.getNamespaceImport() == null &&
        importDeclaration.getDefaultImport() == null)
    )
      removeCommaSeparatedChild(this);
    else importDeclaration.removeNamedImports();
  }
  set(structure) {
  ...
  return class extends Base {
    getParentModuleOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? "Expected to find the parent module declaration.",
        this,
      );
    }
    getParentModule() {
      let parent = this.getParentOrThrow();
      if (!Node.isModuleBlock(parent)) return undefined;
      while (
        parent.getParentOrThrow().getKind() ===
        common.SyntaxKind.ModuleDeclaration
      )
...
class ModuleDeclaration extends ModuleDeclarationBase {
  ...
  setName(newName) {
    ...
    if (moduleName instanceof Array) {
      ...
      if (newName !== "global") addNamespaceKeywordIfNecessary(this);
  ...
  rename(newName, options) {
    ...
    if (nameNodes instanceof Array) {
      if (nameNodes.length > 1) {
        throw new common.errors.NotSupportedError(
          `Cannot rename a namespace name that uses dot notation. Rename the individual nodes via .${common.nameof(this, "getNameNodes")}()`,
        );
      }
      if (newName !== "global") addNamespaceKeywordIfNecessary(this);
  ...
  getNameNodes() {
    ...
    if (Node.isStringLiteral(name)) return name;
    else {
      const nodes = [];
      let current = this;
      do {
        nodes.push(this._getNodeFromCompilerNode(current.compilerNode.name));
        current = current.getFirstChildByKind(
          common.SyntaxKind.ModuleDeclaration,
        );
      } while (current != null);
      return nodes;
  ...
  setDeclarationKind(kind) {
    if (this.getDeclarationKind() === kind) return this;
  ...
  getDeclarationKindKeyword() {
    return this.getFirstChild(
      (child) =>
        child.getKind() === common.SyntaxKind.NamespaceKeyword ||
...
function addNamespaceKeywordIfNecessary(namespaceDec) {
  if (
    namespaceDec.getDeclarationKind() === exports.ModuleDeclarationKind.Global
  )
...
class NamespaceImport extends NamespaceImportBase {
  setName(name) {
    const nameNode = this.getNameNode();
    if (nameNode.getText() === name) return this;
...
((FileSystemRefreshResult) => {
  FileSystemRefreshResult[(FileSystemRefreshResult["NoChange"] = 0)] =
    "NoChange";
...
const SourceFile = (() => {
  const _classSuper = SourceFileBase;
  ...
  return class SourceFile extends _classSuper {
    static {
      ...
      __esDecorate(
        SourceFile,
        null,
        _isFromExternalLibrary_decorators,
        {
          kind: "method",
          name: "isFromExternalLibrary",
          static: false,
          private: false,
          access: {
            has: (obj) => "isFromExternalLibrary" in obj,
            get: (obj) => obj.isFromExternalLibrary,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(SourceFile, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    getDirectoryPath() {
      return this._context.fileSystemWrapper.getStandardizedAbsolutePath(
        common.FileUtils.getDirPath(this.compilerNode.fileName),
      );
    }
    getFullText() {
      return this.compilerNode.text;
    ...
    getLineAndColumnAtPos(pos) {
      ...
      return {
        ...
        column:
          common.StringUtils.getLengthFromLineStartAtPos(fullText, pos) + 1,
      };
    }
    getLengthFromLineStartAtPos(pos) {
      return common.StringUtils.getLengthFromLineStartAtPos(
        this.getFullText(),
        pos,
      );
    }
    copyToDirectory(dirPathOrDirectory, options) {
      const dirPath =
        typeof dirPathOrDirectory === "string"
          ? dirPathOrDirectory
          : dirPathOrDirectory.getPath();
      return this.copy(
        common.FileUtils.pathJoin(dirPath, this.getBaseName()),
        options,
      );
    }
    ...
    _copyInternal(fileAbsoluteOrRelativePath, options = {}) {
      ...
      const standardizedFilePath =
        fileSystemWrapper.getStandardizedAbsolutePath(
          fileAbsoluteOrRelativePath,
          this.getDirectoryPath(),
        );
      if (standardizedFilePath === this.getFilePath()) return false;
      ...
      function getCopiedSourceFile(currentFile) {
        ...
        function getShouldBeInProject() {
          if (currentFile._isInProject()) return true;
          const destinationFile =
            compilerFactory.getSourceFileFromCacheFromFilePath(
              standardizedFilePath,
            );
    ...
    _updateReferencesForCopyInternal(literalReferences) {
      for (const reference of literalReferences)
        reference[0] =
          this.getChildSyntaxListOrThrow().getDescendantAtStartWithWidth(
    ...
    move(filePath, options = {}) {
      ...
      if (!this._moveInternal(filePath, options)) return this;
    ...
    _moveInternal(fileRelativeOrAbsolutePath, options = {}) {
      ...
      if (filePath === this.getFilePath()) return false;
      let markAsInProject = false;
      if (overwrite) {
        const existingSourceFile =
          this._context.compilerFactory.getSourceFileFromCacheFromFilePath(
            filePath,
          );
      ...
      if (markAsInProject) this._markAsInProject();
      if (this._isInProject()) this.getDirectory()._markAsInProject();
      return true;
    }
    ...
    async save() {
      if (this._isLibFileInMemory()) return;
    ...
    getPathReferenceDirectives() {
      if (this.#referencedFiles == null) {
        this.#referencedFiles = (this.compilerNode.referencedFiles || []).map(
          (f) => new FileReference(f, this),
        );
      }
    ...
    getReferencedSourceFiles() {
      ...
      function* getSourceFilesFromEntries() {
        for (const [, sourceFile] of entries) yield sourceFile;
      }
    }
    ...
    getLiteralsReferencingOtherSourceFiles() {
      ...
      function* getLiteralsFromEntries() {
        for (const [literal] of entries) yield literal;
      }
    }
    getImportStringLiterals() {
      ...
      return literals
        .filter((l) => l.pos !== -1)
        .map((l) => this._getNodeFromCompilerNode(l));
    }
    getLanguageVersion() {
    ...
    isFromExternalLibrary() {
      if (!this._context.program._isCompilerProgramCreated()) return false;
    ...
    indent(positionRangeOrPos, times = 1) {
      ...
      const positionRange =
        typeof positionRangeOrPos === "number"
      ...
      const correctedText = common.StringUtils.indent(
      ...
      replaceSourceFileTextForFormatting({
        sourceFile: this,
        newText:
          sourceFileText.substring(0, startLinePos) +
          correctedText +
    ...
    async refreshFromFileSystem() {
      const fileReadResult =
        await this._context.fileSystemWrapper.readFileOrNotExists(
          this.getFilePath(),
    ...
    refreshFromFileSystemSync() {
      const fileReadResult =
        this._context.fileSystemWrapper.readFileOrNotExistsSync(
          this.getFilePath(),
          this._context.getEncoding(),
        );
      return this.#refreshFromFileSystemInternal(fileReadResult);
    ...
    onModified(subscription, subscribe = true) {
      if (subscribe) this.#modifiedEventContainer.subscribe(subscription);
      else this.#modifiedEventContainer.unsubscribe(subscription);
      return this;
    }
    ...
    organizeImports(formatSettings = {}, userPreferences = {}) {
      this._context.languageService
        .organizeImports(this, formatSettings, userPreferences)
    ...
    fixMissingImports(formatSettings = {}, userPreferences = {}) {
      ...
      function applyTextChanges(changes) {
        const groups = common.ArrayUtils.groupBy(changes, (change) =>
          change.getSpan().getStart(),
        );
        let addedLength = 0;
    ...
    applyTextChanges(textChanges) {
      if (textChanges.length === 0) return this;
      this.forgetDescendants();
      replaceNodeText({
        sourceFile: this._sourceFile,
        start: 0,
    ...
    _isLibFileInMemory() {
      return this.compilerNode.fileName.startsWith(
        common.libFolderInMemoryPath,
      );
    }
...
function OverloadableNode(Base) {
  return class extends Base {
    ...
    getImplementation() {
      if (this.isImplementation()) return this;
...
function insertOverloads(opts) {
  ...
  for (const structure of structures) {
    if (writer.getLength() > 0) writer.newLine();
  ...
  getStructure() {
    ...
    function getStructure(thisNode) {
      ...
      function getSpecificStructure() {
        if (!hasImplementation) return { kind: exports.StructureKind.Function };
        else {
          return {
...
class ParameterDeclaration extends ParameterDeclarationBase {
  ...
  isParameterProperty() {
    return (
      this.getScope() != null || this.isReadonly() || this.hasOverrideKeyword()
    );
  }
  setIsRestParameter(value) {
    if (this.isRestParameter() === value) return this;
    if (value) {
  ...
  isOptional() {
    return (
      this.compilerNode.questionToken != null ||
  ...
  setHasQuestionToken(value) {
    if (value) addParensIfNecessary(this);
...
function addParensIfNecessary(parameter) {
  ...
  if (isParameterWithoutParens()) addParens();
  function isParameterWithoutParens() {
    return (
      ...
      parameter
        ...
        .getPreviousSiblingIfKind(common.SyntaxKind.OpenParenToken) == null
    );
  }
  function addParens() {
...
class ClassElement extends Node {
  remove() {
    ...
    if (Node.isClassDeclaration(parent) || Node.isClassExpression(parent))
      ...
      removeCommaSeparatedChild(this);
    else if (Node.isInterfaceDeclaration(parent)) removeInterfaceMember(this);
    else common.errors.throwNotImplementedForSyntaxKindError(parent.getKind());
  }
}
...
class MethodDeclaration extends MethodDeclarationBase {
  ...
  getStructure() {
    ...
    function getStructure(thisNode) {
      ...
      function getSpecificStructure() {
        if (!hasImplementation) return { kind: exports.StructureKind.Method };
        else {
          return {
  ...
  return class extends Base {
    setExtends(text) {
      ...
      if (extendsClause != null) {
        ...
        let insertPos;
        if (implementsClause != null) insertPos = implementsClause.getStart();
        else
          insertPos = this.getFirstChildByKindOrThrow(
            common.SyntaxKind.OpenBraceToken,
          ).getStart();
        ...
        if (!isLastSpace) newText = " " + newText;
        insertIntoParentTextRange({
          parent:
    ...
    removeExtends() {
      ...
      if (extendsClause == null) return this;
    ...
    getExtends() {
      ...
      if (extendsClause == null) return undefined;
    ...
    insertMembers(index, members) {
      ...
      return insertIntoBracesOrSourceFileWithGetChildrenWithComments({
        ...
        parent: this,
        write: (writer, info) => {
          const previousMemberHasBody =
            !isAmbient &&
            info.previousMember != null &&
            Node.isBodyable(info.previousMember) &&
            info.previousMember.hasBody();
          const firstStructureHasBody =
            !isAmbient &&
            members instanceof Array &&
            structureHasBody(members[0]);
          if (
          ...
          const memberWriter = this._getWriter();
          const memberPrinter =
            this._context.structurePrinterFactory.forClassMember({ isAmbient });
          memberPrinter.printTexts(memberWriter, members);
          writer.write(memberWriter.toString());
          const lastStructureHasBody =
            !isAmbient &&
            members instanceof Array &&
            structureHasBody(members[members.length - 1]);
          const nextMemberHasBody =
            !isAmbient &&
            info.nextMember != null &&
            Node.isBodyable(info.nextMember) &&
            info.nextMember.hasBody();
          if (
            (info.nextMember != null && lastStructureHasBody) ||
            nextMemberHasBody
          )
            writer.blankLineIfLastNot();
          else writer.newLineIfLastNot();
      ...
      return insertChildren(this, {
        ...
        write: (writer, info) => {
          ...
          this._context.structurePrinterFactory
            ...
            .printTexts(writer, structures);
          if (info.nextMember != null) writer.blankLineIfLastNot();
          else writer.newLineIfLastNot();
        },
      });
    }
    ...
    getInstanceProperties() {
      return this.getInstanceMembers().filter((m) => isClassPropertyType(m));
    }
    ...
    getStaticProperties() {
      return this.getStaticMembers().filter((m) => isClassPropertyType(m));
    }
    getProperty(nameOrFindFunction) {
      return getNodeByNameOrFindFunction(
        this.getProperties(),
    ...
    getInstanceMethods() {
      return this.getInstanceMembers().filter(
        (m) => m instanceof MethodDeclaration,
      );
    }
    ...
    getStaticMethods() {
      return this.getStaticMembers().filter(
        (m) => m instanceof MethodDeclaration,
      );
    }
    ...
    getMembersWithParameterProperties() {
      ...
      const implementationCtors = members.filter(
        (c) => Node.isConstructorDeclaration(c) && c.isImplementation(),
      );
    ...
    getMembersWithComments() {
      ...
      return getAllMembers(this, members).filter(
        (m) => isSupportedClassMember(m) || Node.isCommentClassElement(m),
      );
    }
    ...
    getBaseClassOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getBaseClass(),
        message ?? `Expected to find the base class of ${this.getName()}.`,
        this,
      );
    }
    getBaseClass() {
      const baseTypes = this.getBaseTypes().flatMap((t) =>
  ...
  return isAmbient
    ...
    : members.filter((m) => {
        if (!(Node.isConstructorDeclaration(m) || Node.isMethodDeclaration(m)))
          return true;
        if (Node.isMethodDeclaration(m) && m.isAbstract()) return true;
...
function getImmediateDerivedClasses(classDec) {
  ...
  for (let node of nameNode.findReferencesAsNodes()) {
    node =
      node.getParentWhileKind(common.SyntaxKind.PropertyAccessExpression) ??
      node;
    ...
    const heritageClause = nodeParent.getParentIfKind(
      common.SyntaxKind.HeritageClause,
    );
    if (
      heritageClause == null ||
      heritageClause.getToken() !== common.SyntaxKind.ExtendsKeyword
    )
      continue;
...
class ClassDeclaration extends ClassDeclarationBase {
  set(structure) {
    ...
    if (structure.extends != null) this.setExtends(structure.extends);
    else if (Object.hasOwn(structure, common.nameof(structure, "extends")))
      this.removeExtends();
  ...
  extractInterface(name) {
    ...
    const parameterProperties = constructors
      .flatMap((c) => c.getParameters().filter((p) => p.isParameterProperty()))
      .filter(
        (p) => p.getName() != null && p.getScope() === exports.Scope.Public,
      );
    return {
...
function getExtractedClassDetails(classDec, isStatic) {
  const constructors = classDec
    .getConstructors()
    .flatMap((c) => (c.getOverloads().length > 0 ? c.getOverloads() : [c]));
  const properties = classDec
    .getProperties()
    .filter(
      (p) => p.isStatic() === isStatic && p.getScope() === exports.Scope.Public,
    );
  const methods = classDec
    .getMethods()
    .filter(
      (p) => p.isStatic() === isStatic && p.getScope() === exports.Scope.Public,
    )
    .flatMap((m) => (m.getOverloads().length > 0 ? m.getOverloads() : [m]));
  ...
  function getAccessors() {
    ...
    for (const accessor of [
      ...
      if (
        ...
        accessor.getScope() === exports.Scope.Public
      )
...
function getExtractedInterfaceMethodStructure(method) {
  return {
    ...
    parameters: method
      .getParameters()
      .map(getExtractedInterfaceParameterStructure),
  ...
  getSetAccessor() {
    ...
    return this.getParentOrThrow().forEachChild((sibling) => {
      if (
        ...
        sibling.getName() === thisName &&
        sibling.isStatic() === isStatic
      )
        return sibling;
      return undefined;
    });
...
class Decorator extends DecoratorBase {
  ...
  getNameNode() {
    ...
    else return getIdentifierFromName(this._getInnerExpression());
  ...
  setIsDecoratorFactory(isDecoratorFactory) {
    if (this.isDecoratorFactory() === isDecoratorFactory) return this;
  ...
  getCallExpressionOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find a call expression.",
      this,
    );
  }
  getCallExpression() {
  ...
  return class extends Base {
    ...
    getTypeExpressionOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        ...
        message ?? `Expected to find a JS doc type expression.`,
        this,
      );
    }
    getName() {
...
function JSDocTypeExpressionableTag(Base) {
  return class extends Base {
    getTypeExpression() {
      ...
      if (result != null && result.getWidth() === 0) return undefined;
      return result;
    }
    getTypeExpressionOrThrow(message) {
      return common.errors.throwIfNullOrUndefined(
        this.getTypeExpression(),
        message ?? `Expected to find the JS doc tag's type expression.`,
        this,
      );
    }
  };
...
function getTextWithoutStars(inputText) {
  ...
  return innerTextWithStars
    ...
    .map((line) => {
      ...
      if (starPos === -1) return line;
      const substringStart =
        line[starPos + 1] === " " ? starPos + 2 : starPos + 1;
  ...
  function getStarPosIfFirstNonWhitespaceChar(text) {
    for (let i = 0; i < text.length; i++) {
      ...
      if (charCode === CharCodes.ASTERISK) return i;
      else if (!common.StringUtils.isWhitespaceCharCode(charCode)) break;
    }
    return -1;
  }
}
...
class JSDoc extends JSDocBase {
  ...
  getTags() {
    return (
      this.compilerNode.tags?.map((t) => this._getNodeFromCompilerNode(t)) ?? []
    );
  }
  getInnerText() {
  ...
  getComment() {
    if (this.compilerNode.comment == null) return undefined;
  ...
  getCommentText() {
    if (typeof this.compilerNode.comment === "string")
      return this.compilerNode.comment;
    else return common.ts.getTextOfJSDocComment(this.compilerNode.comment);
  }
  getDescription() {
  ...
  setDescription(textOrWriterFunction) {
    ...
    function getNewText() {
      ...
      const isSingleLine =
        rawLines.length === 1 && (this.compilerNode.tags?.length ?? 0) === 0;
  ...
  insertTags(index, structures) {
    ...
    function getReplaceStart() {
      const searchStart =
        index < tags.length ? tags[index].getStart() : this.getEnd() - 2;
    ...
    function getReplaceEnd() {
      if (index < tags.length) return tags[index].getStart();
...
class ImportTypeNode extends NodeWithTypeArguments {
  ...
  setQualifier(text) {
    ...
    if (qualifier != null)
      ...
      const paren = this.getFirstChildByKindOrThrow(
        common.SyntaxKind.CloseParenToken,
      );
      insertIntoParentTextRange({
...
class MappedTypeNode extends TypeNode {
  ...
  getReadonlyTokenOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Readonly token did not exist.",
      this,
    );
  }
  getQuestionToken() {
  ...
  getQuestionTokenOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Question token did not exist.",
      this,
    );
  }
  getTypeParameter() {
  ...
  getTypeNodeOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getTypeNode(),
      message ?? "Type did not exist, but was expected to exist.",
      this,
    );
  }
}
...
class TypeParameterDeclaration extends TypeParameterDeclarationBase {
  ...
  getConstraintOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getConstraint(),
      message ?? "Expected to find the type parameter's constraint.",
      this,
    );
  }
  setConstraint(text) {
  ...
  getDefaultOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getDefault(),
      message ?? "Expected to find the type parameter's default.",
      this,
    );
  }
  setDefault(text) {
  ...
  setVariance(variance) {
    this.toggleModifier(
      "in",
      (variance & exports.TypeParameterVariance.In) !== 0,
    );
    this.toggleModifier(
      "out",
      (variance & exports.TypeParameterVariance.Out) !== 0,
    );
    return this;
  }
  getVariance() {
  ...
  remove() {
    ...
    const typeParameters = parentSyntaxList.getChildrenOfKind(
      common.SyntaxKind.TypeParameter,
    );
    if (typeParameters.length === 1) removeAllTypeParameters();
    else removeCommaSeparatedChild(this);
  ...
  set(structure) {
    ...
    if (structure.isConst != null) this.setIsConst(structure.isConst);
    if (structure.constraint != null) this.setConstraint(structure.constraint);
    else if (Object.hasOwn(structure, common.nameof(structure, "constraint")))
      this.removeConstraint();
    if (structure.default != null) this.setDefault(structure.default);
    else if (Object.hasOwn(structure, common.nameof(structure, "default")))
      this.removeDefault();
    if (structure.variance != null) this.setVariance(structure.variance);
    return this;
  }
  getStructure() {
...
class TypePredicateNode extends TypeNode {
  ...
  getAssertsModifierOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an asserts modifier.",
      this,
    );
  }
  getTypeNode() {
...
class JSDocTag extends JSDocTagBase {
  ...
  remove() {
    ...
    function getReplaceTrivia() {
      if (removalStart === jsDocBodyStart && isLastJsDoc) return "";
      const newLineKind =
        this._context.manipulationSettings.getNewLineKindAsString();
...
class JSDocTemplateTag extends JSDocTemplateTagBase {
  ...
  getConstraintOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getConstraint(),
      message ?? "Expected to find the JS doc template tag's constraint.",
      this,
    );
  }
}
...
class JSDocTypeTag extends JSDocTag {
  getTypeExpression() {
    ...
    if (node != null && node.pos === node.end) return undefined;
    return this._getNodeFromCompilerNodeIfExists(
      this.compilerNode.typeExpression,
    );
  }
...
class EnumDeclaration extends EnumDeclarationBase {
  set(structure) {
    ...
    if (structure.isConst != null) this.setIsConstEnum(structure.isConst);
  ...
  insertMembers(index, structures) {
    ...
    const writer = this._getWriterWithChildIndentation();
    const structurePrinter =
      this._context.structurePrinterFactory.forEnumMember();
    structurePrinter.printTexts(writer, structures);
    insertIntoCommaSeparatedNodes({
      parent: this.getChildSyntaxListOrThrow(),
    ...
    function areAllStructuresStructures() {
      if (!(structures instanceof Array)) return false;
...
class EnumMember extends EnumMemberBase {
  ...
  set(structure) {
    ...
    if (structure.value != null) this.setValue(structure.value);
    else if (
      Object.hasOwn(structure, common.nameof(structure, "value")) &&
...
class HeritageClause extends Node {
  getTypeNodes() {
    return (
      this.compilerNode.types?.map((t) => this._getNodeFromCompilerNode(t)) ??
      []
    );
  }
  getToken() {
  ...
  removeExpression(expressionNodeOrIndex) {
    ...
    const expressionNodeToRemove =
      typeof expressionNodeOrIndex === "number"
    ...
    if (expressions.length === 1) {
      ...
      else removeChildren({ children: [this], removePrecedingSpaces: true });
    } else {
...
class IndexSignatureDeclaration extends IndexSignatureDeclarationBase {
  ...
  setKeyName(name) {
    ...
    if (this.getKeyName() === name) return;
  ...
  setKeyType(type) {
    ...
    if (keyTypeNode.getText() === type) return this;
  ...
  set(structure) {
    ...
    if (structure.keyName != null) this.setKeyName(structure.keyName);
    if (structure.keyType != null) this.setKeyType(structure.keyType);
    return this;
  }
  getStructure() {
    const keyTypeNode = this.getKeyTypeNode();
...
class JsxAttribute extends JsxAttributeBase {
  ...
  removeInitializer() {
    ...
    removeChildren({
      children: [
        initializer.getPreviousSiblingIfKindOrThrow(
          common.SyntaxKind.EqualsToken,
        ),
        initializer,
      ],
  ...
  getStructure() {
    ...
    return callBaseGetStructure(JsxAttributeBase.prototype, this, {
      name:
        nameNode instanceof Identifier
          ? nameNode.getText()
          : nameNode.getStructure(),
...
class JsxElement extends JsxElementBase {
  ...
  set(structure) {
    ...
    if (structure.bodyText != null) this.setBodyText(structure.bodyText);
    else if (Object.hasOwn(structure, common.nameof(structure, "bodyText")))
...
class BigIntLiteral extends BigIntLiteralBase {
  ...
  setLiteralValue(value) {
    if (typeof value !== "bigint")
      throw new common.errors.ArgumentTypeError(
        "value",
        "bigint",
        typeof value,
      );
...
function setLiteralValue(node, value) {
  if (getLiteralValue(node) === value) return node;
...
class NumericLiteral extends NumericLiteralBase {
  getLiteralValue() {
    const text = this.compilerNode.text;
    if (text.indexOf(".") >= 0) return parseFloat(text);
...
class RegularExpressionLiteral extends RegularExpressionLiteralBase {
  ...
  setLiteralValue(regExpOrPattern, flags) {
    let pattern;
    if (typeof regExpOrPattern === "string") pattern = regExpOrPattern;
    else {
...
class StringLiteral extends StringLiteralBase {
  ...
  setLiteralValue(value) {
    replaceNodeText({
      ...
      start: this.getStart() + 1,
      replacingLength: this.getWidth() - 2,
      newText: common.StringUtils.escapeForWithinString(
        value,
        this.getQuoteKind(),
      ),
    });
...
class TaggedTemplateExpression extends MemberExpression {
  ...
  removeTag() {
    ...
    insertIntoParentTextRange({
      customMappings: (newParent, newSourceFile) => [
        {
...
class VariableDeclaration extends VariableDeclarationBase {
  remove() {
    ...
    function removeFromDeclarationList(node) {
      const variableStatement = parent.getParentIfKindOrThrow(
        common.SyntaxKind.VariableStatement,
      );
      const declarations = variableStatement.getDeclarations();
      if (declarations.length === 1) variableStatement.remove();
      else removeCommaSeparatedChild(node);
    }
    function removeFromCatchClause(node) {
      removeChildren({
        children: [
          node.getPreviousSiblingIfKindOrThrow(
            common.SyntaxKind.OpenParenToken,
          ),
          node,
  ...
  getVariableStatementOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected the grandparent to be a variable statement.",
      this,
    );
  }
...
class VariableDeclarationList extends VariableDeclarationListBase {
  ...
  getDeclarationKind() {
    ...
    if (nodeFlags & common.ts.NodeFlags.Let)
      ...
      (nodeFlags & common.ts.NodeFlags.AwaitUsing) ===
      common.ts.NodeFlags.AwaitUsing
    )
      return exports.VariableDeclarationKind.AwaitUsing;
    else if (
      (nodeFlags & common.ts.NodeFlags.Using) ===
      common.ts.NodeFlags.Using
    )
    ...
    else return exports.VariableDeclarationKind.Var;
  }
  getDeclarationKindKeywords() {
    ...
    switch (declarationKind) {
      case exports.VariableDeclarationKind.Const:
        return [
          this.getFirstChildByKindOrThrow(common.SyntaxKind.ConstKeyword),
        ];
      ...
      case exports.VariableDeclarationKind.Using:
        return [
          this.getFirstChildByKindOrThrow(common.SyntaxKind.UsingKeyword),
        ];
      case exports.VariableDeclarationKind.AwaitUsing: {
        const awaitKeyword = this.getFirstChildByKindOrThrow(
          common.SyntaxKind.AwaitKeyword,
        );
        const usingKeyword = awaitKeyword.getNextSiblingIfKindOrThrow(
          common.SyntaxKind.UsingKeyword,
        );
  ...
  setDeclarationKind(type) {
    if (this.getDeclarationKind() === type) return this;
...
class Signature {
  ...
  getTypeParameters() {
    ...
    return typeParameters.map((t) =>
      this.#context.compilerFactory.getTypeParameter(t),
    );
  }
  getParameters() {
    return this.compilerSignature.parameters.map((p) =>
      this.#context.compilerFactory.getSymbol(p),
    );
  }
  getReturnType() {
    return this.#context.compilerFactory.getType(
      this.compilerSignature.getReturnType(),
    );
  }
  getDocumentationComments() {
    const docs = this.compilerSignature.getDocumentationComment(
      this.#context.typeChecker.compilerObject,
    );
    return docs.map((d) =>
      this.#context.compilerFactory.getSymbolDisplayPart(d),
    );
  }
  getJsDocTags() {
  ...
  getDeclaration() {
    ...
    const compilerSignatureDeclaration =
      this.compilerSignature.getDeclaration();
...
const Symbol$1 = class Symbol {
  ...
  getAliasedSymbolOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an aliased symbol.",
    );
  }
  ...
  getImmediatelyAliasedSymbolOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an immediately aliased symbol.",
    );
  }
  getAliasedSymbol() {
  ...
  isAlias() {
    return (
      (this.getFlags() & common.SymbolFlags.Alias) === common.SymbolFlags.Alias
    );
  }
  isOptional() {
    return (
      (this.getFlags() & common.SymbolFlags.Optional) ===
      common.SymbolFlags.Optional
    );
  }
  getFlags() {
  ...
  getValueDeclaration() {
    ...
    return this.#context.compilerFactory.getNodeFromCompilerNode(
      declaration,
      this.#context.compilerFactory.getSourceFileForNode(declaration),
    );
  }
  getDeclarations() {
    return (this.compilerSymbol.declarations ?? []).map((d) =>
      this.#context.compilerFactory.getNodeFromCompilerNode(
        d,
        this.#context.compilerFactory.getSourceFileForNode(d),
      ),
    );
  }
  getExportOrThrow(name, message) {
    return common.errors.throwIfNullOrUndefined(
      this.getExport(name),
      message ?? (() => `Expected to find export with name: ${name}`),
    );
  }
  getExport(name) {
    if (this.compilerSymbol.exports == null) return undefined;
    const tsSymbol = this.compilerSymbol.exports.get(
      common.ts.escapeLeadingUnderscores(name),
    );
    return tsSymbol == null
      ? undefined
      : this.#context.compilerFactory.getSymbol(tsSymbol);
  }
  getExports() {
    if (this.compilerSymbol.exports == null) return [];
    return Array.from(this.compilerSymbol.exports.values()).map((symbol) =>
      this.#context.compilerFactory.getSymbol(symbol),
    );
  }
  getGlobalExportOrThrow(name, message) {
    return common.errors.throwIfNullOrUndefined(
      this.getGlobalExport(name),
      message ?? (() => `Expected to find global export with name: ${name}`),
    );
  }
  getGlobalExport(name) {
    if (this.compilerSymbol.globalExports == null) return undefined;
  ...
  getGlobalExports() {
    if (this.compilerSymbol.globalExports == null) return [];
    return Array.from(this.compilerSymbol.globalExports.values()).map(
      (symbol) => this.#context.compilerFactory.getSymbol(symbol),
    );
  }
  getMemberOrThrow(name, message) {
    return common.errors.throwIfNullOrUndefined(
      this.getMember(name),
      message ?? `Expected to find member with name: ${name}`,
    );
  }
  getMember(name) {
    if (this.compilerSymbol.members == null) return undefined;
  ...
  getMembers() {
    if (this.compilerSymbol.members == null) return [];
    return Array.from(this.compilerSymbol.members.values()).map((symbol) =>
      this.#context.compilerFactory.getSymbol(symbol),
    );
  }
  getDeclaredType() {
  ...
  getJsDocTags() {
    return this.compilerSymbol
      .getJsDocTags(this.#context.typeChecker.compilerObject)
    ...
    static {
      ...
      __esDecorate(
        ...
        _getSpan_decorators,
        {
          ...
          static: false,
          private: false,
          access: { has: (obj) => "getSpan" in obj, get: (obj) => obj.getSpan },
          metadata: _metadata,
        },
        null,
        _instanceExtraInitializers,
      ...
      if (_metadata)
        Object.defineProperty(TextChange, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
...
const FileTextChanges = (() => {
  ...
  return class FileTextChanges {
    static {
      ...
      __esDecorate(
        ...
        _getTextChanges_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getTextChanges" in obj,
            get: (obj) => obj.getTextChanges,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(FileTextChanges, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    constructor(context, compilerObject) {
      this.#context = context;
      this.#compilerObject = compilerObject;
      const file = context.compilerFactory.addOrGetSourceFileFromFilePath(
      ...
      if (!compilerObject.isNewFile) this.#sourceFile = file;
    }
    getFilePath() {
      return this.#compilerObject.fileName;
    }
    ...
    applyChanges(options = {}) {
      ...
      let file;
      if (this.isNewFile())
        file = this.#context.project.createSourceFile(this.getFilePath(), "", {
          overwrite: options.overwrite,
        });
      else file = this.getSourceFile();
      if (file == null) {
        throw new common.errors.InvalidOperationError(
...
class CodeAction {
  ...
  getChanges() {
    return this.compilerObject.changes.map(
      (change) => new FileTextChanges(this.#context, change),
    );
  }
}
...
const CombinedCodeActions = (() => {
  ...
  return class CombinedCodeActions {
    static {
      ...
      __esDecorate(
        ...
        _getChanges_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getChanges" in obj,
            get: (obj) => obj.getChanges,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(CombinedCodeActions, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    applyChanges(options) {
      for (const change of this.getChanges()) change.applyChanges(options);
      return this;
    }
  };
})();
    ...
    static {
      ...
      __esDecorate(
        ...
        _getTextSpan_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getTextSpan" in obj,
            get: (obj) => obj.getTextSpan,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _getNode_decorators,
        {
          ...
          static: false,
          private: false,
          access: { has: (obj) => "getNode" in obj, get: (obj) => obj.getNode },
          metadata: _metadata,
        },
        null,
        _instanceExtraInitializers,
      ...
      __esDecorate(
        DocumentSpan,
        null,
        _getOriginalTextSpan_decorators,
        {
          kind: "method",
          name: "getOriginalTextSpan",
          static: false,
          private: false,
          access: {
            has: (obj) => "getOriginalTextSpan" in obj,
            get: (obj) => obj.getOriginalTextSpan,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(DocumentSpan, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
      ...
      function findBestMatchingNode() {
        ...
        sourceFile._context.compilerFactory.forgetNodesCreatedInBlock(
          (remember) => {
            ...
            while (nextNode != null) {
              if (foundNode == null) bestNode = nextNode;
              if (
                nextNode.getStart() === start &&
                nextNode.getWidth() === width
              )
                bestNode = foundNode = nextNode;
              else if (foundNode != null) break;
              nextNode = nextNode.getChildAtPos(start);
            }
            if (bestNode != null) remember(bestNode);
          },
        );
        return bestNode;
      }
...
const DefinitionInfo = (() => {
  const _classSuper = DocumentSpan;
  const _instanceExtraInitializers = [];
  ...
  return class DefinitionInfo extends _classSuper {
    static {
      ...
      __esDecorate(
        DefinitionInfo,
        null,
        _getDeclarationNode_decorators,
        {
          kind: "method",
          name: "getDeclarationNode",
          static: false,
          private: false,
          access: {
            has: (obj) => "getDeclarationNode" in obj,
            get: (obj) => obj.getDeclarationNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(DefinitionInfo, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    getDeclarationNode() {
      ...
      function findIdentifier(node) {
        if (
          node.getKind() === common.SyntaxKind.Identifier &&
          node.getStart() === start
        )
          return node;
        for (const child of node._getChildrenIterator()) {
...
const Diagnostic = (() => {
  ...
  return class Diagnostic {
    static {
      ...
      __esDecorate(
        ...
        _getSourceFile_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getSourceFile" in obj,
            get: (obj) => obj.getSourceFile,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(Diagnostic, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    getSourceFile() {
      if (this._context == null) return undefined;
    ...
    getMessageText() {
      ...
      if (this._context == null) return new DiagnosticMessageChain(messageText);
      else
    ...
    getLineNumber() {
      const sourceFile = this.getSourceFile();
      const start = this.getStart();
      if (sourceFile == null || start == null) return undefined;
      return common.StringUtils.getLineNumberAtPos(
...
class OutputFile {
  ...
  getFilePath() {
    return this.#context.fileSystemWrapper.getStandardizedAbsolutePath(
  ...
  return class EmitOutput {
    static {
      ...
      __esDecorate(
        ...
        _getOutputFiles_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getOutputFiles" in obj,
            get: (obj) => obj.getOutputFiles,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(EmitOutput, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    getDiagnostics() {
      return this.compilerObject.diagnostics.map((d) =>
        this.#context.compilerFactory.getDiagnostic(d),
      );
    }
    getEmitSkipped() {
    ...
    getOutputFiles() {
      return this.compilerObject.outputFiles.map(
        (f) => new OutputFile(this.#context, f),
      );
    }
  };
})();
...
const EmitResult = (() => {
  ...
  return class EmitResult {
    static {
      ...
      if (_metadata)
        Object.defineProperty(EmitResult, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
  ...
  return class ImplementationLocation extends _classSuper {
    static {
      ...
      __esDecorate(
        ...
        null,
        _getDisplayParts_decorators,
        {
          kind: "method",
          name: "getDisplayParts",
          static: false,
          private: false,
          access: {
            has: (obj) => "getDisplayParts" in obj,
            get: (obj) => obj.getDisplayParts,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(ImplementationLocation, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
...
const RefactorEditInfo = (() => {
  ...
  return class RefactorEditInfo {
    static {
      ...
      __esDecorate(
        ...
        _getEdits_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getEdits" in obj,
            get: (obj) => obj.getEdits,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(RefactorEditInfo, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    getEdits() {
      return this.compilerObject.edits.map(
        (edit) => new FileTextChanges(this.#context, edit),
      );
    }
    getRenameFilePath() {
    ...
    applyChanges(options) {
      for (const change of this.getEdits()) change.applyChanges(options);
      return this;
    }
  };
})();
...
const ReferencedSymbol = (() => {
  ...
  return class ReferencedSymbol {
    static {
      ...
      __esDecorate(
        ...
        _getDefinition_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "getDefinition" in obj,
            get: (obj) => obj.getDefinition,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(ReferencedSymbol, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    constructor(context, compilerObject) {
      this._context = context;
      this.#compilerObject = compilerObject;
      this.#references = this.compilerObject.references.map((r) =>
...
const ReferencedSymbolDefinitionInfo = (() => {
  const _classSuper = DefinitionInfo;
  const _instanceExtraInitializers = [];
  let _getDisplayParts_decorators;
  return class ReferencedSymbolDefinitionInfo extends _classSuper {
    static {
      ...
      if (_metadata)
        Object.defineProperty(ReferencedSymbolDefinitionInfo, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
...
class TypeChecker {
  ...
  getContextualType(expression) {
    ...
    return contextualType == null
      ? undefined
      : this.#context.compilerFactory.getType(contextualType);
  }
  ...
  getSymbolAtLocation(node) {
    ...
    return compilerSymbol == null
      ? undefined
      : this.#context.compilerFactory.getSymbol(compilerSymbol);
  }
  getAliasedSymbol(symbol) {
    if (!symbol.hasFlags(common.SymbolFlags.Alias)) return undefined;
    const tsAliasSymbol = this.compilerObject.getAliasedSymbol(
      symbol.compilerSymbol,
    );
    return tsAliasSymbol == null
      ? undefined
      : this.#context.compilerFactory.getSymbol(tsAliasSymbol);
  }
  ...
  getExportSymbolOfSymbol(symbol) {
    return this.#context.compilerFactory.getSymbol(
      this.compilerObject.getExportSymbolOfSymbol(symbol.compilerSymbol),
    );
  }
  getPropertiesOfType(type) {
    return this.compilerObject
      .getPropertiesOfType(type.compilerType)
      .map((p) => this.#context.compilerFactory.getSymbol(p));
  }
  ...
  getSignatureFromNode(node) {
    ...
    return signature == null
      ? undefined
      : this.#context.compilerFactory.getSignature(signature);
  }
  getExportsOfModule(moduleSymbol) {
    ...
    return (symbols || []).map((s) =>
      this.#context.compilerFactory.getSymbol(s),
    );
  }
  ...
  getResolvedSignature(node) {
    ...
    if (!resolvedSignature || !resolvedSignature.declaration) return undefined;
  ...
  getResolvedSignatureOrThrow(node, message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Signature could not be resolved.",
      node,
    );
  }
  ...
  getSymbolsInScope(node, meaning) {
    return this.compilerObject
      .getSymbolsInScope(node.compilerNode, meaning)
      .map((s) => this.#context.compilerFactory.getSymbol(s));
  }
  getTypeArguments(typeReference) {
    return this.compilerObject
      .getTypeArguments(typeReference.compilerType)
  ...
  #getDefaultTypeFormatFlags(enclosingNode) {
    let formatFlags =
      common.TypeFormatFlags.UseTypeOfFunction |
      common.TypeFormatFlags.NoTruncation |
      common.TypeFormatFlags.UseFullyQualifiedType |
      common.TypeFormatFlags.WriteTypeArgumentsOfSignature;
    if (
      enclosingNode != null &&
      enclosingNode.getKind() === common.SyntaxKind.TypeAliasDeclaration
    )
      formatFlags |= common.TypeFormatFlags.InTypeAlias;
    return formatFlags;
  }
  getShorthandAssignmentValueSymbol(node) {
    const symbol = this.compilerObject.getShorthandAssignmentValueSymbol(
      node.compilerNode,
    );
    return symbol ? this.#context.compilerFactory.getSymbol(symbol) : undefined;
  }
  resolveName(name, location, meaning, excludeGlobals) {
    const symbol = this.compilerObject.resolveName(
      name,
...
class Program {
  ...
  _reset(rootNames, host) {
    ...
    this.#getOrCreateCompilerObject = () => {
      if (this.#createdCompilerObject == null) {
        this.#createdCompilerObject = common.ts.createProgram(
          rootNames,
          compilerOptions,
          host,
          this.#oldProgram,
  ...
  async emit(options = {}) {
    if (options.writeFile) {
      const message =
        `Cannot specify a ${common.nameof(options, "writeFile")} option when emitting asynchrously. ` +
  ...
  getSyntacticDiagnostics(sourceFile) {
    ...
    return compilerDiagnostics.map((d) =>
      this.#context.compilerFactory.getDiagnosticWithLocation(d),
    );
  }
  ...
  getConfigFileParsingDiagnostics() {
    const compilerDiagnostics =
      this.compilerObject.getConfigFileParsingDiagnostics();
    return compilerDiagnostics.map((d) =>
      this.#context.compilerFactory.getDiagnostic(d),
    );
  }
  getEmitModuleResolutionKind() {
    return common.getEmitModuleResolutionKind(
...
class LanguageService {
  ...
  constructor(params) {
    ...
    const { languageServiceHost, compilerHost } = common.createHosts({
      ...
      getNewLine: () =>
        this.#context.manipulationSettings.getNewLineKindAsString(),
    ...
    this.#compilerObject = common.ts.createLanguageService(
      languageServiceHost,
      this.#context.compilerFactory.documentRegistry,
    );
    ...
    this.#context.compilerFactory.onSourceFileAdded((sourceFile) => {
      if (sourceFile._isInProject()) this._reset();
    });
  ...
  _reset() {
    ...
    this.#program._reset(
      Array.from(this.#context.compilerFactory.getSourceFilePaths()),
      this.#compilerHost,
    );
  }
  getProgram() {
  ...
  getDefinitionsAtPosition(sourceFile, pos) {
    const results =
      this.compilerObject.getDefinitionAtPosition(
        sourceFile.getFilePath(),
        pos,
      ) || [];
    return results.map((info) =>
      this.#context.compilerFactory.getDefinitionInfo(info),
    );
  }
  ...
  getImplementationsAtPosition(sourceFile, pos) {
    const results =
      this.compilerObject.getImplementationAtPosition(
        sourceFile.getFilePath(),
        pos,
      ) || [];
    return results.map(
      (location) => new ImplementationLocation(this.#context, location),
    );
  }
  findReferences(node) {
  ...
  findReferencesAsNodes(node) {
    ...
    function* getReferencingNodes() {
      for (const referencedSymbol of referencedSymbols) {
        const isAlias =
          referencedSymbol.getDefinition().getKind() ===
          common.ts.ScriptElementKind.alias;
  ...
  findReferencesAtPosition(sourceFile, pos) {
    ...
    return results.map((s) =>
      this.#context.compilerFactory.getReferencedSymbol(s),
    );
  }
  findRenameLocations(node, options = {}) {
    const usePrefixAndSuffixText =
      options.usePrefixAndSuffixText == null
  ...
  getSuggestionDiagnostics(filePathOrSourceFile) {
    const filePath =
      this.#getFilePathFromFilePathOrSourceFile(filePathOrSourceFile);
    const suggestionDiagnostics =
      this.compilerObject.getSuggestionDiagnostics(filePath);
  ...
  getFormattingEditsForDocument(filePath, formatSettings) {
    ...
    return (
      this.compilerObject.getFormattingEditsForDocument(
        standardizedFilePath,
        this.#getFilledSettings(formatSettings),
      ) || []
    ).map((e) => new TextChange(e));
  ...
  getFormattedDocumentText(filePath, formatSettings) {
    const standardizedFilePath =
      this.#context.fileSystemWrapper.getStandardizedAbsolutePath(filePath);
    const sourceFile =
      this.#context.compilerFactory.getSourceFileFromCacheFromFilePath(
        standardizedFilePath,
      );
    if (sourceFile == null)
    ...
    if (
      formatSettings.ensureNewLineAtEndOfFile &&
      !newText.endsWith(newLineChar)
    )
      newText += newLineChar;
  ...
  organizeImports(
    ...
    return this.compilerObject
      .organizeImports(
        scope,
        this.#getFilledSettings(formatSettings),
        this.#getFilledUserPreferences(userPreferences),
      )
      .map(
        (fileTextChanges) =>
          new FileTextChanges(this.#context, fileTextChanges),
      );
  }
  getEditsForRefactor(
    filePathOrSourceFile,
    ...
    return compilerObject != null
      ? new RefactorEditInfo(this.#context, compilerObject)
      : undefined;
  }
  getCombinedCodeFix(
    filePathOrSourceFile,
  ...
  getCodeFixesAtPosition(
    ...
    return compilerResult.map(
      (compilerObject) => new CodeFixAction(this.#context, compilerObject),
    );
  }
  #getFilePathFromFilePathOrSourceFile(filePathOrSourceFile) {
    const filePath =
      typeof filePathOrSourceFile === "string"
        ? this.#context.fileSystemWrapper.getStandardizedAbsolutePath(
            filePathOrSourceFile,
          )
  ...
  #getFilledSettings(settings) {
    if (settings["_filled"]) return settings;
...
class Type {
  ...
  getArrayElementTypeOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Expected to find an array element type.",
    );
  }
  getArrayElementType() {
  ...
  getCallSignatures() {
    return this.compilerType
      .getCallSignatures()
      .map((s) => this._context.compilerFactory.getSignature(s));
  }
  getConstructSignatures() {
    return this.compilerType
      .getConstructSignatures()
  ...
  getConstraintOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getConstraint(),
      message ?? "Expected to find a constraint.",
    );
  }
  getConstraint() {
  ...
  getDefaultOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getDefault(),
      message ?? "Expected to find a default type.",
    );
  }
  getDefault() {
  ...
  getTargetTypeOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getTargetType(),
      message ?? "Expected to find the target type.",
    );
  }
  getTypeArguments() {
  ...
  getLiteralValueOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      ...
      message ?? "Type was not a literal type.",
    );
  }
  getLiteralFreshType() {
  ...
  getSymbolOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getSymbol(),
      message ?? "Expected to find a symbol.",
    );
  }
  ...
  isEnum() {
    ...
    if (hasEnumFlag) return true;
    if (this.isEnumLiteral() && !this.isUnion()) return false;
    const symbol = this.getSymbol();
    if (symbol == null) return false;
...
class TypeParameter extends Type {
  getConstraintOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getConstraint(),
      message ?? "Expected type parameter to have a constraint.",
    );
  }
  getConstraint() {
    ...
    if (constraintNode == null) return undefined;
    return this._context.typeChecker.getTypeAtLocation(constraintNode);
  }
  getDefaultOrThrow(message) {
    return common.errors.throwIfNullOrUndefined(
      this.getDefault(),
      message ?? "Expected type parameter to have a default type.",
    );
  }
  getDefault() {
    const declaration = this.#getTypeParameterDeclaration();
    ...
    if (defaultNode == null) return undefined;
  ...
  #getTypeParameterDeclaration() {
    ...
    const declaration = symbol.getDeclarations()[0];
    if (declaration == null) return undefined;
    if (!Node.isTypeParameterDeclaration(declaration)) return undefined;
    return declaration;
  }
}
...
class Directory {
  ...
  getParent() {
    if (common.FileUtils.isRootDirPath(this.getPath())) return undefined;
    return this.addDirectoryAtPathIfExists(
      common.FileUtils.getDirPath(this.getPath()),
    );
  }
  ...
  getSourceFile(pathOrCondition) {
    ...
    for (const sourceFile of this._getSourceFilesIterator()) {
      if (pathOrCondition(sourceFile)) return sourceFile;
    }
    return undefined;
  }
  getDirectories() {
  ...
  getSourceFiles(globPatterns) {
    ...
    function* getFilteredSourceFiles(globPatterns) {
      ...
      const matchedPaths = common.matchGlobs(
        sourceFilePaths,
        globPatterns,
        dir.getPath(),
      );
  ...
  *_getDescendantSourceFilesIterator() {
    for (const sourceFile of this._getSourceFilesIterator()) yield sourceFile;
  ...
  *_getDescendantDirectoriesIterator() {
  ...
  addSourceFilesAtPaths(fileGlobs) {
    ...
    fileGlobs = fileGlobs.map((g) => {
      if (common.FileUtils.pathIsAbsolute(g)) return g;
  ...
  async emit(options = {}) {
    ...
    for (const emitResult of this.#emitInternal(options)) {
      if (isStandardizedFilePath(emitResult)) skippedFilePaths.push(emitResult);
      else {
        writeTasks.push(
  ...
  #emitInternal(options = {}) {
    ...
    const isDtsFile =
      options.declarationDir == null && options.outDir == null
        ? undefined
        : /\.d\.ts$/i;
      ...
      for (const sourceFile of directory.getSourceFiles()) {
        ...
        for (const outputFile of output.getOutputFiles()) {
          ...
          const fileText = outputFile.getWriteByteOrderMark()
            ? common.FileUtils.getTextWithByteOrderMark(outputFile.getText())
            : outputFile.getText();
          if (
            outDir != null &&
            (isJsFile.test(filePath) ||
              isMapFile.test(filePath) ||
              (!hasDeclarationDir && isDtsFile.test(filePath)))
          )
            filePath = common.FileUtils.pathJoin(
              outDir,
  ...
  copy(relativeOrAbsolutePath, options) {
    ...
    if (originalPath === newPath) return this;
  ...
  move(relativeOrAbsolutePath, options) {
    ...
    if (originalPath === newPath) return this;
    return this.#moveInternal(newPath, options, () =>
  ...
  #moveInternal(newPath, options, preAction) {
    ...
    if (originalPath === newPath) return this;
    const existingDir =
      this._context.compilerFactory.getDirectoryFromCacheOnlyIfInCache(newPath);
    ...
    for (const { directory, oldPath, newDirPath } of movingDirectories) {
      ...
      if (dirToOverwrite != null) dirToOverwrite._forgetOnlyThis();
    ...
    if (markInProject) this._markAsInProject();
    return this;
  }
  clear() {
  ...
  #deleteDescendants() {
    for (const sourceFile of this.getSourceFiles()) sourceFile.delete();
    for (const dir of this.getDirectories()) dir.delete();
  }
  async deleteImmediately() {
    const { fileSystemWrapper } = this._context;
    const path = this.getPath();
  ...
  forget() {
    if (this.wasForgotten()) return;
    for (const sourceFile of this.getSourceFiles()) sourceFile.forget();
    for (const dir of this.getDirectories()) dir.forget();
    this._forgetOnlyThis();
  }
  _forgetOnlyThis() {
    if (this.wasForgotten()) return;
  ...
  async save() {
    ...
    const unsavedSourceFiles = this.getDescendantSourceFiles().filter(
      (s) => !s.isSaved(),
    );
  ...
  getRelativePathAsModuleSpecifierTo(sourceFileDirOrFilePath) {
    ...
    const thisDirectory = this;
    const moduleSpecifier = common.FileUtils.getRelativePathTo(
      this.getPath(),
      getPath(),
    ).replace(/((\.d\.ts$)|(\.[^/.]+$))/i, "");
    ...
    function getPath() {
      ...
      function getPathForFilePath(filePath) {
        ...
        switch (moduleResolution) {
          case common.ModuleResolutionKind.Node10:
            if (dirPath === thisDirectory.getPath()) return filePath;
  ...
  #getReferencesForMove(sourceFile) {
    const { literalReferences, referencingLiterals } =
      sourceFile._getReferencesForMoveInternal();
    return {
      literalReferences: literalReferences.filter(
        (r) => !this.isAncestorOf(r[1]),
      ),
      referencingLiterals: referencingLiterals.filter(
        (l) => !this.isAncestorOf(l._sourceFile),
      ),
    };
  }
  static #isAncestorOfDir(ancestor, descendant) {
    ...
    for (let i = ancestor.#pathParts.length - 1; i >= 0; i--) {
      if (ancestor.#pathParts[i] !== descendant.#pathParts[i]) return false;
    }
    return true;
  }
}
...
class DirectoryCoordinator {
  ...
  addDirectoryAtPathIfExists(dirPath, options) {
    ...
    if (options.recursive) {
      for (const descendantDirPath of common.FileUtils.getDescendantDirectories(
  ...
  addSourceFileAtPath(filePath, options) {
    ...
    if (sourceFile == null)
      throw new common.errors.FileNotFoundError(
  ...
  addSourceFilesAtPaths(fileGlobs, options) {
    ...
    for (const dirPath of common.FileUtils.getParentMostPaths(
...
class DirectoryCache {
  ...
  *getAllByDepth() {
    ...
    let depth = 0;
    for (const orphanDir of this.getOrphans()) addToDirLevels(orphanDir);
  ...
  *getChildDirectoriesOfDirectory(dirPath) {
    ...
    for (const dir of entries) yield dir;
  }
  *getChildSourceFilesOfDirectory(dirPath) {
  ...
  addSourceFile(sourceFile) {
    ...
    const sourceFiles = this.#sourceFilesByDirPath.getOrCreate(
      dirPath,
      () =>
        new common.SortedKeyValueArray(
          (item) => item.getBaseName(),
          common.LocaleStringComparer.instance,
        ),
    );
  ...
  createOrAddIfExists(dirPath) {
    if (this.has(dirPath)) return this.get(dirPath);
  ...
  addDirectory(directory) {
    ...
    if (!isRootDir) this.#addToDirectoriesByDirPath(directory);
    if (!this.has(parentDirPath)) this.#orphanDirs.set(path, directory);
  ...
  #addToDirectoriesByDirPath(directory) {
    if (common.FileUtils.isRootDirPath(directory.getPath())) return;
  ...
  #removeFromDirectoriesByDirPath(dirPath) {
    if (common.FileUtils.isRootDirPath(dirPath)) return;
...
class ForgetfulNodeCache extends common.KeyValueCache {
  ...
  forgetLastPoint() {
    ...
    if (nodes != null) this.#forgetNodes(nodes.values());
  }
  rememberNode(node) {
    ...
    if (wasInForgetStack) this.#rememberParentOfNode(node);
    return wasInForgetStack;
  }
  #rememberParentOfNode(node) {
    const parent = node.getParentSyntaxList() || node.getParent();
    if (parent != null) this.rememberNode(parent);
  }
  #forgetNodes(nodes) {
    for (const node of nodes) {
      if (
        node.wasForgotten() ||
        node.getKind() === common.SyntaxKind.SourceFile
      )
        continue;
      node._forgetOnlyThis();
    }
...
class CompilerFactory {
  ...
  documentRegistry;
  constructor(context) {
    this.documentRegistry = new common.DocumentRegistry(
      context.fileSystemWrapper,
    );
  ...
  *getSourceFilesByDirectoryDepth() {
    for (const dir of this.getDirectoriesByDepth()) yield* dir.getSourceFiles();
  }
  getSourceFilePaths() {
  ...
  onSourceFileAdded(subscription, subscribe = true) {
    if (subscribe) this.#sourceFileAddedEventContainer.subscribe(subscription);
    else this.#sourceFileAddedEventContainer.unsubscribe(subscription);
  }
  ...
  createSourceFile(filePath, sourceFileText, options) {
    sourceFileText =
      sourceFileText instanceof Function
        ? getTextFromStringOrWriter(
            this.#context.createWriter(),
            sourceFileText,
          )
        : sourceFileText || "";
    ...
    const structurePrinter =
      this.#context.structurePrinterFactory.forSourceFile({
  ...
  createSourceFileFromText(filePath, sourceText, options) {
    ...
    return this.#createSourceFileFromTextInternal(
      filePath,
      sourceText,
      options,
    );
  ...
  containsDirectoryAtPath(dirPath) {
    dirPath =
      this.#context.fileSystemWrapper.getStandardizedAbsolutePath(dirPath);
  ...
  getSourceFileForNode(compilerNode) {
    ...
    while (currentNode.kind !== common.SyntaxKind.SourceFile) {
      if (currentNode.parent == null) return undefined;
  ...
  getNodeFromCompilerNode(compilerNode, sourceFile) {
    ...
    function initializeNode(node) {
      ...
      if (parentSyntaxList != null) parentSyntaxList._wrappedChildCount++;
      if (compilerNode.kind === common.SyntaxKind.SyntaxList) {
        let count = 0;
        for (const _ of node._getChildrenInCacheIterator()) count++;
  ...
  #createSourceFileFromTextInternal(filePath, text, options) {
    ...
    if (hasBom) text = common.StringUtils.stripBom(text);
    const sourceFile = this.getSourceFile(
      this.createCompilerSourceFileFromText(filePath, text, options.scriptKind),
      options,
    );
    if (hasBom) sourceFile._hasBom = true;
    return sourceFile;
  }
  createCompilerSourceFileFromText(filePath, text, scriptKind) {
    return this.documentRegistry.createOrUpdateSourceFile(
      ...
      common.ts.ScriptSnapshot.fromString(text),
      scriptKind,
    );
  }
  getSourceFile(compilerSourceFile, options) {
    let wasAdded = false;
    const sourceFile =
      this.#sourceFileCacheByFilePath.get(compilerSourceFile.fileName) ??
    ...
    if (options.markInProject) sourceFile._markAsInProject();
    if (wasAdded) this.#sourceFileAddedEventContainer.fire(sourceFile);
    return sourceFile;
  }
  ...
  getType(type) {
    if (
      (type.flags & common.TypeFlags.TypeParameter) ===
      common.TypeFlags.TypeParameter
    )
      return this.getTypeParameter(type);
    return this.#typeCache.getOrCreate(
      type,
      () => new Type(this.#context, type),
    );
  }
  ...
  getSymbol(symbol) {
    return this.#symbolCache.getOrCreate(
      symbol,
      () => new Symbol$1(this.#context, symbol),
    );
  }
  ...
  replaceCompilerNode(oldNode, newNode) {
    const nodeToReplace =
      oldNode instanceof Node ? oldNode.compilerNode : oldNode;
    const node =
      oldNode instanceof Node ? oldNode : this.#nodeCache.get(oldNode);
    if (
      nodeToReplace.kind === common.SyntaxKind.SourceFile &&
      ...
      if (node != null) node._replaceCompilerNodeFromFactory(newNode);
    }
  }
  ...
  #removeCompilerNodeFromCache(compilerNode) {
    ...
    if (compilerNode.kind === common.SyntaxKind.SourceFile) {
      ...
      const wrappedSourceFile =
        this.#sourceFileCacheByFilePath.get(standardizedFilePath);
  ...
  forgetNodesCreatedInBlock(block) {
    ...
    try {
      result = block((...nodes) => {
        for (const node of nodes) this.#nodeCache.rememberNode(node);
      });
      if (Node.isNode(result)) this.#nodeCache.rememberNode(result);
      if (isPromise(result)) {
        wasPromise = true;
        return result.then((value) => {
          if (Node.isNode(value)) this.#nodeCache.rememberNode(value);
          this.#nodeCache.forgetLastPoint();
          return value;
        });
      }
    } finally {
      if (!wasPromise) this.#nodeCache.forgetLastPoint();
    }
    return result;
...
class InProjectCoordinator {
  ...
  markSourceFileAsInProject(sourceFile) {
    if (this.isSourceFileInProject(sourceFile)) return;
  ...
  markSourceFilesAsInProjectForResolution() {
    ...
    function shouldMarkInProject(sourceFile) {
      ...
      const nodeModulesDir =
        compilerFactory.getDirectoryFromCacheOnlyIfInCache(nodeModulesPath);
      if (nodeModulesDir != null && nodeModulesDir._isInProject()) return true;
      let directory = sourceFile.getDirectory();
      while (directory != null && directory.getPath() !== nodeModulesPath) {
        if (directory._isInProject()) return true;
        directory = compilerFactory.getDirectoryFromCacheOnlyIfInCache(
          common.FileUtils.getDirPath(directory.getPath()),
        );
      }
      return false;
    }
  ...
  markDirectoryAsInProject(directory) {
    if (this.isDirectoryInProject(directory)) return;
    ...
    function markAncestorDirs(dir) {
      ...
      if (
        ...
        return;
      for (const ancestorDir of ancestorDirs) ancestorDir._inProject = true;
    }
    function* getAncestorsUpToOneInProject(dir) {
      if (common.FileUtils.isRootDirPath(dir.getPath())) return;
      const parentDirPath = common.FileUtils.getDirPath(dir.getPath());
      const parentDir =
        compilerFactory.getDirectoryFromCacheOnlyIfInCache(parentDirPath);
  ...
  return class StructurePrinterFactory {
    static {
      ...
      __esDecorate(
        ...
        null,
        _forInitializerExpressionableNode_decorators,
        {
          kind: "method",
          name: "forInitializerExpressionableNode",
          static: false,
          private: false,
          access: {
            has: (obj) => "forInitializerExpressionableNode" in obj,
            get: (obj) => obj.forInitializerExpressionableNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forModifierableNode_decorators,
        {
          kind: "method",
          name: "forModifierableNode",
          static: false,
          private: false,
          access: {
            has: (obj) => "forModifierableNode" in obj,
            get: (obj) => obj.forModifierableNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forReturnTypedNode_decorators,
        {
          kind: "method",
          name: "forReturnTypedNode",
          static: false,
          private: false,
          access: {
            has: (obj) => "forReturnTypedNode" in obj,
            get: (obj) => obj.forReturnTypedNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forTypedNode_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forTypedNode" in obj,
            get: (obj) => obj.forTypedNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forClassDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forClassDeclaration" in obj,
            get: (obj) => obj.forClassDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forClassMember_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forClassMember" in obj,
            get: (obj) => obj.forClassMember,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forClassStaticBlockDeclaration_decorators,
        {
          kind: "method",
          name: "forClassStaticBlockDeclaration",
          static: false,
          private: false,
          access: {
            has: (obj) => "forClassStaticBlockDeclaration" in obj,
            get: (obj) => obj.forClassStaticBlockDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forConstructorDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forConstructorDeclaration" in obj,
            get: (obj) => obj.forConstructorDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forGetAccessorDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forGetAccessorDeclaration" in obj,
            get: (obj) => obj.forGetAccessorDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forMethodDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forMethodDeclaration" in obj,
            get: (obj) => obj.forMethodDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forPropertyDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forPropertyDeclaration" in obj,
            get: (obj) => obj.forPropertyDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forSetAccessorDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forSetAccessorDeclaration" in obj,
            get: (obj) => obj.forSetAccessorDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forDecorator_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forDecorator" in obj,
            get: (obj) => obj.forDecorator,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forJSDoc_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forJSDoc" in obj,
            get: (obj) => obj.forJSDoc,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forJSDocTag_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forJSDocTag" in obj,
            get: (obj) => obj.forJSDocTag,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forEnumDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forEnumDeclaration" in obj,
            get: (obj) => obj.forEnumDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forEnumMember_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forEnumMember" in obj,
            get: (obj) => obj.forEnumMember,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        {
          kind: "method",
          name: "forObjectLiteralExpressionProperty",
          static: false,
          private: false,
          access: {
            has: (obj) => "forObjectLiteralExpressionProperty" in obj,
            get: (obj) => obj.forObjectLiteralExpressionProperty,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forPropertyAssignment_decorators,
        {
          kind: "method",
          name: "forPropertyAssignment",
          static: false,
          private: false,
          access: {
            has: (obj) => "forPropertyAssignment" in obj,
            get: (obj) => obj.forPropertyAssignment,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forShorthandPropertyAssignment_decorators,
        {
          kind: "method",
          name: "forShorthandPropertyAssignment",
          static: false,
          private: false,
          access: {
            has: (obj) => "forShorthandPropertyAssignment" in obj,
            get: (obj) => obj.forShorthandPropertyAssignment,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forSpreadAssignment_decorators,
        {
          kind: "method",
          name: "forSpreadAssignment",
          static: false,
          private: false,
          access: {
            has: (obj) => "forSpreadAssignment" in obj,
            get: (obj) => obj.forSpreadAssignment,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forFunctionDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forFunctionDeclaration" in obj,
            get: (obj) => obj.forFunctionDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forParameterDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forParameterDeclaration" in obj,
            get: (obj) => obj.forParameterDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forCallSignatureDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forCallSignatureDeclaration" in obj,
            get: (obj) => obj.forCallSignatureDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forConstructSignatureDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forConstructSignatureDeclaration" in obj,
            get: (obj) => obj.forConstructSignatureDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forIndexSignatureDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forIndexSignatureDeclaration" in obj,
            get: (obj) => obj.forIndexSignatureDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forInterfaceDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forInterfaceDeclaration" in obj,
            get: (obj) => obj.forInterfaceDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forMethodSignature_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forMethodSignature" in obj,
            get: (obj) => obj.forMethodSignature,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forPropertySignature_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forPropertySignature" in obj,
            get: (obj) => obj.forPropertySignature,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forTypeElementMemberedNode_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forTypeElementMemberedNode" in obj,
            get: (obj) => obj.forTypeElementMemberedNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forTypeElementMember_decorators,
        {
          kind: "method",
          name: "forTypeElementMember",
          static: false,
          private: false,
          access: {
            has: (obj) => "forTypeElementMember" in obj,
            get: (obj) => obj.forTypeElementMember,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forJsxAttributeDecider_decorators,
        {
          kind: "method",
          name: "forJsxAttributeDecider",
          static: false,
          private: false,
          access: {
            has: (obj) => "forJsxAttributeDecider" in obj,
            get: (obj) => obj.forJsxAttributeDecider,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forJsxAttribute_decorators,
        {
          kind: "method",
          name: "forJsxAttribute",
          static: false,
          private: false,
          access: {
            has: (obj) => "forJsxAttribute" in obj,
            get: (obj) => obj.forJsxAttribute,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forJsxChildDecider_decorators,
        {
          kind: "method",
          name: "forJsxChildDecider",
          static: false,
          private: false,
          access: {
            has: (obj) => "forJsxChildDecider" in obj,
            get: (obj) => obj.forJsxChildDecider,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forJsxElement_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forJsxElement" in obj,
            get: (obj) => obj.forJsxElement,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forJsxNamespacedName_decorators,
        {
          kind: "method",
          name: "forJsxNamespacedName",
          static: false,
          private: false,
          access: {
            has: (obj) => "forJsxNamespacedName" in obj,
            get: (obj) => obj.forJsxNamespacedName,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forJsxSelfClosingElement_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forJsxSelfClosingElement" in obj,
            get: (obj) => obj.forJsxSelfClosingElement,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forJsxSpreadAttribute_decorators,
        {
          kind: "method",
          name: "forJsxSpreadAttribute",
          static: false,
          private: false,
          access: {
            has: (obj) => "forJsxSpreadAttribute" in obj,
            get: (obj) => obj.forJsxSpreadAttribute,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forExportAssignment_decorators,
        {
          kind: "method",
          name: "forExportAssignment",
          static: false,
          private: false,
          access: {
            has: (obj) => "forExportAssignment" in obj,
            get: (obj) => obj.forExportAssignment,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forExportDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forExportDeclaration" in obj,
            get: (obj) => obj.forExportDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forImportAttribute_decorators,
        {
          kind: "method",
          name: "forImportAttribute",
          static: false,
          private: false,
          access: {
            has: (obj) => "forImportAttribute" in obj,
            get: (obj) => obj.forImportAttribute,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forImportDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forImportDeclaration" in obj,
            get: (obj) => obj.forImportDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forModuleDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forModuleDeclaration" in obj,
            get: (obj) => obj.forModuleDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forNamedImportExportSpecifier_decorators,
        {
          kind: "method",
          name: "forNamedImportExportSpecifier",
          static: false,
          private: false,
          access: {
            has: (obj) => "forNamedImportExportSpecifier" in obj,
            get: (obj) => obj.forNamedImportExportSpecifier,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forSourceFile_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forSourceFile" in obj,
            get: (obj) => obj.forSourceFile,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forStatementedNode_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forStatementedNode" in obj,
            get: (obj) => obj.forStatementedNode,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ...
        _forStatement_decorators,
        {
          ...
          private: false,
          access: {
            has: (obj) => "forStatement" in obj,
            get: (obj) => obj.forStatement,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forVariableStatement_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forVariableStatement" in obj,
            get: (obj) => obj.forVariableStatement,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forTypeAliasDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forTypeAliasDeclaration" in obj,
            get: (obj) => obj.forTypeAliasDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forTypeParameterDeclaration_decorators,
        {
          kind: "method",
          name: "forTypeParameterDeclaration",
          static: false,
          private: false,
          access: {
            has: (obj) => "forTypeParameterDeclaration" in obj,
            get: (obj) => obj.forTypeParameterDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        StructurePrinterFactory,
        null,
        _forVariableDeclaration_decorators,
        {
          kind: "method",
          ...
          private: false,
          access: {
            has: (obj) => "forVariableDeclaration" in obj,
            get: (obj) => obj.forVariableDeclaration,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(StructurePrinterFactory, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    _getFormatCodeSettings = __runInitializers(
      this,
      _instanceExtraInitializers,
    );
  ...
  return class ProjectContext {
    static {
      ...
      __esDecorate(
        ProjectContext,
        null,
        _getSourceFileContainer_decorators,
        {
          kind: "method",
          name: "getSourceFileContainer",
          static: false,
          private: false,
          access: {
            has: (obj) => "getSourceFileContainer" in obj,
            get: (obj) => obj.getSourceFileContainer,
          },
          metadata: _metadata,
        },
        null,
      ...
      __esDecorate(
        ProjectContext,
        null,
        _getModuleResolutionHost_decorators,
        {
          kind: "method",
          name: "getModuleResolutionHost",
          static: false,
          private: false,
          access: {
            has: (obj) => "getModuleResolutionHost" in obj,
            get: (obj) => obj.getModuleResolutionHost,
          },
          metadata: _metadata,
        },
        null,
      ...
      if (_metadata)
        Object.defineProperty(ProjectContext, Symbol.metadata, {
          enumerable: true,
          configurable: true,
          writable: true,
          value: _metadata,
    ...
    constructor(params) {
      ...
      this.inProjectCoordinator = new InProjectCoordinator(
        this.compilerFactory,
      );
    ...
    get typeChecker() {
      if (this.#customTypeChecker != null) return this.#customTypeChecker;
      if (this.#languageService == null)
    ...
    createWriter() {
      ...
      return new CodeBlockWriter__default.default({
        ...
        indentNumberOfSpaces:
          indentationText === exports.IndentationText.Tab
            ? undefined
            : indentationText.length,
    ...
    getPreEmitDiagnostics(sourceFile) {
      const compilerDiagnostics = common.ts.getPreEmitDiagnostics(
      ...
      return {
        addOrGetSourceFileFromFilePath: (filePath, opts) => {
          return Promise.resolve(
            this.compilerFactory.addOrGetSourceFileFromFilePath(filePath, opts)
              ?.compilerNode,
          );
        },
        addOrGetSourceFileFromFilePathSync: (filePath, opts) => {
          return this.compilerFactory.addOrGetSourceFileFromFilePath(
            filePath,
            opts,
          )?.compilerNode;
        },
        ...
        getSourceFileVersion: (sourceFile) =>
          this.compilerFactory.documentRegistry.getSourceFileVersion(
            sourceFile,
          ),
        getChildDirectoriesOfDirectory: (dirPath) => {
          const result = [];
          for (const dir of this.compilerFactory.getChildDirectoriesOfDirectory(
            dirPath,
          ))
...
class Project {
  ...
  constructor(options = {}) {
    ...
    if (
      ...
      this.#addSourceFilesForTsConfigResolver(
        tsConfigResolver,
        compilerOptions,
      );
  ...
  resolveSourceFileDependencies() {
    ...
    const result =
      inProjectCoordinator.markSourceFilesAsInProjectForResolution();
  ...
  addSourceFilesFromTsConfig(tsConfigFilePath) {
    const resolver = new common.TsConfigResolver(
    ...
    return this.#addSourceFilesForTsConfigResolver(
      resolver,
  ...
  getSourceFileOrThrow(fileNameOrSearchFunction) {
    ...
    if (typeof fileNameOrSearchFunction === "string") {
      const fileNameOrPath = common.FileUtils.standardizeSlashes(
        fileNameOrSearchFunction,
      );
      if (
        common.FileUtils.pathIsAbsolute(fileNameOrPath) ||
        fileNameOrPath.indexOf("/") >= 0
      ) {
        const errorFileNameOrPath =
          this._context.fileSystemWrapper.getStandardizedAbsolutePath(
            fileNameOrPath,
          );
        throw new common.errors.InvalidOperationError(
  ...
  getSourceFile(fileNameOrSearchFunction) {
    ...
    return common.IterableUtils.find(
      this.#getProjectSourceFilesByDirectoryDepth(),
      filePathOrSearchFunction,
    );
    function getFilePathOrSearchFunction(fileSystemWrapper) {
      ...
      if (
        ...
        return (def) =>
          common.FileUtils.pathEndsWith(def.getFilePath(), fileNameOrPath);
    }
  ...
  getSourceFiles(globPatterns) {
    ...
    else return Array.from(sourceFiles);
    function* getFilteredSourceFiles() {
      ...
      function* getSourceFilePaths() {
        for (const sourceFile of sourceFiles) yield sourceFile.getFilePath();
      }
    }
  }
  ...
  *#getProjectDirectoriesByDirectoryDepth() {
    ...
    for (const directory of compilerFactory.getDirectoriesByDepth()) {
      if (inProjectCoordinator.isDirectoryInProject(directory)) yield directory;
    }
  }
  ...
  saveSync() {
    ...
    for (const file of this.#getUnsavedSourceFiles()) file.saveSync();
  }
  ...
  #getUnsavedSourceFiles() {
    ...
    function* getUnsavedIterator(sourceFiles) {
      for (const sourceFile of sourceFiles) {
        if (!sourceFile.isSaved()) yield sourceFile;
      }
    }
  }
  getPreEmitDiagnostics() {
  ...
  formatDiagnosticsWithColorAndContext(diagnostics, opts = {}) {
    return common.ts.formatDiagnosticsWithColorAndContext(
...
function createWrappedNode(node, opts = {}) {
  ...
  function getSourceFileFromNode(compilerNode) {
    if (compilerNode.kind === common.SyntaxKind.SourceFile) return compilerNode;
    if (compilerNode.parent == null)
      throw new common.errors.InvalidOperationError(
...
function writeValue(writer, value) {
  if (value instanceof Function) value(writer);
...
Object.defineProperty(exports, "CompilerOptionsContainer", {
  enumerable: true,
  get: () => common.CompilerOptionsContainer,
});
Object.defineProperty(exports, "DiagnosticCategory", {
  enumerable: true,
  get: () => common.DiagnosticCategory,
});
...
Object.defineProperty(exports, "CodeBlockWriter", {
  enumerable: true,
  get: () => CodeBlockWriter__default.default,
});
...
exports.CompilerCommentObjectLiteralElement =
