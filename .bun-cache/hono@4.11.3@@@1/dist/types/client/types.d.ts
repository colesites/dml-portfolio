...
export type BuildSearchParamsFn = (
...
type ParseHostName<T extends string> = T extends `${infer Host}:${infer Port}`
  ? [Host, Port]
  : [T, ""];
type TrimStartSlash<T extends string> = T extends `/${infer R}`
  ? TrimStartSlash<R>
  : T;
type TrimEndSlash<T extends string> = T extends `${infer R}/`
  ? TrimEndSlash<R>
  : T;
type IsLiteral<T extends string> = [T] extends [never]
  ? false
  : string extends T
    ? false
    : true;
...
export type Client<T, Prefix extends string> = T extends HonoBase<
  any,
  infer S,
  any
>
