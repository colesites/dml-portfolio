...
var intrinsicElementTags = __toESM(
  require("./intrinsic-element/components"),
  1,
);
var import_utils = require("./utils");
let nameSpaceContext = void 0;
const getNameSpaceContext = () => nameSpaceContext;
const toSVGAttributeName = (key) =>
...
class JSXNode {
  ...
  toString() {
    ...
    return buffer.length === 1
      ? "callbacks" in buffer
        ? (0, import_html2.resolveCallbackSync)(
            (0, import_html.raw)(buffer[0], buffer.callbacks),
          ).toString()
        : buffer[0]
      : (0, import_html2.stringBufferToString)(buffer, buffer.callbacks);
  }
  toStringToBuffer(buffer) {
    ...
    const normalizeKey =
      nameSpaceContext &&
      (0, import_context.useContext)(nameSpaceContext) === "svg"
        ? (key) =>
            toSVGAttributeName(
              (0, import_utils.normalizeIntrinsicElementKey)(key),
            )
  ...
  toStringToBuffer(buffer) {
    ...
    if (typeof res === "boolean" || res == null) {
      ...
      if (import_context.globalContexts.length === 0) {
        ...
        const currentContexts = import_context.globalContexts.map((c) => [
          c,
          c.values.at(-1),
        ]);
        buffer.unshift(
...
const jsxFn = (tag, props, children) => {
  if (!initDomRenderer) {
    for (const k in import_common.domRenderers) {
      intrinsicElementTags[k][import_constants.DOM_RENDERER] =
        import_common.domRenderers[k];
    }
