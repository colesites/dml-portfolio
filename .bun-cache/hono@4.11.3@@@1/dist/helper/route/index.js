...
var routePath = (c, index) =>
  matchedRoutes(c).at(index ?? c.req.routeIndex)?.path ?? "";
var baseRoutePath = (c, index) =>
...
var basePath = (c, index) => {
  ...
  if (!/[:*]/.test(rp)) {
    ...
    for (let i = 0, len = paths.length; i < len; i++) {
      ...
      if (pattern) {
        const re =
          pattern[2] === true || pattern === "*" ? /[^/]+/ : pattern[2];
        basePathLength +=
          reqPath.substring(basePathLength + 1).match(re)?.[0].length || 0;
      } else {
...
export { basePath, baseRoutePath, matchedRoutes, routePath };
