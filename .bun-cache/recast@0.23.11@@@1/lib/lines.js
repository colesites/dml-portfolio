...
var Lines = /** @class */ (() => {
  ...
  Lines.prototype.getSourceMap = function (sourceMapName, sourceRoot) {
    ...
    if (this.cachedSourceMap) {
      // Since Lines objects are immutable, we can reuse any source map
      // that was previously generated. Nevertheless, we return a new
      // JSON object here to protect the cached source map from outside
      // modification.
      return updateJSON(this.cachedSourceMap.toJSON());
    }
    ...
    this.mappings.forEach((mapping) => {
      ...
      var targetCursor =
        this.skipSpaces(mapping.targetLoc.start) || this.lastPos();
      while (
        (0, util_1.comparePos)(sourceCursor, mapping.sourceLoc.end) < 0 &&
        (0, util_1.comparePos)(targetCursor, mapping.targetLoc.end) < 0
      ) {
        var sourceChar = mapping.sourceLines.charAt(sourceCursor);
        var targetChar = this.charAt(targetCursor);
        ...
        this.nextPos(targetCursor, true);
        mapping.sourceLines.nextPos(sourceCursor, true);
      }
    });
    this.cachedSourceMap = smg;
    return smg.toJSON();
  };
  ...
  Lines.prototype.lockIndentTail = function () {
    ...
    return new Lines(
      this.infos.map((info, i) =>
        tslib_1.__assign(tslib_1.__assign({}, info), { locked: i > 0 }),
      ),
    );
  };
  Lines.prototype.getIndentAt = function (line) {
    (0, tiny_invariant_1.default)(
      line >= 1,
      "no line " + line + " (line numbers start from 1)",
    );
  ...
  Lines.prototype.sliceString = function (start, end, options) {
    ...
    var _a = (0, options_1.normalize)(options),
    ...
    function appendLines(linesOrNull) {
      ...
      if (prevInfo) {
        ...
        if (linesOrNull.mappings.length > 0) {
          linesOrNull.mappings.forEach((mapping) => {
      ...
      linesOrNull.infos.forEach((info, i) => {
        if (!prevInfo || i > 0) {
...
function fromString(string, options) {
  ...
  var lines = new Lines(
    string.split(lineTerminatorSeqExp).map((line) => {
