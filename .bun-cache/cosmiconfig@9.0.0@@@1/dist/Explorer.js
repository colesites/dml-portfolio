...
class Explorer extends ExplorerBase_js_1.ExplorerBase {
    ...
    const search = async () => {
      ...
      if (await (0, util_js_1.isDirectory)(currentDir.path)) {
        for (const filepath of this.getSearchPlacesForDir(
          currentDir,
          defaults_1.globalConfigSearchPlaces,
        )) {
          try {
      ...
      if (!nextDirIter.done) {
        ...
        if (this.searchCache) {
          return await (0, util_js_1.emplace)(
            this.searchCache,
            currentDir.path,
            search,
          );
  ...
  async #loadConfiguration(filepath, contents) {
    ...
    if (!loader) {
      throw new Error(
        `No loader specified for ${(0, ExplorerBase_js_1.getExtensionDescription)(extension)}`,
      );
    }
    try {
      ...
      case "project": {
        ...
        while (true) {
          ...
          for (const ext of ["json", "yaml"]) {
            const packageFile = path_1.default.join(
              currentDir,
              `package.${ext}`,
            );
