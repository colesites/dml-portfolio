  ...
  #validateConfig() {
    ...
    for (const place of config.searchPlaces) {
      ...
      if (typeof loader !== "function") {
        throw new Error(
          `Loader for ${getExtensionDescription(place)} is not a function: Received ${typeof loader}.`,
        );
      }
    }
  }
...
${[...importStack, fullPath]
  ...
  getSearchPlacesForDir(dir, globalConfigPlaces) {
    ...
    ).map((place) => path_1.default.join(dir.path, place));
  }
  getGlobalConfigDir() {
    return (0, env_paths_1.default)(this.config.moduleName, { suffix: "" })
      .config;
  }
  *getGlobalDirs(startDir) {
    const stopDir = path_1.default.resolve(
      this.config.stopDir ?? os_1.default.homedir(),
    );
