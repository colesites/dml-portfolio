...
class ZodType {
  ...
  "~validate"(data) {
    ...
    return this._parseAsync({ data, path: [], parent: ctx }).then((result) =>
      (0, parseUtil_js_1.isValid)(result)
        ? {
            value: result.value,
          }
        : {
  ...
  async safeParseAsync(data, params) {
    ...
    const result = await ((0, parseUtil_js_1.isAsync)(maybeAsyncResult)
      ? maybeAsyncResult
...
class ZodNumber extends ZodType {
  ...
  gte(value, message) {
    return this.setLimit(
      ...
      value,
      true,
      errorUtil_js_1.errorUtil.toString(message),
    );
  }
  gt(value, message) {
    return this.setLimit(
  ...
  get isInt() {
    return !!this._def.checks.find(
      (ch) =>
        ch.kind === "int" ||
        (ch.kind === "multipleOf" && util_js_1.util.isInteger(ch.value)),
    );
  }
  get isFinite() {
    let max = null;
  ...
  _parse(input) {
    ...
    if (def.exactLength !== null) {
      ...
      if (tooBig || tooSmall) {
        (0, parseUtil_js_1.addIssueToContext)(ctx, {
          code: tooBig
            ? ZodError_js_1.ZodIssueCode.too_big
            : ZodError_js_1.ZodIssueCode.too_small,
        ...
        ? {
            errorMap: (issue, ctx) => {
              ...
              if (issue.code === "unrecognized_keys")
                return {
                  message:
                    errorUtil_js_1.errorUtil.errToObj(message).message ??
                    defaultError,
                };
              return {
...
function mergeValues(a, b) {
  ...
  if (a === b) {
    ...
    aType === util_js_1.ZodParsedType.object &&
    bType === util_js_1.ZodParsedType.object
  ) {
    const bKeys = util_js_1.util.objectKeys(b);
    const sharedKeys = util_js_1.util
      .objectKeys(a)
    ...
    aType === util_js_1.ZodParsedType.array &&
    bType === util_js_1.ZodParsedType.array
  ) {
    ...
    aType === util_js_1.ZodParsedType.date &&
    bType === util_js_1.ZodParsedType.date &&
    +a === +b
  ) {
    return { valid: true, data: a };
  } else {
...
class ZodIntersection extends ZodType {
  _parse(input) {
    ...
    const handleParsed = (parsedLeft, parsedRight) => {
      if (
        (0, parseUtil_js_1.isAborted)(parsedLeft) ||
        (0, parseUtil_js_1.isAborted)(parsedRight)
      ) {
        return parseUtil_js_1.INVALID;
      }
      ...
      if (
        (0, parseUtil_js_1.isDirty)(parsedLeft) ||
        (0, parseUtil_js_1.isDirty)(parsedRight)
      ) {
        status.dirty();
      }
...
class ZodNativeEnum extends ZodType {
  _parse(input) {
    const nativeEnumValues = util_js_1.util.getValidEnumValues(
      this._def.values,
    );
    const ctx = this._getOrReturnCtx(input);
    if (
      ctx.parsedType !== util_js_1.ZodParsedType.string &&
      ctx.parsedType !== util_js_1.ZodParsedType.number
    ) {
    ...
    if (!this._cache) {
      this._cache = new Set(
        util_js_1.util.getValidEnumValues(this._def.values),
      );
    }
    if (!this._cache.has(input.data)) {
      const expectedValues = util_js_1.util.objectValues(nativeEnumValues);
...
class ZodPromise extends ZodType {
  ...
  _parse(input) {
    const { ctx } = this._processInputParams(input);
    if (
      ctx.parsedType !== util_js_1.ZodParsedType.promise &&
...
class ZodReadonly extends ZodType {
  _parse(input) {
    ...
    return (0, parseUtil_js_1.isAsync)(result)
