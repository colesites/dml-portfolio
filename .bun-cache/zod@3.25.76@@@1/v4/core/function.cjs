...
class $ZodFunction {
  ...
  implement(func) {
    ...
    const impl = (...args) => {
      const parsedArgs = this._def.input
        ? (0, parse_js_1.parse)(this._def.input, args, undefined, {
            callee: impl,
          })
        : args;
      ...
      return this._def.output
        ? (0, parse_js_1.parse)(this._def.output, output, undefined, {
            callee: impl,
          })
        : output;
    };
  ...
  implementAsync(func) {
    ...
    const impl = async (...args) => {
      const parsedArgs = this._def.input
        ? await (0, parse_js_1.parseAsync)(this._def.input, args, undefined, {
            callee: impl,
          })
        : args;
      if (!Array.isArray(parsedArgs)) {
      ...
      return this._def.output
        ? (0, parse_js_1.parseAsync)(this._def.output, output, undefined, {
            callee: impl,
          })
        : output;
    };
  ...
  return new $ZodFunction({
    ...
    input: Array.isArray(params?.input)
      ...
      : (params?.input ??
        (0, api_js_1._array)(
          schemas.$ZodArray,
          (0, api_js_1._unknown)(schemas.$ZodUnknown),
        )),
