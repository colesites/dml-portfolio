...
> {
  ...
  (
    ...
    const impl = ((...args: any[]) => {
      const parsedArgs = this._def.input
        ? parse(this._def.input, args, undefined, { callee: impl })
        : args;
      ...
      return this._def.output
        ? parse(this._def.output, output, undefined, { callee: impl })
        : output;
    }) as any;
    return impl;
  }
  ...
  implementAsync<F extends $InferInnerFunctionTypeAsync<Args, Returns>>(
    ...
    const impl = (async (...args: any[]) => {
      const parsedArgs = this._def.input
        ? await parseAsync(this._def.input, args, undefined, { callee: impl })
        : args;
      if (!Array.isArray(parsedArgs)) {
        throw new Error(
          "Invalid arguments schema: not an array or tuple schema.",
      ...
      return this._def.output
        ? parseAsync(this._def.output, output, undefined, { callee: impl })
        : output;
    }) as any;
    return impl;
  }
...
>(params: { input: In }): $ZodFunction<$ZodTuple<In, null>, $ZodFunctionOut>;
function _function<
  const In extends Array<schemas.$ZodType> = Array<schemas.$ZodType>,
...
function _function<
  const Out extends $ZodFunctionOut = $ZodFunctionOut,
>(params: { output: Out }): $ZodFunction<$ZodFunctionIn, Out>;
function _function<
  In extends $ZodFunctionIn = $ZodFunctionIn,
...
function _function(params?: {
  ...
  return new $ZodFunction({
    ...
    input: Array.isArray(params?.input)
      ...
      : (params?.input ??
        _array(schemas.$ZodArray, _unknown(schemas.$ZodUnknown))),
