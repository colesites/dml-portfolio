...
test("locales - tr", () => {
  ...
  expect(invalidType.error!.issues[0].message).toBe(
    "Geçersiz değer: beklenen number, alınan string",
  );

  const invalidType2 = z.string().safeParse(1);
  expect(invalidType2.error!.issues[0].code).toBe("invalid_type");
  expect(invalidType2.error!.issues[0].message).toBe(
    "Geçersiz değer: beklenen string, alınan number",
  );

  const invalidValue = z.enum(["a", "b"]).safeParse(1);
  expect(invalidValue.error!.issues[0].code).toBe("invalid_value");
  expect(invalidValue.error!.issues[0].message).toBe(
    'Geçersiz seçenek: aşağıdakilerden biri olmalı: "a"|"b"',
  );
  ...
  expect(tooSmall.error!.issues[0].message).toBe(
    "Çok küçük: beklenen number >=10",
  );

  const invalidFormatRegex = z
    .string()
    .regex(/abcd/)
    .safeParse("invalid-string");
  expect(invalidFormatRegex.error!.issues[0].code).toBe("invalid_format");
  expect(invalidFormatRegex.error!.issues[0].message).toBe(
    "Geçersiz metin: /abcd/ desenine uymalı",
  );

  const invalidFormatStartsWith = z
    .string()
    .startsWith("abcd")
    .safeParse("invalid-string");
  expect(invalidFormatStartsWith.error!.issues[0].code).toBe("invalid_format");
  expect(invalidFormatStartsWith.error!.issues[0].message).toBe(
    'Geçersiz metin: "abcd" ile başlamalı',
  );

  const notMultipleOf = z.number().multipleOf(3).safeParse(10);
  expect(notMultipleOf.error!.issues[0].code).toBe("not_multiple_of");
  expect(notMultipleOf.error!.issues[0].message).toBe(
    "Geçersiz sayı: 3 ile tam bölünebilmeli",
  );

  const unrecognizedKeys = z
    .object({ a: z.string(), b: z.number() })
    .strict()
    .safeParse({ a: "a", b: 1, c: 2 });
  expect(unrecognizedKeys.error!.issues[0].code).toBe("unrecognized_keys");
  expect(unrecognizedKeys.error!.issues[0].message).toBe(
    'Tanınmayan anahtar: "c"',
  );
