...
test("floor", (t) => {
  ...
  t.equal(
    floor(maxSafeInteger),
    maxSafeInteger,
    "floor(maxSafeInteger) === maxSafeInteger",
  );
...
test("isFinite", (t) => {
  ...
  forEach(v.nonNumbers, (nonNumber) => {
    t.equal(
      isFinite(nonNumber),
      false,
      "isFinite(" + inspect(nonNumber) + ") === false",
    );
  });
...
test("isInteger", (t) => {
  forEach(
    ...
    (nonInteger) => {
      t.equal(
        isInteger(nonInteger),
        false,
        "isInteger(" + inspect(nonInteger) + ") === false",
      );
    },
  );
...
test("isNegativeZero", (t) => {
  ...
  forEach(v.nonNumbers, (nonNumber) => {
    t.equal(
      isNegativeZero(nonNumber),
      false,
      "isNegativeZero(" + inspect(nonNumber) + ") === false",
    );
  });
...
test("constants", (t) => {
  t.equal(
    typeof maxArrayLength,
    "number",
    'typeof maxArrayLength === "number"',
  );
  t.equal(
    typeof maxSafeInteger,
    "number",
