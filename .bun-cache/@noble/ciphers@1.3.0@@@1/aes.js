...
function validateBlockDecrypt(data) {
  ...
  if (data.length % BLOCK_SIZE !== 0) {
    throw new Error(
      "aes-(cbc/ecb).decrypt ciphertext should consist of blocks with size " +
        BLOCK_SIZE,
    );
  }
}
...
exports.gcmsiv = (0, utils_ts_1.wrapCipher)(
  ...
  function aessiv(key, nonce, AAD) {
    ...
    function processSiv(encKey, tag, input) {
      const block = (0, utils_ts_1.copyBytes)(tag);
...
function encryptBlock(xk, block) {
  ...
  const { s0, s1, s2, s3 } = encrypt(xk, b32[0], b32[1], b32[2], b32[3]);
...
function decryptBlock(xk, block) {
  ...
  const b32 = (0, utils_ts_1.u32)(block);
  const { s0, s1, s2, s3 } = decrypt(xk, b32[0], b32[1], b32[2], b32[3]);
  (b32[0] = s0), (b32[1] = s1), (b32[2] = s2), (b32[3] = s3);
  return block;
}
/**
