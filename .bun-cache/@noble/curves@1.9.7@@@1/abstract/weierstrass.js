...
function _splitEndoScalar(k, basis, n) {
  ...
  const MAX_NUM =
    (0, utils_ts_1.bitMask)(Math.ceil((0, utils_ts_1.bitLen)(n) / 2)) + _1n; // Half bits of N
...
function validateSigOpts(opts, def) {
  const optsn = {};
  for (const optName of Object.keys(def)) {
    // @ts-expect-error
...
function _normFnElement(Fn, key) {
  ...
  if (typeof key === "bigint") {
    ...
    const bytes = (0, utils_ts_1.ensureBytes)("private key", key);
    try {
  ...
  class Point {
    ...
    static fromBytes(bytes) {
      const P = Point.fromAffine(
        decodePoint((0, utils_ts_1._abytes2)(bytes, undefined, "point")),
      );
      P.assertValidity();
      return P;
    }
    ...
    multiplyUnsafe(sc) {
      ...
      if (sc === _0n || this.is0()) return Point.ZERO;
      if (sc === _1n) return this; // fast-path
      if (wnaf.hasCache(this)) return this.multiply(sc);
      if (endo) {
        const { k1neg, k1, k2neg, k2 } = splitEndoScalarN(sc);
        const { p1, p2 } = (0, curve_ts_1.mulEndoUnsafe)(Point, this, k1, k2); // 30% faster vs wnaf.unsafe
        return finishEndo(endo.beta, p1, p2, k1neg, k2neg);
      } else {
        return wnaf.unsafe(this, sc);
      }
    }
...
function SWUFpSqrtRatio(Fp, Z) {
  ...
  let sqrtRatio = (u, v) => {
    ...
    const isQR = Fp.eql(tv5, Fp.ONE); // 12. isQR = tv5 == 1
  ...
  if (Fp.ORDER % _4n === _3n) {
    ...
    sqrtRatio = (u, v) => {
      ...
      const y = Fp.cmov(y2, y1, isQR); // 10. y = CMOV(y2, y1, isQR)
...
function ecdsa(Point, hash, ecdsaOpts = {}) {
  ...
  class Signature {
    ...
    static fromHex(hex, format) {
      return Signature.fromBytes((0, utils_ts_1.hexToBytes)(hex), format);
    }
  ...
  function validateMsgAndHash(message, prehash) {
    ...
    return prehash
      ? (0, utils_ts_1._abytes2)(hash(message), undefined, "prehashed message")
      : message;
  }
  /**
...
function _weierstrass_legacy_opts_to_new(c) {
  ...
  const Fp = c.Fp;
  const allowedLengths = c.allowedPrivateKeyLengths
    ? Array.from(
