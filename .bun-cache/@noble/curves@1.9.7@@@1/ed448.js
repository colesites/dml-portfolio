...
exports.edwardsToMontgomery =
  exports.hash_to_decaf448 =
...
const shake256_114 = /* @__PURE__ */ (0, utils_js_1.createHasher)(() =>
  sha3_js_1.shake256.create({ dkLen: 114 }),
);
const shake256_64 = /* @__PURE__ */ (0, utils_js_1.createHasher)(() =>
  sha3_js_1.shake256.create({ dkLen: 64 }),
);
// prettier-ignore
const _1n = BigInt(1),
  _2n = BigInt(2),
...
const Fp = /* @__PURE__ */ (() =>
  (0, modular_ts_1.Field)(ed448_CURVE.p, { BITS: 456, isLE: true }))();
const Fn = /* @__PURE__ */ (() =>
  (0, modular_ts_1.Field)(ed448_CURVE.n, { BITS: 456, isLE: true }))();
// decaf448 uses 448-bit (56-byte) keys
const Fp448 = /* @__PURE__ */ (() =>
  (0, modular_ts_1.Field)(ed448_CURVE.p, { BITS: 448, isLE: true }))();
const Fn448 = /* @__PURE__ */ (() =>
  (0, modular_ts_1.Field)(ed448_CURVE.n, { BITS: 448, isLE: true }))();
...
function dom4(data, ctx, phflag) {
  ...
  return (0, utils_js_1.concatBytes)(
    (0, utils_ts_1.asciiToBytes)("SigEd448"),
...
function map_to_curve_elligator2_curve448(u) {
  ...
  const e1 = Fp.eql(tv1, Fp.ONE); // 2.   e1 = tv1 == 1
  tv1 = Fp.cmov(tv1, Fp.ZERO, e1); // 3.  tv1 = CMOV(tv1, 0, e1)  # If Z * u^2 == -1, set tv1 = 0
  const xd = Fp.sub(Fp.ONE, tv1); // 4.   xd = 1 - tv1
  const x1n = Fp.neg(ELL2_J); // 5.  x1n = -J
  let tv2 = Fp.sqr(xd); // 6.  tv2 = xd^2
  const gxd = Fp.mul(tv2, xd); // 7.  gxd = tv2 * xd          # gxd = xd^3
  ...
  const x2n = Fp.mul(x1n, Fp.neg(tv1)); // 17. x2n = -tv1 * x1n        # x2 = x2n / xd = -1 * u^2 * x1n / xd
  let y2 = Fp.mul(y1, u); // 18.  y2 = y1 * u
  y2 = Fp.cmov(y2, Fp.ZERO, e1); // 19.  y2 = CMOV(y2, 0, e1)
  tv2 = Fp.sqr(y1); // 20. tv2 = y1^2
  tv2 = Fp.mul(tv2, gxd); // 21. tv2 = tv2 * gxd
  const e2 = Fp.eql(tv2, gx1); // 22.  e2 = tv2 == gx1
  const xn = Fp.cmov(x2n, x1n, e2); // 23.  xn = CMOV(x2n, x1n, e2)  # If e2, x = x1, else x = x2
  let y = Fp.cmov(y2, y1, e2); // 24.   y = CMOV(y2, y1, e2)    # If e2, y = y1, else y = y2
  const e3 = Fp.isOdd(y); // 25.  e3 = sgn0(y) == 1        # Fix sign of y
  y = Fp.cmov(y, Fp.neg(y), e2 !== e3); // 26.   y = CMOV(y, -y, e2 XOR e3)
  return { xn, xd, yn: y, yd: Fp.ONE }; // 27. return (xn, xd, y, 1)
}
function map_to_curve_elligator2_edwards448(u) {
  const { xn, xd, yn, yd } = map_to_curve_elligator2_curve448(u); // 1. (xn, xd, yn, yd) = map_to_curve_elligator2_curve448(u)
  const xn2 = Fp.sqr(xn); // 2.  xn2 = xn^2
  const xd2 = Fp.sqr(xd); // 3.  xd2 = xd^2
  const xd4 = Fp.sqr(xd2); // 4.  xd4 = xd2^2
  const yn2 = Fp.sqr(yn); // 5.  yn2 = yn^2
  const yd2 = Fp.sqr(yd); // 6.  yd2 = yd^2
  ...
  const tv3 = Fp.mul(yn2, _4n); // 15. tv3 = 4 * yn2
  ...
  const e = Fp.eql(tv1, Fp.ZERO); // 33.   e = tv1 == 0
...
function calcElligatorDecafMap(r0) {
  ...
  const { isValid: was_square, value: v } = uvRatio(
    ONE_MINUS_TWO_D,
    mod((r + _1n) * u1),
  ); // 4
  let v_prime = v; // 5
  ...
  let s_abs = s;
  if ((0, modular_ts_1.isNegativeLE)(s, P)) s_abs = mod(-s);
  const s2 = s * s;
  const W0 = mod(s_abs * _2n); // 8
  const W1 = mod(s2 + _1n); // 9
  const W2 = mod(s2 - _1n); // 10
  const W3 = mod(v_prime * s * (r - _1n) * ONE_MINUS_TWO_D + sgn); // 11
  return new exports.ed448.Point(
    mod(W0 * W3),
    mod(W2 * W1),
    mod(W1 * W3),
    mod(W0 * W2),
...
class _DecafPoint extends edwards_ts_1.PrimeEdwardsPoint {
  ...
  static fromBytes(bytes) {
    ...
    if (
      !(0, utils_ts_1.equalBytes)(Fn448.toBytes(s), bytes) ||
    ...
    if ((0, modular_ts_1.isNegativeLE)(u3, P)) u3 = mod(-u3);
  ...
  static fromHex(hex) {
    return _DecafPoint.fromBytes(
      (0, utils_ts_1.ensureBytes)("decafHex", hex, 56),
    );
  }
  ...
  toBytes() {
    ...
    if ((0, modular_ts_1.isNegativeLE)(ratio, P)) ratio = mod(-ratio);
...
// prettier-ignore
_DecafPoint.BASE = /* @__PURE__ */ (() =>
  new _DecafPoint(exports.ed448.Point.BASE).multiplyUnsafe(_2n))();
// prettier-ignore
_DecafPoint.ZERO = /* @__PURE__ */ (() =>
  new _DecafPoint(exports.ed448.Point.ZERO))();
// prettier-ignore
...
exports.decaf448_hasher = {
  hashToCurve(msg, options) {
    ...
    return decaf448_map(
      (0, hash_to_curve_ts_1.expand_message_xof)(
        msg,
        DST,
        112,
        224,
  ...
  hashToScalar(msg, options = { DST: hash_to_curve_ts_1._DST_scalar }) {
    ...
    const xof = (0, hash_to_curve_ts_1.expand_message_xof)(
      msg,
      options.DST,
      64,
      256,
...
function edwardsToMontgomeryPub(edwardsPub) {
  return exports.ed448.utils.toMontgomery(
