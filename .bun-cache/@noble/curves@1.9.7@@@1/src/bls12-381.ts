...
import type { BigintTuple, Fp, Fp2, Fp6, Fp12 } from "./abstract/tower.ts";
...
const { G2psi, G2psi2 } = psiFrobenius(
  Fp,
  Fp2,
...
function signatureG1FromBytes(hex: Hex): WeierstrassPoint<Fp> {
  const { infinity, sort, value } = parseMask(
    ensureBytes("signatureHex", hex, 48),
  );
  const P = Fp.ORDER;
...
function pointG2FromBytes(bytes: Uint8Array): AffinePoint<Fp2> {
  ...
  const slc = (b: Uint8Array, from: number, to?: number) =>
    bytesToNumberBE(b.slice(from, to));
...
const isogenyMapG2 = isogenyMap(
  ...
  [
    ...
    i.map((pair) => Fp2.fromBigTuple(pair.map(BigInt) as BigintTuple)),
  ) as [Fp2[], Fp2[], Fp2[], Fp2[]],
);
